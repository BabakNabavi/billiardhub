...
* **حفره‌ی بحرانیِ کشف‌شده و بسته‌شده:** کلیدِ `anon` داخلِ باندلِ مرورگر منتشر می‌شود و سه جدولِ پایه RLS نداشتند. اندازه‌گیریِ زنده پیش از اصلاح: `GET /rest/v1/users` → ۲۰۰ با ردیف‌های واقعی (کد ملی، هشِ رمز، کدِ OTP) و `PATCH /rest/v1/users` → ۲۰۴. یعنی هر کسی می‌توانست کد ملیِ خود را عوض کند (سهمیه‌ی تازه)، باشگاهی را verified کند (سهمیه‌ی ۴تایی) یا خودش را admin کند. پس از مایگریشن ۰۱۷: هر چهار مسیر ۴۰۱ می‌دهند و `products` فقط خواندنی مانده. Storage و Realtime دست‌نخورده کار می‌کنند.
...
* **نیاز به تصمیمِ مالک:** به‌خاطر حفره‌ی RLS، هشِ رمزها و کدهای OTP و کد ملی‌ها مدتی از بیرون خواندنی بوده‌اند. حفره بسته شد، ولی چرخاندنِ رمزها (اجبار به بازنشانی) یا بی‌اعتبارکردنِ نشست‌ها یک اقدامِ کاربرمحور است و بدونِ دستور شما انجام نشده.
