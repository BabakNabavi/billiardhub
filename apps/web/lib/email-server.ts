/* ─────────────────────────────────────────────────────────────
   اعتبارسنجی ایمیل — s.api.ir/api/sw1/CheckEmail

   سرویس فقط می‌گوید این نشانی واقعی و فعال هست یا نه؛ کدی به ایمیل
   فرستاده نمی‌شود.

   تله‌ی همیشگی این خانواده از سرویس‌ها: «نامعتبر» هم HTTP 200 با
   success:true برمی‌گردد و فقط data=false می‌شود. آن حالت یک پاسخ
   منفی واقعی است، نه «سرویس در دسترس نیست» — اگر اشتباه بگیریمش،
...
export async function checkEmail(email: string): Promise<EmailCheck> {
  ...
  try {
    ...
    /* سطح دسترسی/کلید/اعتبار ⇒ نامشخص، نه «نامعتبر» */
    ...
    if (denied) {
      ...
      return { ok: false, unavailable: true, message: 'سرویس اعتبارسنجی ایمیل در دسترس نیست' }
    }
    if (!r.ok || !j) {
      console.error('CheckEmail failed:', r.status)
      return { ok: false, unavailable: true, message: 'استعلام ایمیل ناموفق بود' }
    }

    /* success:true + data:false ⇒ پاسخ منفی واقعی */
    if (j.success === false) {
      return { ok: false, unavailable: true, message: j.message || 'استعلام ایمیل ناموفق بود' }
    }

    const valid = j.data === true
    return {
      ok: true, valid,
      message: valid ? undefined : 'این نشانی ایمیل فعال نیست یا وجود ندارد',
    }
  } catch {
    return { ok: false, unavailable: true, message: 'ارتباط با سرویس اعتبارسنجی ایمیل برقرار نشد' }
  }
}
