...
/* ─────────────────────────────────────────────────────────────
   منبعِ واحدِ باشگاه‌ها — همان داده‌ای که صفحه‌ی /clubs نمایش می‌دهد:
   باشگاه‌های واقعیِ ثبت‌شده. هر جای سایت که «انتخاب باشگاه» لازم دارد
...
/* SAMPLE_CLUBS حذف شد — شش باشگاهِ ساختگی که روی صفحه‌ی عمومی
   نمایش داده می‌شدند و باشگاه‌های واقعیِ هم‌نام را پنهان می‌کردند. */

export interface ClubOption { id: string; name: string; city: string }

/* فقط باشگاه‌های واقعی.

   پیش‌تر شش باشگاهِ نمونه اینجا بودند و همیشه اولِ فهرست می‌آمدند، و
   باشگاه‌های واقعیِ هم‌نام با آن‌ها فیلتر می‌شدند. نتیجه‌اش این بود که
   باشگاه‌دارِ واقعی، نسخه‌ی ساختگی را به‌جای باشگاهِ خودش می‌دید. */
