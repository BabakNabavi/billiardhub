/* ─────────────────────────────────────────────────────────────
   ...
   ترتیب:
     ۱) کلیدِ stories_quota_enabled خاموش ⇒ سهمیه‌ی رایگانِ نقش اعمال
        می‌شود (همان رفتارِ امروزِ سایت)، ولی بسته اجباری نیست
     ۲) بسته‌ی فعال ⇒ سهمیه‌ی همان بسته
     ۳) وگرنه ⇒ سهمیه‌ی رایگانِ نقش

   تفاوتِ ریزِ مهم با آگهی: سقفِ استوری از اول وجود داشته (هاردکد در
   /api/social/stories) و حذفش رفتارِ سایت را عوض می‌کرد. پس این‌جا
   کلیدِ enabled فقط تعیین می‌کند «خریدِ بسته معنا دارد یا نه»؛ سقفِ
   رایگان همیشه اعمال می‌شود.
   ───────────────────────────────────────────────────────────── */
...
export async function getStoryQuotaState(
  ...
  if (limit <= 0 && plan) return { ...base, allowed: true }   // بسته‌ی نامحدود
  ...
  if (limit <= 0) {
    return {
      ...base, allowed: false,
      message: enabled
        ? 'نقشِ شما سهمیه‌ی رایگانِ استوری ندارد؛ برای انتشار استوری یکی از بسته‌ها را تهیه کنید.'
        : 'حساب شما امکان انتشار استوری ندارد.',
    }
  }
    ...
    return {
      ...
      message: plan
        ...
        : enabled
          ...
          : `سقفِ استوریِ این ${PERIOD_FA[period]} شما (${limit}) پر شده است.`,
    }
  }
