/* ─────────────────────────────────────────────────────────────
   استعلامِ اعتبارِ مجوزِ شغلی (جواز کسب) — سمتِ سرور.

   مثلِ بقیه‌ی سرویس‌های این ارائه‌دهنده، «پیدا نشد» هم HTTP 200 با
   success=true برمی‌گرداند و فقط data را خالی می‌گذارد؛ پس آن حالت
   پاسخِ منفیِ واقعی است، نه «سرویس در دسترس نیست».
   ───────────────────────────────────────────────────────────── */
...
/** «۱۴۰۲/۰۱/۰۱» گذشته است؟ مقایسه در همان تقویمِ شمسی انجام می‌شود. */
...
export async function lookupLicense(trackingCode: string): Promise<LicenseResult> {
  ...
  if (!key) return { ok: false, unavailable: true, message: 'سرویسِ استعلامِ مجوز پیکربندی نشده است' }
  ...
  try {
    ...
    return { ok: false, unavailable: true, message: 'خطا در اتصال به سرویسِ استعلامِ مجوز' }
  }
  ...
  if (denied) {
    ...
    return { ok: false, unavailable: true, message: 'سرویسِ استعلامِ مجوز در دسترس نیست' }
  }
  if (!j) return { ok: false, unavailable: true, message: 'پاسخِ سرویسِ استعلامِ مجوز خوانده نشد' }

  if (j.success === false) {
    if (j.code === 400) return { ok: false, message: 'کد پیگیری مجوز معتبر نیست' }
    console.error('License failed:', j.message || j.code)
    return { ok: false, unavailable: true, message: 'استعلامِ مجوز ناموفق بود' }
  }

  /* success=true ولی بدونِ داده ⇒ مجوزی با این کد پیدا نشد */
