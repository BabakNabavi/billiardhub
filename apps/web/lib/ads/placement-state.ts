/* ─────────────────────────────────────────────────────────────
   تایپ‌ها و تبدیلِ وضعیتِ جایگاه‌های تبلیغاتی — مشترکِ سرور و کلاینت.

   چرا از `components/ads/AdSlot.tsx` بیرون کشیده شد: آن فایل
   `'use client'` است، پس هرچه از آن export شود یک «ارجاعِ کلاینت»
   ...
   سمتِ سرور هم در دسترس بود؛ با اضافه‌شدنِ یک یالِ جدید به گرافِ
   ماژول‌ها آن شانس از بین رفت و صفحه‌ی اصلی ۵۰۰ داد.

   این فایل هیچ `'use client'`ی ندارد، پس هر دو طرف می‌توانند از آن
   بخوانند و مرزِ سرور/کلاینت دیگر به ترتیبِ چانک‌بندی وابسته نیست.
   ───────────────────────────────────────────────────────────── */
...
/**
 * تبدیلِ payloadِ خام به وضعیتِ جایگاه.
 *
