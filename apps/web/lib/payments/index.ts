...
export function getPaymentProvider(name?: string): PaymentProvider {
  ...
  const p = make()
  /* اگر درگاه انتخابی تنظیم نشده باشد، به‌جای شکست، mock استفاده می‌شود
     تا فلوی سایت هرگز قطع نشود (در لاگ سرور هشدار ثبت می‌شود). */
  if (!p.isConfigured()) {
...
/* آیا یک درگاه **واقعی** پیکربندی شده است؟

   `getPaymentProvider` عمداً وقتی درگاه تنظیم نباشد به mock برمی‌گردد
   تا فلوی سایت قطع نشود. این رفتار برای رزرو قابل قبول است، ولی
   برای پول واقعی خطرناک: باعث می‌شود سیستم «پرداخت موفق» اعلام کند
   بدون آنکه ریالی جابه‌جا شده باشد.

   هر جریانی که پول واقعی می‌گیرد باید **اول** این را بپرسد و اگر
