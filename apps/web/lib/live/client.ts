...
export const fetchLiveSessions = () => j<LiveSession[]>(fetch('/api/live', { cache: 'no-store' }), [])
export const fetchLiveSession = (id: string) =>
  j<LiveSession | null>(fetch(`/api/live?id=${encodeURIComponent(id)}`, { cache: 'no-store' }), null)
...
export const startLive = (body: { clubId: string; clubName: string; ownerKey: string; title: string; discipline: string }) =>
  j<{ ok?: boolean; session?: LiveSession; message?: string }>(
    fetch('/api/live', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'start', ...body }) }), {})

export const beatLive = (id: string, ownerKey: string, viewers: number) =>
  j(fetch('/api/live', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'beat', id, ownerKey, viewers }) }), {})

export const stopLive = (id: string, ownerKey: string) =>
  j(fetch('/api/live', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'stop', id, ownerKey }) }), {})
