/* ─────────────────────────────────────────────────────────────
   منطقِ راهنمای «افزودن به صفحه‌ی اصلی» (Add to Home Screen) — iOS

   این فایل عمداً هیچ Reactای، هیچ DOMای و هیچ side-effectای در سطحِ
   ماژول ندارد. همه‌ی ورودی‌ها از بیرون تزریق می‌شوند (`A2hsEnv`) تا
   بشود بدونِ مرورگر و بدونِ شبیه‌سازِ دستگاه، با UAهای واقعی تستش کرد.

   جداییِ منطق از UI عمدی است: تصمیمِ «نشان بده یا نه» چیزی است که باید
   اثبات‌پذیر باشد، نه چیزی که فقط روی یک آیفونِ واقعی معلوم شود.
   ───────────────────────────────────────────────────────────── */

/** کلیدِ localStorage — با پیشوندِ `bh_` مثلِ بقیه‌ی کلیدهای پروژه.
 *  بررسی شد که با هیچ کلیدِ موجودی تداخل ندارد. */
 ...
 * از یک مکِ واقعی قابلِ تفکیک نیست — مگر با یک نشانه‌ی واقعیِ سخت‌افزاری.
 * `maxTouchPoints` همان نشانه است: مکِ دسکتاپ صفحه‌ی لمسی ندارد (۰)،
 * آیپد دارد (۵).
 */
...
/* مرورگرهای غیرِ سافاری روی iOS و وب‌ویوهای درون‌برنامه‌ای.
   همه‌ی این‌ها روی iOS موتورِ WebKit دارند، پس رشته‌ی «Safari» در UAشان
   هست — یعنی تشخیصِ مثبت (`/Safari/`) بی‌فایده است و باید فهرستِ منفی
   داشت. در هیچ‌کدامِ این‌ها منوی Share گزینه‌ی افزودن به صفحه‌ی اصلی
   ندارد، پس نمایشِ راهنما فقط کاربر را سردرگم می‌کند. */
...
/**
 * «سافاری هست؟» — با اطمینانِ درجه‌بندی‌شده، نه یک بولِ شکننده.
 *
 * `navigator.standalone` یک ویژگیِ اختصاصیِ WebKitِ سافاریِ iOS است و در
 * هیچ مرورگرِ دیگری تعریف نشده. **بودنش** تأییدِ قطعی است؛ **نبودنش**
 * رد نیست (ممکن است نسخه‌ای در آینده حذفش کند) — در آن حالت به `likely`
 * بسنده می‌کنیم و باز هم نشان می‌دهیم. این همان «رفتارِ امن» است:
 * شکستِ تشخیص به «مخفی‌شدنِ همیشگی» منجر نمی‌شود.
 */
...
export interface A2hsEnv {
  ...
  /** `navigator.standalone` — عمداً `unknown` است چون در اکثرِ مرورگرها وجود ندارد */
...
/**
 * هر دو راهِ تشخیصِ نصب‌بودن پوشش داده می‌شود:
 *   • `navigator.standalone` — راهِ قدیمی و مخصوصِ iOS
 *   • `display-mode: standalone|fullscreen|minimal-ui` — استانداردِ PWA
 * (مانیفستِ پروژه `display: standalone` دارد، ولی دوتای دیگر هم چک
...
/* ── پل به مرورگر ───────────────────────────────────────────────
   ...
   SSR صدا زده نمی‌شود (فقط از داخلِ useEffect). */
...
export function readEnv(): A2hsEnv {
  ...
  try { stored = localStorage.getItem(A2HS_STORAGE_KEY) } catch { /* حالتِ خصوصی/محدود */ }
