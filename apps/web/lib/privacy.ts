...
/* ─────────────────────────────────────────────────────────────
   پوشاندنِ کلیدِ مالک در پاسخ‌های عمومی.

   `ownerKey` در سراسرِ پروژه از `user.phone || user.id` ساخته می‌شود،
   پس برای بیشترِ کاربران **همان شماره‌ی موبایل** است. این کلید در
   فهرست‌های عمومی — ویدیوها، استوری‌ها، پخشِ زنده، کانال‌ها — عیناً
   برمی‌گشت، یعنی هر کسی بدونِ ورود می‌توانست شماره‌ی موبایلِ سازنده‌ها
   را جمع کند.

   راهِ حل بدونِ شکستنِ کلاینت: به‌جای حذفِ کامل، کلید با HMAC به یک
   شناسه‌ی پایدارِ غیرقابلِ‌بازگشت تبدیل می‌شود. مقایسه‌ی «مالِ من است؟»
...
/** کلیدِ عمومیِ پایدار و غیرقابلِ‌بازگشت از یک ownerKey */
...
/** آیا این کلیدِ عمومی به همین ownerKey تعلق دارد؟ */
...
/* فیلدهایی که هرگز نباید در پاسخِ عمومی بیایند */
const SENSITIVE = new Set(['ownerPhone', 'phone', 'nationalId', 'national_id', 'email', 'password'])

/**
 * یک ردیفِ عمومی را امن می‌کند:
 *   • ownerKey ⇒ هشِ پایدار
 *   • فیلدهای حساس حذف
 *   • اگر نشستِ کاربر داده شود، پرچمِ `mine` اضافه می‌شود
 */
