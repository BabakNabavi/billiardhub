...
/* قفلِ حساب: **۱۵ دقیقه‌ی ثابت** پس از سه تلاشِ ناموفق.

   پیش‌تر پلکانی بود (۵د → ۱۵د → ۱س → ۴س). عملاً یعنی کاربری که رمزِ
   خودش را چند بار اشتباه می‌زد، ساعت‌ها از حسابِ خودش بیرون می‌ماند —
   هزینه‌ای که تقریباً همیشه گردنِ کاربرِ واقعی می‌افتاد، نه مهاجم.

   با ۱۵ دقیقه‌ی ثابت، سقفِ حدس روی یک حساب می‌شود ۳ تلاش در ربع ساعت
   (≈ ۲۸۸ در شبانه‌روز). برای حمله‌ی دیکشنری بی‌فایده است، و قفلِ IP
