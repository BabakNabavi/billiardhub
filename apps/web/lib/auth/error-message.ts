...
/* ─────────────────────────────────────────────────────────────
   ترجمه‌ی خطای درخواست به پیام درست برای کاربر.
   ...
   یعنی هر خطایی که **پاسخی از سرور نداشت** — قطعی لحظه‌ای شبکه،
   تایم‌اوت، VPNای که وسط کار افت می‌کند، یا سردشدن تابع سرور — به
   ...
   حالا سه حالت از هم جدا می‌شوند:
     • اصلاً به سرور نرسید      ⇒ مشکل اتصال
     • رسید ولی خطای سرور بود   ⇒ مشکل از ما، نه از کاربر
     • رسید و سرور رد کرد       ⇒ همان پیام خود سرور
   ───────────────────────────────────────────────────────────── */

export interface AuthError {
  message: string
  /** آیا مشکل از ورودی کاربر بود؟ اگر نه، نباید فیلدها قرمز شوند */
  isCredentialError: boolean
  /** برای قفل موقت — ثانیه‌های باقی‌مانده */
  retryAfter?: number
}
...
export function toAuthError(err: unknown, fallback = 'انجام نشد'): AuthError {
  ...
  /* خطای سمت سرور ⇒ تقصیر کاربر نیست و نباید پیام «رمز اشتباه» بگیرد */
  ...
  /* قفل موقت یا محدودیت نرخ */
  if (status === 429) {
    return {
      message: serverMsg || 'تلاش بیش از حد. کمی صبر کنید.',
      isCredentialError: false,
      retryAfter: ax?.response?.data?.retryAfter,
    }
  }

  /* توکن امنیتی منقضی — با تازه‌کردن صفحه حل می‌شود */
