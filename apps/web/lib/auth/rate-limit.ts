/* ─────────────────────────────────────────────────────────────
   محدودیت نرخ — سرورساید و مشترک بین نمونه‌ها (فاز ۷A).
   ...
   بخورد. جدول مشترک این را می‌بندد و به وابستگی تازه (Redis) هم
   نیاز ندارد.

   کلید شمارش عمداً ترکیبی است: هم IP و هم شناسه‌ی هدف (شماره/ایمیل).
   با IP تنها، NAT یک اپراتور همه را قفل می‌کرد؛ با هدف تنها،
   چرخاندن شماره محدودیت را دور می‌زد.
   ───────────────────────────────────────────────────────────── */
...
export interface RateRule {
  /** نام منطقی عملیات — در کلید می‌آید تا سطل‌ها قاطی نشوند */
  action: string
  /** سقف تلاش در پنجره */
  max: number
  /** طول پنجره به ثانیه */
...
/** IP واقعی کاربر پشت پراکسی Vercel */
export function ipOf(req: NextRequest): string {
  const fwd = req.headers.get('x-forwarded-for') || ''
  /* اولین مقدار = نزدیک‌ترین کلاینت به لبه؛ بقیه را خود Vercel اضافه
     می‌کند. هدر ارسالی کاربر جایگزین این نمی‌شود چون Vercel آن را
     بازنویسی می‌کند. */
 ...
 *
 * fail-open عمدی: اگر خود جدول در دسترس نباشد، درخواست را رد نمی‌کنیم
 * — قفل‌کردن ورود همه‌ی کاربران به‌خاطر خطای زیرساخت بدتر از نبود
...
/** پاسخ استاندارد ۴۲۹ با هدر Retry-After */
export function tooMany(retryAfterSec: number) {
  return new Response(
    JSON.stringify({ message: `تلاش بیش از حد. ${retryAfterSec} ثانیه دیگر دوباره تلاش کنید.`, retryAfter: retryAfterSec }),
    { status: 429, headers: { 'Content-Type': 'application/json', 'Retry-After': String(retryAfterSec) } },
  )
}

/* سقف‌های پیش‌فرض — محافظه‌کارانه تا کاربر واقعی گیر نکند */
export const RULES = {
  ...
  /* استعلام شاهکار/ثبت‌احوال: سرویس پولی بیرونی است و بدون سقف هم
     اعتبار حساب را می‌سوزاند و هم به یک اوراکل «این کد ملی مال این
