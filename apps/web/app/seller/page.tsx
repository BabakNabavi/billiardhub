...
/* ─────────────────────────────────────────────────────────────
   ...
   نسخه‌ی قبلی تماماً روی داده‌ی ساختگی بنا شده بود: پنج سفارش جعلی با
   نام خریداران ساختگی، چهار محصول ثابت، نمودار درآمد دوازده‌ماهه با
   اعداد دلخواه، و «امتیاز فروشنده» که از همان اعداد درمی‌آمد. هیچ‌کدام
   به دیتابیس وصل نبودند — فروشنده عددهایی می‌دید که هیچ ربطی به
   فروشگاهش نداشت.

   حالا فقط چیزی نشان داده می‌شود که واقعاً وجود دارد:

     • محصولات ⇒ جدول `products` با `sellerId` برابر خود کاربر
     • آمار    ⇒ از همان محصولات محاسبه می‌شود (تعداد، بازدید، موجودی)

   آنچه حذف شد و برنگشت:
     • سفارش‌ها — جدول `orders` در این پروژه اصلاً وجود ندارد
     • درآمد و نمودار ماهانه — بدون سفارش، عددی برای نمایش نیست
     • پیام‌ها — پیش‌تر با پرچم تعاملات اجتماعی برداشته شد
...
function SellerContent() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      /* پاسخ ممکن است {products:[…]} یا خود آرایه باشد */
      const list: Product[] = Array.isArray(j) ? j : (j.products ?? j.data ?? []);
      setProducts(list);
    } catch {
      setError('خواندن محصولات انجام نشد. اتصال را بررسی کنید و دوباره تلاش کنید.');
      setProducts([]);
    } finally {
      setLoading(false);
    }
        ...
        <div style={{ maxWidth: 1180, margin: '0 auto', padding: '22px clamp(14px,4vw,32px) 0' }}>
          ...
          {loading ? (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9, padding: 50, color: SEC, fontSize: 13 }}>
              <Loader2 size={17} className="spin" /> در حال خواندن آگهی‌ها…
            </div>
          ) : shown.length === 0 ? (
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
              ...
              <div>
                ...
                <p style={{ margin: 0, fontSize: 12.5, lineHeight: 2, color: '#6B5225' }}>
                  ...
                  به‌جای نشان‌دادن آمار ساختگی، این بخش تا آماده‌شدن زیرساخت خالی می‌ماند.
                </p>
              </div>
            </div>
          </div>
