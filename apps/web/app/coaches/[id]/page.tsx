      ...
      <div style={{ minHeight:'100vh', background:'#F7F7F5', paddingBottom:'80px' }}>
        ...
        <div style={{ position:'relative', height:'clamp(460px,60vh,640px)', overflow:'hidden' }}>
          ...
          <div style={{ position:'absolute', top:'24px', left:0, right:0, padding:'0 clamp(16px,4vw,48px)', display:'flex', justifyContent:'space-between', zIndex:10 }}>
            <Link href="/coaches" style={{ display:'flex', alignItems:'center', gap:'6px', color:'rgba(255,255,255,0.5)', fontSize: '13px', textDecoration:'none', background:'rgba(0,0,0,0.4)', backdropFilter:'blur(16px)', border:'1px solid rgba(0,0,0,0.06)', borderRadius:'10px', padding:'7px 14px' }}>
          ...
          <div style={{ position:'absolute', bottom:0, left:0, right:0, padding:'clamp(24px,4vw,52px)', zIndex:10, opacity:heroOpacity }}>
            <div style={{ display:'flex', alignItems:'flex-end', gap:'24px', flexWrap:'wrap' }}>
              ...
              <div style={{ flex:1, minWidth:'200px' }}>
                ...
                <div style={{ fontSize: '15px', color:'rgba(255,255,255,0.5)', marginBottom:'10px' }}>{coach.title}</div>
                ...
                <div style={{ display:'flex', gap:'10px', flexWrap:'wrap' }}>
                  ...
                  <div style={{ display:'flex', gap:'2px', alignItems:'center', background:'rgba(0,0,0,0.06)', backdropFilter:'blur(12px)', borderRadius:'20px', padding:'5px 13px' }}>
                    ...
                    <span style={{ fontSize: '13px', color:'rgba(255,255,255,0.7)', marginRight:'5px' }}>{coach.rating}</span>
                    <span style={{ fontSize: '11px', color:'rgba(255,255,255,0.35)', marginRight:'3px' }}>({toFa(coach.reviewCount)})</span>
                  </div>
                </div>
              </div>
              ...
              <div style={{ display:'flex', gap:'10px', flexWrap:'wrap', flexShrink:0 }}>
                ...
                <button style={{ display:'flex', alignItems:'center', gap:'7px', padding:'11px 18px', borderRadius:'12px', background:'rgba(0,0,0,0.05)', border:'1px solid rgba(0,0,0,0.08)', color:'rgba(255,255,255,0.7)', fontSize: '14px', fontWeight:600, cursor:'pointer', fontFamily:'inherit', transition:'all 0.3s' }}>
          ...
          <div className="profile-g" style={{ display:'grid', gridTemplateColumns:'1fr 340px', gap:'28px', alignItems:'start' }}>
              ...
              {tab==='overview' && (
                <div style={{ animation:'fadeUp 0.4s ease both', display:'flex', flexDirection:'column', gap:'20px' }}>
                  ...
                  <div style={{ background:'#FFFFFF', border:'1px solid rgba(0,0,0,0.07)', borderRadius:'20px', padding:'26px' }}>
                    ...
                    <p style={{ fontSize: '15px', color:'rgba(0,0,0,0.50)', lineHeight:1.9, margin:'0 0 20px' }}>{coach.bio}</p>
                    ...
                    <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
                      {coach.certifications.map((cert: string, i: number) => (
                        <div key={i} style={{ display:'flex', alignItems:'center', gap:'12px', padding:'13px 16px', background:'rgba(245,158,11,0.04)', border:'1px solid rgba(245,158,11,0.12)', borderRadius:'14px' }}>
                          ...
                          <span style={{ fontSize: '14px', color:'rgba(0,0,0,0.45)', fontWeight:600 }}>{cert}</span>
                    ...
                    <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
                      {coach.achievements.map((a: string, i: number) => (
                        <div key={i} style={{ display:'flex', alignItems:'center', gap:'10px', padding:'10px 14px', background:'rgba(199,166,106,0.04)', border:'1px solid rgba(199,166,106,0.1)', borderRadius:'12px' }}>
                          ...
                          <span style={{ fontSize: '14px', color:'rgba(0,0,0,0.48)' }}>{a}</span>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {tab==='schedule' && (
                <div style={{ animation:'fadeUp 0.4s ease both' }}>
                  <div style={{ background:'#FFFFFF', border:'1px solid rgba(0,0,0,0.07)', borderRadius:'20px', padding:'26px', marginBottom:'16px' }}>
                    ...
                    <p style={{ fontSize: '14px', color:'rgba(0,0,0,0.42)', margin:'0 0 22px' }}>روی ساعت مورد نظر کلیک کنید تا رزرو کنید.</p>

                    {/* Legend */}
                    <div style={{ display:'flex', gap:'16px', marginBottom:'20px', fontSize: '12px', color:'rgba(0,0,0,0.42)' }}>
                    ...
                    <div style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
                      {SCHEDULE.map(day => (
                        <div key={day.day}>
                          <div style={{ fontSize: '13px', fontWeight:700, color:'rgba(0,0,0,0.45)', marginBottom:'8px', letterSpacing:'0.03em' }}>{day.day}</div>
                    ...
                    {selSlot && (
                      <div style={{ marginTop:'20px', padding:'14px 18px', background:'rgba(199,166,106,0.08)', border:'1px solid rgba(199,166,106,0.22)', borderRadius:'14px', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'10px', animation:'fadeUp 0.3s ease both' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:'8px', color:'#C7A66A', fontSize: '15px', fontWeight:700 }}>
                          <Calendar size={14}/>
                          {selSlot.day} — {selSlot.time}
                        </div>
                        <Link href={user?`/booking/${coach.club.id}`:'/login'} style={{ padding:'10px 22px', background:'linear-gradient(135deg,#C7A66A,#A07840)', borderRadius:'11px', color:'#fff', fontSize: '14px', fontWeight:700, textDecoration:'none', boxShadow:'0 6px 18px rgba(199,166,106,0.3)' }}>
                          رزرو این جلسه →
                        </Link>
                      </div>
                    )}
                ...
                <div style={{ animation:'fadeUp 0.4s ease both' }}>
                    ...
                    {PACKAGES.map(pkg => (
                      <div key={pkg.id} className="pkg-card" onClick={()=>setSelPkg(pkg.id===selPkg?null:pkg.id)}
                        ...
                        <div style={{ marginTop: pkg.popular?'16px':'0', marginBottom:'16px' }}>
                          <div style={{ fontSize: '12px', color:pkg.color, fontWeight:700, marginBottom:'6px' }}>{pkg.title}</div>
                          ...
                          <div style={{ fontSize: '11px', color:'rgba(0,0,0,0.40)' }}>{toFa(pkg.sessions)} جلسه · {toFa((pkg.perSession/1000).toLocaleString())} هزار/جلسه</div>
                        </div>
                        ...
                        <div style={{ display:'flex', flexDirection:'column', gap:'8px', marginBottom:'18px' }}>
                          {pkg.features.map((f,i) => (
                            <div key={i} style={{ display:'flex', alignItems:'flex-start', gap:'7px', fontSize: '13px', color:'rgba(0,0,0,0.48)' }}>
                              <Check size={12} style={{ color:pkg.color, flexShrink:0, marginTop:'1px' }}/>{f}
                            </div>
                          ))}
                        </div>

                        <button style={{ width:'100%', padding:'11px', borderRadius:'12px', background: selPkg===pkg.id?`linear-gradient(135deg,${pkg.color},${pkg.color}cc)`:`${pkg.color}10`, color: selPkg===pkg.id?'#fff':pkg.color, fontSize: '13px', fontWeight:700, cursor:'pointer', fontFamily:'inherit', transition:'all 0.3s', boxShadow: selPkg===pkg.id?`0 6px 18px ${pkg.color}35`:'none', border:`1px solid ${selPkg===pkg.id?'transparent':`${pkg.color}22`}` }}>
                  ...
                  {selPkg && (
                    <div style={{ marginTop:'20px', padding:'18px 22px', background:'rgba(199,166,106,0.06)', border:'1px solid rgba(199,166,106,0.2)', borderRadius:'16px', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'12px', animation:'fadeUp 0.3s ease both' }}>
                      <div>
                        <div style={{ fontSize: '15px', fontWeight:800, color:'#111111', marginBottom:'3px' }}>
                          پکیج {PACKAGES.find(p=>p.id===selPkg)?.title} انتخاب شد
                        </div>
                        <div style={{ fontSize: '13px', color:'rgba(0,0,0,0.42)' }}>برای ادامه روی دکمه رزرو کلیک کنید</div>
                      </div>
                      <Link href={user?`/booking/${coach.club.id}`:'/login'} style={{ display:'flex', alignItems:'center', gap:'8px', padding:'12px 24px', background:'linear-gradient(135deg,#C7A66A,#A07840)', borderRadius:'12px', color:'#fff', fontSize: '15px', fontWeight:700, textDecoration:'none', boxShadow:'0 6px 18px rgba(199,166,106,0.3)' }}>
                ...
                <div style={{ animation:'fadeUp 0.4s ease both', display:'flex', flexDirection:'column', gap:'12px' }}>
                  ...
                  <div style={{ background:'#FFFFFF', border:'1px solid rgba(0,0,0,0.07)', borderRadius:'20px', padding:'26px', display:'flex', gap:'28px', alignItems:'center', flexWrap:'wrap', marginBottom:'4px' }}>
                    <div style={{ textAlign:'center', flexShrink:0 }}>
                      ...
                      <div style={{ fontSize: '12px', color:'rgba(0,0,0,0.40)' }}>{toFa(coach.reviewCount)} نظر</div>
                    </div>
                    <div style={{ flex:1, minWidth:'180px', display:'flex', flexDirection:'column', gap:'6px' }}>
                      {[{s:5,p:91},{s:4,p:6},{s:3,p:2},{s:2,p:1},{s:1,p:0}].map(r => (
                        <div key={r.s} style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                          <span style={{ fontSize: '11px', color:'rgba(0,0,0,0.42)', width:'12px' }}>{toFa(r.s)}</span>
                          ...
                          <span style={{ fontSize: '11px', color:'rgba(0,0,0,0.30)', width:'28px', textAlign:'left' }}>{toFa(r.p)}٪</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {REVIEWS.map((r,i) => (
                    <div key={i} style={{ padding:'20px', background:'#FFFFFF', border:'1px solid rgba(0,0,0,0.05)', borderRadius:'18px' }}>
                      ...
                      <p style={{ fontSize: '14px', color:'rgba(0,0,0,0.50)', margin:0, lineHeight:1.75 }}>{r.text}</p>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div style={{ position:'sticky', top:'80px', display:'flex', flexDirection:'column', gap:'16px' }}>
              ...
              <div style={{ background:'#FFFFFF', border:`1px solid ${coach.avatarColor}25`, borderRadius:'22px', padding:'22px', position:'relative', overflow:'hidden' }}>
                ...
                <div style={{ fontSize: '11px', color:`${coach.avatarColor}80`, fontWeight:700, marginBottom:'14px', textAlign:'center' }}>رزرو جلسه</div>
                ...
                <div style={{ fontSize: '12px', color:'rgba(0,0,0,0.40)', textAlign:'center', marginBottom:'18px' }}>تومان / ساعت</div>

                <div style={{ display:'flex', gap:'8px', marginBottom:'12px', flexWrap:'wrap' }}>
                  {['private','group','online'].filter(t=>coach.lessonTypes.includes(t)).map(t => (
                    <div key={t} style={{ flex:1, padding:'8px 10px', background:'rgba(0,0,0,0.03)', border:'1px solid rgba(0,0,0,0.07)', borderRadius:'10px', textAlign:'center', fontSize: '12px', color:'rgba(0,0,0,0.45)', fontWeight:600 }}>
                      {t==='private'?'خصوصی':t==='group'?'گروهی':'آنلاین'}
                    </div>
                  ))}
                </div>

                <button onClick={()=>setTab('schedule')} style={{ width:'100%', padding:'14px', borderRadius:'13px', border:'none', background:`linear-gradient(135deg,${coach.avatarColor},${coach.avatarColor}cc)`, color:'#fff', fontSize: '15px', fontWeight:800, cursor:'pointer', fontFamily:'inherit', boxShadow:`0 8px 24px ${coach.avatarColor}30`, transition:'all 0.3s', display:'flex', alignItems:'center', justifyContent:'center', gap:'8px' }}>
              ...
              <div style={{ background:'#FFFFFF', border:'1px solid rgba(0,0,0,0.07)', borderRadius:'20px', padding:'20px' }}>
                <div style={{ fontSize: '12px', color:'rgba(0,0,0,0.35)', fontWeight:700, marginBottom:'14px' }}>تخصص‌ها</div>
                <div style={{ display:'flex', flexWrap:'wrap', gap:'7px' }}>
                  {coach.speciality.map((s: string) => (
                    <span key={s} style={{ fontSize: '12px', color:coach.avatarColor, background:`${coach.avatarColor}10`, border:`1px solid ${coach.avatarColor}22`, borderRadius:'20px', padding:'5px 12px', fontWeight:600 }}>{s}</span>
                  ))}
                </div>
              </div>
              ...
              {/* Club */}
              <div style={{ background:'#FFFFFF', border:'1px solid rgba(0,0,0,0.07)', borderRadius:'20px', padding:'20px' }}>
                <div style={{ fontSize: '12px', color:'rgba(199,166,106,0.6)', letterSpacing:'0.18em', fontWeight:700, marginBottom:'12px' }}>CLUB</div>
                <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
                  ...
                  <div>
                    <div style={{ fontSize: '14px', fontWeight:700, color:'#111111', marginBottom:'3px' }}>{coach.club.name}</div>
                    <Link href={`/clubs/${coach.club.id}`} style={{ fontSize: '12px', color:'#C7A66A', textDecoration:'none', fontWeight:600 }}>مشاهده باشگاه ←</Link>
                  </div>
                </div>
              </div>
              ...
              {/* Quick stats */}
              <div style={{ background:'#FFFFFF', border:'1px solid rgba(0,0,0,0.07)', borderRadius:'20px', padding:'20px' }}>
                <div style={{ fontSize: '12px', color:'rgba(167,139,250,0.6)', fontWeight:700, marginBottom:'14px' }}>آمار مربی</div>
                {[
                  ...
                  <div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'9px 0', borderBottom: i<3?'1px solid rgba(0,0,0,0.04)':'none' }}>
                    ...
                    <span style={{ fontSize: '14px', fontWeight:700, color:s.c }}>{s.v}</span>
                  </div>
                ))}
              </div>
            </div>
