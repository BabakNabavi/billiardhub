...
const PACKAGES: Package[] = [
  ...
  { id:'pro',     title:'حرفه‌ای',   sessions:8,  price:5600000,  perSession:700000, color:'#10b981', popular:true, features:['۸ جلسه ۹۰ دقیقه‌ای','ارزیابی جامع','برنامه شخصی‌سازی‌شده','ویدیوی تحلیل ضربات','پشتیبانی ۲۴ ساعته'] },
  ...
  return (
    <>
      <style>{`
        ...
        .slot-avail { background:rgba(255,255,255,0.03); border-color:rgba(255,255,255,0.09); color:rgba(240,250,245,0.65); }
        .slot-avail:hover { background:rgba(16,185,129,0.08); border-color:rgba(16,185,129,0.3); color:#10b981; }
      ...
      <div style={{ minHeight:'100vh', background:'linear-gradient(180deg,#020806,#060d0a)', paddingBottom:'80px' }}>
        ...
        <div style={{ position:'relative', height:'clamp(460px,60vh,640px)', overflow:'hidden' }}>
          ...
          <div style={{ position:'absolute', top:'24px', left:0, right:0, padding:'0 clamp(16px,4vw,48px)', display:'flex', justifyContent:'space-between', zIndex:10 }}>
            <Link href="/coaches" style={{ display:'flex', alignItems:'center', gap:'6px', color:'rgba(255,255,255,0.5)', fontSize:'12px', textDecoration:'none', background:'rgba(0,0,0,0.4)', backdropFilter:'blur(16px)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'10px', padding:'7px 14px' }}>
          ...
          <div style={{ position:'absolute', bottom:0, left:0, right:0, padding:'clamp(24px,4vw,52px)', zIndex:10, opacity:heroOpacity }}>
            <div style={{ display:'flex', alignItems:'flex-end', gap:'24px', flexWrap:'wrap' }}>
              ...
              <div style={{ position:'relative', flexShrink:0 }}>
                <div style={{ width:'clamp(72px,12vw,108px)', height:'clamp(72px,12vw,108px)', borderRadius:'22px', background:`linear-gradient(135deg,${coach.avatarColor},${coach.avatarColor}80)`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'clamp(28px,6vw,44px)', fontWeight:900, color:'#fff', border:'3px solid rgba(16,185,129,0.4)', boxShadow:`0 0 40px ${coach.avatarColor}30,0 20px 60px rgba(0,0,0,0.5)` }}>
                  {coach.avatar}
                </div>
                {coach.isOnline && <div style={{ position:'absolute', bottom:'4px', right:'4px', width:'14px', height:'14px', borderRadius:'50%', background:'#10b981', border:'2px solid rgba(2,8,6,0.98)', boxShadow:'0 0 10px #10b981' }}/>}
              </div>
              ...
              <div style={{ flex:1, minWidth:'200px' }}>
                <div style={{ display:'flex', gap:'8px', marginBottom:'10px', flexWrap:'wrap' }}>
                  ...
                  <div style={{ display:'inline-flex', alignItems:'center', gap:'6px', background:`${coach.avatarColor}12`, border:`1px solid ${coach.avatarColor}28`, borderRadius:'20px', padding:'4px 13px', backdropFilter:'blur(16px)' }}>
                    ...
                    <span style={{ fontSize:'9px', color:coach.avatarColor, fontWeight:700, letterSpacing:'0.12em' }}>PRO COACH</span>
                  </div>
                </div>

                <h1 style={{ fontSize:'clamp(24px,5vw,48px)', fontWeight:900, color:'#fff', margin:'0 0 8px', letterSpacing:'-0.035em', lineHeight:1.0, textShadow:`0 0 60px ${coach.avatarColor}20` }}>
                  {coach.name}
                </h1>
                <div style={{ fontSize:'14px', color:'rgba(255,255,255,0.5)', marginBottom:'10px' }}>{coach.title}</div>
                ...
                <div style={{ display:'flex', gap:'10px', flexWrap:'wrap' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:'5px', background:'rgba(255,255,255,0.08)', backdropFilter:'blur(12px)', borderRadius:'20px', padding:'5px 13px', fontSize:'12px', color:'rgba(255,255,255,0.8)' }}>
                    <MapPin size={10} style={{ color:'#10b981' }}/>{coach.city}
                  </div>
                  <div style={{ display:'flex', gap:'2px', alignItems:'center', background:'rgba(255,255,255,0.08)', backdropFilter:'blur(12px)', borderRadius:'20px', padding:'5px 13px' }}>
                    {[1,2,3,4,5].map(s=><Star key={s} size={11} style={{ color:'#f59e0b', fill: s<=Math.floor(coach.rating)?'#f59e0b':'transparent' }}/>)}
                    <span style={{ fontSize:'12px', color:'rgba(255,255,255,0.7)', marginRight:'5px' }}>{coach.rating}</span>
                    <span style={{ fontSize:'10px', color:'rgba(255,255,255,0.35)', marginRight:'3px' }}>({toFa(coach.reviewCount)})</span>
                  </div>
                </div>
              </div>
              ...
              <div style={{ display:'flex', gap:'10px', flexWrap:'wrap', flexShrink:0 }}>
                ...
                <button style={{ display:'flex', alignItems:'center', gap:'7px', padding:'11px 18px', borderRadius:'12px', background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)', color:'rgba(255,255,255,0.7)', fontSize:'13px', fontWeight:600, cursor:'pointer', fontFamily:'inherit', transition:'all 0.3s' }}>
          ...
          <div style={{ maxWidth:'1280px', margin:'0 auto', display:'grid', gridTemplateColumns:'repeat(4,1fr)' }}>
            {[
              ...
              <div key={i} style={{ padding:'18px 16px', textAlign:'center', borderLeft: i>0?'1px solid rgba(255,255,255,0.05)':'none' }}>
                <div style={{ fontSize:'clamp(18px,2.5vw,24px)', fontWeight:900, color:'#f0faf5', letterSpacing:'-0.02em', textShadow:`0 0 16px ${s.c}25` }}>{s.v}</div>
        ...
        <div style={{ maxWidth:'1280px', margin:'0 auto', padding:'clamp(24px,4vw,40px) clamp(16px,3vw,32px)' }}>
          ...
          <div className="profile-g" style={{ display:'grid', gridTemplateColumns:'1fr 340px', gap:'28px', alignItems:'start' }}>
            ...
            <div>
              ...
              {tab==='overview' && (
                <div style={{ animation:'fadeUp 0.4s ease both', display:'flex', flexDirection:'column', gap:'20px' }}>
                  ...
                  <div style={{ background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'20px', padding:'26px' }}>
                    ...
                    <p style={{ fontSize:'14px', color:'rgba(240,250,245,0.55)', lineHeight:1.9, margin:'0 0 20px' }}>{coach.bio}</p>
                    ...
                    <div style={{ display:'flex', flexDirection:'column', gap:'10px' }}>
                      {coach.certifications.map((cert: string, i: number) => (
                        <div key={i} style={{ display:'flex', alignItems:'center', gap:'12px', padding:'13px 16px', background:'rgba(245,158,11,0.04)', border:'1px solid rgba(245,158,11,0.12)', borderRadius:'14px' }}>
                          ...
                          <span style={{ fontSize:'13px', color:'rgba(240,250,245,0.7)', fontWeight:600 }}>{cert}</span>
                          <Check size={13} style={{ color:'#10b981', marginRight:'auto', flexShrink:0 }}/>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div style={{ background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'20px', padding:'26px' }}>
                    ...
                    <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
                      {coach.achievements.map((a: string, i: number) => (
                        <div key={i} style={{ display:'flex', alignItems:'center', gap:'10px', padding:'10px 14px', background:'rgba(16,185,129,0.04)', border:'1px solid rgba(16,185,129,0.1)', borderRadius:'12px' }}>
                          <Trophy size={13} style={{ color:'#10b981', flexShrink:0 }}/>
                          <span style={{ fontSize:'13px', color:'rgba(240,250,245,0.65)' }}>{a}</span>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {tab==='schedule' && (
                <div style={{ animation:'fadeUp 0.4s ease both' }}>
                  <div style={{ background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'20px', padding:'26px', marginBottom:'16px' }}>
                    <h3 style={{ fontSize:'15px', fontWeight:800, color:'#f0faf5', margin:'0 0 8px' }}>انتخاب زمان جلسه</h3>
                    <p style={{ fontSize:'13px', color:'rgba(240,250,245,0.4)', margin:'0 0 22px' }}>روی ساعت مورد نظر کلیک کنید تا رزرو کنید.</p>

                    {/* Legend */}
                    <div style={{ display:'flex', gap:'16px', marginBottom:'20px', fontSize:'11px', color:'rgba(240,250,245,0.4)' }}>
                      {[{bg:'rgba(255,255,255,0.06)',bc:'rgba(255,255,255,0.1)',l:'آزاد'},{bg:'rgba(16,185,129,0.14)',bc:'rgba(16,185,129,0.4)',l:'انتخاب'},{bg:'rgba(239,68,68,0.04)',bc:'rgba(239,68,68,0.15)',l:'مشغول'}].map((x,i) => (
                    ...
                    <div style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
                      {SCHEDULE.map(day => (
                        <div key={day.day}>
                          <div style={{ fontSize:'12px', fontWeight:700, color:'rgba(240,250,245,0.5)', marginBottom:'8px', letterSpacing:'0.03em' }}>{day.day}</div>
                            ...
                            {day.slots.map(slot => {
                              ...
                              return (
                                <button key={slot.time}
                                  ...
                                  style={{ borderColor:!slot.available?'rgba(239,68,68,0.12)':isSel?'rgba(16,185,129,0.5)':'rgba(255,255,255,0.09)', background:!slot.available?'rgba(239,68,68,0.04)':isSel?'rgba(16,185,129,0.12)':'rgba(255,255,255,0.03)', color:!slot.available?'rgba(239,68,68,0.3)':isSel?'#10b981':'rgba(240,250,245,0.65)', boxShadow:isSel?'0 0 14px rgba(16,185,129,0.25)':'none' }}>
                                  {slot.time}
                                  {!slot.available && <div style={{ fontSize:'9px', opacity:0.6, marginTop:'2px' }}>مشغول</div>}
                                </button>
                              );
                            })}
                          </div>
                    ...
                    {selSlot && (
                      <div style={{ marginTop:'20px', padding:'14px 18px', background:'rgba(16,185,129,0.08)', border:'1px solid rgba(16,185,129,0.22)', borderRadius:'14px', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'10px', animation:'fadeUp 0.3s ease both' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:'8px', color:'#10b981', fontSize:'14px', fontWeight:700 }}>
                          <Calendar size={14}/>
                          {selSlot.day} — {selSlot.time}
                        </div>
                        <Link href={user?`/booking/${coach.club.id}`:'/login'} style={{ padding:'10px 22px', background:'linear-gradient(135deg,#10b981,#059669)', borderRadius:'11px', color:'#fff', fontSize:'13px', fontWeight:700, textDecoration:'none', boxShadow:'0 6px 18px rgba(16,185,129,0.3)' }}>
                          رزرو این جلسه →
                        </Link>
                      </div>
                    )}
              ...
              {tab==='packages' && (
                <div style={{ animation:'fadeUp 0.4s ease both' }}>
                  <div className="pkg-g" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:'16px' }}>
                    {PACKAGES.map(pkg => (
                      <div key={pkg.id} className="pkg-card" onClick={()=>setSelPkg(pkg.id===selPkg?null:pkg.id)}
                        style={{ background: selPkg===pkg.id?`${pkg.color}0e`:'rgba(255,255,255,0.025)', border:`1px solid ${selPkg===pkg.id?`${pkg.color}40`:'rgba(255,255,255,0.07)'}`, boxShadow: selPkg===pkg.id?`0 12px 36px rgba(0,0,0,0.4),0 0 0 1px ${pkg.color}20`:'none' }}>
                        ...
                        <div style={{ marginTop: pkg.popular?'16px':'0', marginBottom:'16px' }}>
                          ...
                          <div style={{ fontSize:'28px', fontWeight:900, color:'#f0faf5', letterSpacing:'-0.03em', marginBottom:'2px', textShadow:`0 0 20px ${pkg.color}30` }}>
                            {toFa((pkg.price/1000000).toFixed(1))}م
                          </div>
                          <div style={{ fontSize:'10px', color:'rgba(240,250,245,0.35)' }}>{toFa(pkg.sessions)} جلسه · {toFa((pkg.perSession/1000).toLocaleString())} هزار/جلسه</div>
                        </div>
                        ...
                        <div style={{ display:'flex', flexDirection:'column', gap:'8px', marginBottom:'18px' }}>
                          {pkg.features.map((f,i) => (
                            <div key={i} style={{ display:'flex', alignItems:'flex-start', gap:'7px', fontSize:'12px', color:'rgba(240,250,245,0.6)' }}>
                  ...
                  {selPkg && (
                    <div style={{ marginTop:'20px', padding:'18px 22px', background:'rgba(16,185,129,0.06)', border:'1px solid rgba(16,185,129,0.2)', borderRadius:'16px', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'12px', animation:'fadeUp 0.3s ease both' }}>
                      <div>
                        <div style={{ fontSize:'14px', fontWeight:800, color:'#f0faf5', marginBottom:'3px' }}>
                          پکیج {PACKAGES.find(p=>p.id===selPkg)?.title} انتخاب شد
                        </div>
                        <div style={{ fontSize:'12px', color:'rgba(240,250,245,0.4)' }}>برای ادامه روی دکمه رزرو کلیک کنید</div>
                      </div>
                      <Link href={user?`/booking/${coach.club.id}`:'/login'} style={{ display:'flex', alignItems:'center', gap:'8px', padding:'12px 24px', background:'linear-gradient(135deg,#10b981,#059669)', borderRadius:'12px', color:'#fff', fontSize:'14px', fontWeight:700, textDecoration:'none', boxShadow:'0 6px 18px rgba(16,185,129,0.3)' }}>
              ...
              {tab==='reviews' && (
                <div style={{ animation:'fadeUp 0.4s ease both', display:'flex', flexDirection:'column', gap:'12px' }}>
                  {/* Summary */}
                  <div style={{ background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'20px', padding:'26px', display:'flex', gap:'28px', alignItems:'center', flexWrap:'wrap', marginBottom:'4px' }}>
                    <div style={{ textAlign:'center', flexShrink:0 }}>
                      <div style={{ fontSize:'52px', fontWeight:900, color:'#f0faf5', lineHeight:1, letterSpacing:'-0.04em' }}>{coach.rating}</div>
                      ...
                      <div style={{ fontSize:'11px', color:'rgba(240,250,245,0.35)' }}>{toFa(coach.reviewCount)} نظر</div>
                    </div>
                    <div style={{ flex:1, minWidth:'180px', display:'flex', flexDirection:'column', gap:'6px' }}>
                      {[{s:5,p:91},{s:4,p:6},{s:3,p:2},{s:2,p:1},{s:1,p:0}].map(r => (
                        <div key={r.s} style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                          <span style={{ fontSize:'10px', color:'rgba(240,250,245,0.4)', width:'12px' }}>{toFa(r.s)}</span>
                          ...
                          <span style={{ fontSize:'10px', color:'rgba(240,250,245,0.25)', width:'28px', textAlign:'left' }}>{toFa(r.p)}٪</span>
                        </div>
                      ))}
                    </div>
                  </div>
                    ...
                    <div key={i} style={{ padding:'20px', background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'18px' }}>
                      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:'12px' }}>
                        <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
                          <div style={{ width:'40px', height:'40px', borderRadius:'12px', background:`linear-gradient(135deg,${coach.avatarColor},${coach.avatarColor}80)`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:'16px', fontWeight:900, color:'#fff', flexShrink:0 }}>
                            {r.name[0]}
                          </div>
                          <div>
                            <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'3px' }}>
                              <span style={{ fontSize:'14px', fontWeight:700, color:'#f0faf5' }}>{r.name}</span>
                              <span style={{ fontSize:'9px', color:coach.avatarColor, background:`${coach.avatarColor}12`, border:`1px solid ${coach.avatarColor}22`, borderRadius:'20px', padding:'2px 8px', fontWeight:700 }}>{r.level}</span>
                            </div>
            ...
            <div style={{ position:'sticky', top:'80px', display:'flex', flexDirection:'column', gap:'16px' }}>
              ...
              {/* Book CTA */}
              <div style={{ background:'rgba(255,255,255,0.025)', border:`1px solid ${coach.avatarColor}25`, borderRadius:'22px', padding:'22px', position:'relative', overflow:'hidden' }}>
                ...
                <div style={{ fontSize:'28px', fontWeight:900, color:'#10b981', textAlign:'center', letterSpacing:'-0.03em', marginBottom:'4px', textShadow:'0 0 20px rgba(16,185,129,0.4)' }}>
                  {toFa(coach.hourlyRate.toLocaleString())}
                </div>
                <div style={{ fontSize:'11px', color:'rgba(240,250,245,0.35)', textAlign:'center', marginBottom:'18px' }}>تومان / ساعت</div>

                <div style={{ display:'flex', gap:'8px', marginBottom:'12px', flexWrap:'wrap' }}>
                  {['private','group','online'].filter(t=>coach.lessonTypes.includes(t)).map(t => (
                    <div key={t} style={{ flex:1, padding:'8px 10px', background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'10px', textAlign:'center', fontSize:'11px', color:'rgba(240,250,245,0.5)', fontWeight:600 }}>
                      {t==='private'?'خصوصی':t==='group'?'گروهی':'آنلاین'}
                    </div>
                  ))}
                </div>

                <button onClick={()=>setTab('schedule')} style={{ width:'100%', padding:'14px', borderRadius:'13px', border:'none', background:`linear-gradient(135deg,${coach.avatarColor},${coach.avatarColor}cc)`, color:'#fff', fontSize:'14px', fontWeight:800, cursor:'pointer', fontFamily:'inherit', boxShadow:`0 8px 24px ${coach.avatarColor}30`, transition:'all 0.3s', display:'flex', alignItems:'center', justifyContent:'center', gap:'8px' }}>
              ...
              <div style={{ background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'20px', padding:'20px' }}>
                <div style={{ fontSize:'11px', color:'rgba(240,250,245,0.3)', letterSpacing:'0.15em', fontWeight:700, marginBottom:'14px', textTransform:'uppercase' }}>تخصص‌ها</div>
              ...
              {/* Club */}
              <div style={{ background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'20px', padding:'20px' }}>
                <div style={{ fontSize:'11px', color:'rgba(16,185,129,0.6)', letterSpacing:'0.18em', fontWeight:700, marginBottom:'12px' }}>CLUB</div>
                <div style={{ display:'flex', alignItems:'center', gap:'12px' }}>
                  <div style={{ width:'42px', height:'42px', borderRadius:'12px', background:'rgba(16,185,129,0.1)', border:'1px solid rgba(16,185,129,0.2)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'20px', flexShrink:0 }}>🏆</div>
                  <div>
                    <div style={{ fontSize:'13px', fontWeight:700, color:'#f0faf5', marginBottom:'3px' }}>{coach.club.name}</div>
                    <Link href={`/clubs/${coach.club.id}`} style={{ fontSize:'11px', color:'#10b981', textDecoration:'none', fontWeight:600 }}>مشاهده باشگاه ←</Link>
                  </div>
                </div>
              </div>
              ...
              <div style={{ background:'rgba(255,255,255,0.025)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'20px', padding:'20px' }}>
                ...
                {[
                  ...
                  { l:'زبان تدریس',     v:toFa(coach.languages.length), c:'#10b981' },
                ].map((s,i) => (
