      ...
      <div style={{ background: 'linear-gradient(180deg,#111111 0%,#1a1a1a 100%)', padding: 'clamp(40px,5vw,72px) clamp(16px,4vw,40px) clamp(32px,4vw,56px)', direction: 'rtl', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', position: 'relative' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(199,166,106,0.10)', border: '1px solid rgba(199,166,106,0.20)', borderRadius: 20, padding: '5px 14px', marginBottom: 20 }}>
            ...
            <span style={{ fontSize: 14, fontWeight: 600, color: GOLD, letterSpacing: '0.06em' }}>CERTIFIED COACHES</span>
          </div>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ background: '#FFFFFF', borderRadius: 16, border: '1px solid rgba(0,0,0,0.07)', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', padding: '16px 20px', marginBottom: 24, display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#F7F7F5', border: `1.5px solid ${searchFocus ? `rgba(199,166,106,0.45)` : 'rgba(0,0,0,0.10)'}`, borderRadius: 12, padding: '0 14px', height: 44, flex: '1 1 220px', transition: 'all 0.3s', boxShadow: searchFocus ? '0 0 0 3px rgba(199,166,106,0.10)' : 'none' }}>
              ...
              <input type="text" value={search} onChange={e=>setSearch(e.target.value)} onFocus={()=>setSearchFocus(true)} onBlur={()=>setSearchFocus(false)}
                ...
                style={{ flex: 1, background: 'none', border: 'none', outline: 'none', color: '#111111', fontSize: 15, fontFamily: 'inherit' }} />
          ...
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '80px 20px' }}>
              ...
              <p style={{ fontSize: 15, color: 'rgba(0,0,0,0.38)', margin: 0 }}>جستجو یا فیلترها را تغییر دهید</p>
            </div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: 16 }}>
                  ...
                  <div className="coach-card" style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.06)', borderRadius: 20, padding: 20, cursor: 'pointer', animation: `fadeUp 0.5s ${i*0.05}s ease both`, position: 'relative', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 14 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        {coach.city && <div style={{ display: 'flex', alignItems: 'center', gap: 4, color: 'rgba(0,0,0,0.40)', fontSize: 14 }}><MapPin size={11}/><span>{coach.city}</span></div>}
                      </div>
                    ...
                    {coach.coachProfile && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 10 }}>
                        {coach.coachProfile.specialty && <span style={{ fontSize: 13, fontWeight: 600, color: GOLD_DARK, background: 'rgba(199,166,106,0.10)', border: '1px solid rgba(199,166,106,0.20)', borderRadius: 20, padding: '3px 10px' }}>{specialtyLabels[coach.coachProfile.specialty] ?? coach.coachProfile.specialty}</span>}
                        {coach.coachProfile.experience && <span style={{ fontSize: 13, color: 'rgba(0,0,0,0.45)', background: 'rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 20, padding: '3px 10px' }}>{coach.coachProfile.experience} سال سابقه</span>}
                        {coach.coachProfile.sessionPrice && <span style={{ fontSize: 13, color: '#16a34a', background: 'rgba(22,163,74,0.07)', border: '1px solid rgba(22,163,74,0.15)', borderRadius: 20, padding: '3px 10px' }}>{coach.coachProfile.sessionPrice.toLocaleString('fa-IR')} ت</span>}
                      </div>
                    )}
                    {coach.coachProfile?.certifications && <div style={{ fontSize: 13, color: 'rgba(0,0,0,0.38)', marginBottom: 8 }}>🏅 {coach.coachProfile.certifications}</div>}
                    {coach.bio && <div style={{ color: 'rgba(0,0,0,0.48)', fontSize: 14, lineHeight: 1.6, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{coach.bio}</div>}
                  </div>
                </Link>
              ))}
            </div>
