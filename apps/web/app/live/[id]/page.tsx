  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#F7F7F5', color: '#111111', fontFamily: 'Vazirmatn, sans-serif', direction: 'rtl' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', flexShrink: 0, background: '#050c08', borderBottom: '1px solid rgba(199,166,106,0.15)' }}>
        <button onClick={() => router.push('/live')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#C7A66A', fontSize: 15, fontFamily: 'Vazirmatn, sans-serif', padding: 0, textDecoration: 'none' }}>
          ← زنده
        </button>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginRight: 'auto' }}>
          <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#ef4444', display: 'inline-block' }} />
          <span style={{ fontSize: 12, fontWeight: 700, color: '#ef4444' }}>LIVE</span>
          <span style={{ fontSize: 12, color: '#6b7280' }}>👁 {viewers.toLocaleString('fa')}</span>
        </div>
        <button onClick={() => setChatOpen(o => !o)} style={{ padding: '4px 12px', borderRadius: 999, fontSize: 12, fontFamily: 'Vazirmatn, sans-serif', cursor: 'pointer', background: chatOpen ? 'rgba(199,166,106,0.15)' : 'rgba(107,114,128,0.15)', color: chatOpen ? '#C7A66A' : '#6b7280', border: `1px solid ${chatOpen ? 'rgba(199,166,106,0.3)' : 'rgba(107,114,128,0.2)'}` }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ position: 'relative', width: '100%', aspectRatio: '16/9', background: '#000', maxHeight: '56vw' }}>
            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'radial-gradient(ellipse at center,#0d3320 0%,#061a0f 60%,#010604 100%)' }}>
              <div style={{ position: 'relative', width: '70%', height: '55%', borderRadius: 16, background: 'linear-gradient(135deg,#0a5c2e,#0d7a3c)', border: '4px solid #5c3a1e', boxShadow: '0 0 40px rgba(199,166,106,0.2)' }}>
                ...
                <div style={{ position: 'absolute', bottom: 8, right: 12, fontSize: 11, fontWeight: 700, opacity: 0.4, color: '#C7A66A', letterSpacing: 2 }}>BILLIARDHUB</div>
                {[
              ...
              <div style={{ position: 'absolute', top: 10, left: 10, display: 'flex', alignItems: 'center', gap: 5, padding: '4px 8px', borderRadius: 8, background: 'rgba(239,68,68,0.9)' }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 900, color: '#fff' }}>LIVE</span>
              </div>
              <div style={{ position: 'absolute', top: 10, right: 10, padding: '4px 8px', borderRadius: 8, background: 'rgba(0,0,0,0.6)', fontSize: 12, color: '#111111', fontFamily: 'monospace' }}>
                ▶ {stream.startedAt}
              </div>
            </div>
          ...
          <div style={{ margin: '12px 16px 0', borderRadius: 16, overflow: 'hidden', background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.07)' }}>
            <div style={{ textAlign: 'center', padding: '6px 0', fontSize: 12, fontWeight: 700, color: '#ef4444', background: 'rgba(239,68,68,0.08)', borderBottom: '1px solid rgba(199,166,106,0.08)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 80px 1fr', alignItems: 'center', padding: '14px 16px' }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 4 }}>{stream.player1}</div>
                <div style={{ fontSize: 44, fontWeight: 900, color: '#C7A66A', lineHeight: 1 }}>{stream.score1}</div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 12, color: '#4b5563', marginBottom: 6 }}>{stream.discipline}</div>
              ...
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 4 }}>{stream.player2}</div>
          ...
          <div style={{ padding: '12px 16px 16px' }}>
            ...
            <div style={{ fontSize: 13, color: '#6b7280' }}>{stream.tournament}</div>
          </div>
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', background: '#050c08', borderTop: '1px solid rgba(199,166,106,0.1)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', flexShrink: 0, borderBottom: '1px solid rgba(199,166,106,0.1)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontSize: 14, fontWeight: 700, color: '#111111' }}>💬 نظرات زنده</span>
                <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 999, background: 'rgba(199,166,106,0.15)', color: '#C7A66A' }}>{comments.length}</span>
              </div>
              <button onClick={() => setChatOpen(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#4b5563', fontSize: 14, fontFamily: 'Vazirmatn, sans-serif' }}>
                ✕ بستن
              </button>
            </div>
            ...
            <div ref={chatRef} style={{ overflowY: 'auto', padding: '10px 12px', maxHeight: 280, display: 'flex', flexDirection: 'column', gap: 10 }}>
              {comments.map(c => (
                <div key={c.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                  <div style={{ width: 28, height: 28, borderRadius: 8, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 900, background: 'linear-gradient(135deg,#C7A66A,#A07840)', color: '#010604' }}>
                    {c.user[0]}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 2 }}>
                      <span style={{ fontSize: 12, fontWeight: 700, color: '#C7A66A' }}>{c.user}</span>
                      <span style={{ fontSize: 11, color: '#4b5563' }}>{c.time}</span>
                    </div>
                    <div style={{ fontSize: 14, lineHeight: 1.4, color: '#d1d5db', wordBreak: 'break-word' }}>{c.text}</div>
                    <button onClick={() => toggleLike(c.id)} style={{ marginTop: 4, display: 'flex', alignItems: 'center', gap: 4, background: 'none', border: 'none', cursor: 'pointer', color: liked[c.id] ? '#ef4444' : '#4b5563', fontSize: 12, fontFamily: 'Vazirmatn, sans-serif', padding: 0 }}>
            ...
            <div style={{ padding: '10px 12px', flexShrink: 0, borderTop: '1px solid rgba(199,166,106,0.1)', display: 'flex', gap: 8 }}>
              <input value={newComment} onChange={e => setNewComment(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendComment()} placeholder="نظر بنویس..." style={{ flex: 1, minWidth: 0, padding: '8px 12px', borderRadius: 12, fontSize: 14, outline: 'none', background: 'rgba(199,166,106,0.07)', border: '1px solid rgba(199,166,106,0.15)', color: '#111111', fontFamily: 'Vazirmatn, sans-serif' }} />
              <button onClick={sendComment} style={{ padding: '8px 14px', borderRadius: 12, flexShrink: 0, fontSize: 14, fontWeight: 700, cursor: 'pointer', border: 'none', fontFamily: 'Vazirmatn, sans-serif', background: 'linear-gradient(135deg,#C7A66A,#A07840)', color: '#010604' }}>
                ارسال
              </button>
            </div>
          </div>
