        ...
        <div style={{ position: 'relative', height: '70vh', minHeight: 500, overflow: 'hidden', background: '#F7F7F5' }}>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', padding: '0 20px', textAlign: 'center' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(199,166,106,0.08)', border: '1px solid rgba(199,166,106,0.20)', borderRadius: 100, padding: '7px 20px', marginBottom: 24, animation: 'fadeUp 0.8s ease 0.2s both' }}>
              ...
              <span style={{ fontSize: 10, color: '#C7A66A', fontWeight: 700, letterSpacing: '0.22em' }}>ABOUT US — درباره ما</span>
            </div>
            <h1 style={{ fontSize: 'clamp(36px,6vw,72px)', fontWeight: 900, color: '#fff', lineHeight: 1.05, letterSpacing: '-0.04em', margin: '0 0 20px', animation: 'fadeUp 0.8s ease 0.35s both' }}>
              اکوسیستم جامع<br />
            ...
            <p style={{ fontSize: 'clamp(14px,1.8vw,18px)', color: 'rgba(255,255,255,0.4)', maxWidth: 500, lineHeight: 1.8, margin: 0, animation: 'fadeUp 0.8s ease 0.5s both' }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '80px 24px' }}>
          ...
          <div className="stat-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, marginBottom: 100 }}>
            {STATS.map((s, i) => (
              <div key={i} style={{ padding: '28px 20px', background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 20, textAlign: 'center', position: 'relative', overflow: 'hidden', animation: `fadeUp 0.5s ease ${i * 0.1}s both` }}>
                ...
                <div style={{ fontSize: 'clamp(32px,4vw,44px)', fontWeight: 900, color: '#111111', letterSpacing: '-0.03em', lineHeight: 1, textShadow: `0 0 40px ${s.color}35`, marginBottom: 8 }}>{s.value}</div>
                <div style={{ fontSize: 13, fontWeight: 700, color: s.color, marginBottom: 4 }}>{s.label}</div>
                <div style={{ fontSize: 11, color: 'rgba(0,0,0,0.38)' }}>{s.sub}</div>
              </div>
            ))}
          </div>
          ...
          <section style={{ marginBottom: 100 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, alignItems: 'center' }}>
              <div>
                <p style={{ fontSize: 10, color: '#A07840', letterSpacing: '0.22em', fontWeight: 700, marginBottom: 14, textTransform: 'uppercase' }}>OUR MISSION</p>
                <h2 style={{ fontSize: 'clamp(26px,3vw,38px)', fontWeight: 900, color: '#111111', lineHeight: 1.2, letterSpacing: '-0.03em', margin: '0 0 20px' }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <div style={{ position: 'absolute', bottom: -20, right: -20, background: '#FFFFFF', border: '1px solid rgba(199,166,106,0.25)', borderRadius: 16, padding: '16px 20px', backdropFilter: 'blur(20px)', boxShadow: '0 20px 50px rgba(0,0,0,0.5)' }}>
                  <div style={{ fontSize: 28, fontWeight: 900, color: '#C7A66A', lineHeight: 1 }}>۴.۸</div>
                  <div style={{ display: 'flex', gap: 2, margin: '4px 0' }}>
                    {[1,2,3,4,5].map(i => <i key={i} className="ti ti-star-filled" style={{ fontSize: 12, color: '#f59e0b' }} />)}
                  </div>
                  <div style={{ fontSize: 11, color: 'rgba(0,0,0,0.40)' }}>۸٬۴۰۰+ نظر</div>
                </div>
              </div>
            </div>
          </section>
          ...
          <section style={{ marginBottom: 100 }}>
            <div style={{ textAlign: 'center', marginBottom: 48 }}>
              <p style={{ fontSize: 10, color: '#A07840', letterSpacing: '0.22em', fontWeight: 700, marginBottom: 12, textTransform: 'uppercase' }}>OUR VALUES</p>
              <h2 style={{ fontSize: 'clamp(24px,3vw,34px)', fontWeight: 900, color: '#111111', margin: 0, letterSpacing: '-0.025em' }}>ارزش‌های ما</h2>
            </div>
              ...
              {VALUES.map((v, i) => (
                <div key={i} className="val-card" style={{ padding: '28px 22px', background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 20, animation: `fadeUp 0.5s ease ${i * 0.1}s both` }}>
                  <div style={{ width: 48, height: 48, borderRadius: 14, background: `${v.color}12`, border: `1px solid ${v.color}25`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 18 }}>
                    <i className={`ti ${v.icon}`} style={{ fontSize: 22, color: v.color }} />
                  </div>
                  <h3 style={{ fontSize: 15, fontWeight: 800, color: '#111111', margin: '0 0 10px' }}>{v.title}</h3>
                  <p style={{ fontSize: 13, color: 'rgba(0,0,0,0.45)', lineHeight: 1.7, margin: 0 }}>{v.desc}</p>
                </div>
              ))}
            </div>
          </section>
          ...
          <section style={{ marginBottom: 100 }}>
            <div style={{ textAlign: 'center', marginBottom: 48 }}>
              <p style={{ fontSize: 10, color: '#A07840', letterSpacing: '0.22em', fontWeight: 700, marginBottom: 12, textTransform: 'uppercase' }}>OUR JOURNEY</p>
              <h2 style={{ fontSize: 'clamp(24px,3vw,34px)', fontWeight: 900, color: '#111111', margin: 0, letterSpacing: '-0.025em' }}>مسیر رشد ما</h2>
            </div>
            <div style={{ position: 'relative' }}>
              {/* Center line */}
          ...
          <section style={{ marginBottom: 100 }}>
            <div style={{ textAlign: 'center', marginBottom: 48 }}>
              <p style={{ fontSize: 10, color: '#A07840', letterSpacing: '0.22em', fontWeight: 700, marginBottom: 12, textTransform: 'uppercase' }}>OUR TEAM</p>
              <h2 style={{ fontSize: 'clamp(24px,3vw,34px)', fontWeight: 900, color: '#111111', margin: '0 0 12px', letterSpacing: '-0.025em' }}>تیم ما</h2>
              <p style={{ fontSize: 14, color: 'rgba(0,0,0,0.45)', maxWidth: 400, margin: '0 auto', lineHeight: 1.7 }}>افرادی متعهد به رشد بیلیارد ایران</p>
            </div>
                ...
                <div key={i} className="team-card" style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 20, overflow: 'hidden', animation: `fadeUp 0.5s ease ${i * 0.15}s both` }}>
                  <div style={{ height: 200, overflow: 'hidden', position: 'relative' }}>
                    ...
                    <div style={{ position: 'absolute', bottom: 16, right: 16 }}>
                      <p style={{ fontSize: 16, fontWeight: 800, color: '#fff', margin: '0 0 4px' }}>{member.name}</p>
                      <p style={{ fontSize: 11, color: '#C7A66A', margin: 0, background: 'rgba(199,166,106,0.15)', border: '1px solid rgba(199,166,106,0.25)', borderRadius: 20, padding: '2px 10px', display: 'inline-block' }}>{member.role}</p>
                    </div>
                  </div>
                  <div style={{ padding: '16px 18px' }}>
                    <p style={{ fontSize: 13, color: 'rgba(0,0,0,0.45)', lineHeight: 1.7, margin: 0 }}>{member.desc}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <section style={{ marginBottom: 100 }}>
            <div style={{ textAlign: 'center', marginBottom: 40 }}>
              <p style={{ fontSize: 10, color: '#A07840', letterSpacing: '0.22em', fontWeight: 700, marginBottom: 12, textTransform: 'uppercase' }}>PARTNERS</p>
              <h2 style={{ fontSize: 'clamp(22px,2.5vw,30px)', fontWeight: 900, color: '#111111', margin: 0 }}>شرکای ما</h2>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14 }}>
              {[
                ...
                <div key={i} style={{ padding: '20px 16px', background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 16, textAlign: 'center' }}>
                  ...
                  <p style={{ fontSize: 13, fontWeight: 700, color: '#111111', margin: '0 0 6px', lineHeight: 1.3 }}>{p.name}</p>
                  <span style={{ fontSize: 10, color: p.color, background: `${p.color}10`, border: `1px solid ${p.color}20`, borderRadius: 20, padding: '2px 10px', fontWeight: 700 }}>{p.badge}</span>
                </div>
              ))}
            </div>
          </section>
          ...
          <section>
            <div style={{ position: 'relative', borderRadius: 28, overflow: 'hidden', padding: '60px 48px', textAlign: 'center', background: '#111111', border: 'none' }}>
              ...
              <div style={{ position: 'relative' }}>
                <h2 style={{ fontSize: 'clamp(24px,3.5vw,38px)', fontWeight: 900, color: '#FFFFFF', margin: '0 0 14px', letterSpacing: '-0.03em' }}>به جامعه ما بپیوندید</h2>
                <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.45)', margin: '0 auto 36px', maxWidth: 400, lineHeight: 1.7 }}>ثبت‌نام رایگان — دسترسی کامل به تمام امکانات پلتفرم</p>
                <div style={{ display: 'flex', justifyContent: 'center', gap: 12, flexWrap: 'wrap' }}>
                  <Link href="/register" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'linear-gradient(135deg,#C7A66A,#A07840)', color: '#fff', padding: '14px 30px', borderRadius: 14, textDecoration: 'none', fontWeight: 800, fontSize: 15, boxShadow: '0 8px 28px rgba(199,166,106,0.35)' }}>
                    <i className="ti ti-user-plus" style={{ fontSize: 17 }} /> ثبت‌نام رایگان
                  </Link>
                  <Link href="/contact" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(0,0,0,0.06)', border: '1px solid rgba(0,0,0,0.10)', color: '#FFFFFF', padding: '14px 30px', borderRadius: 14, textDecoration: 'none', fontSize: 15 }}>
                    <i className="ti ti-phone" style={{ fontSize: 17 }} /> تماس با ما
                  </Link>
                </div>
              </div>
            </div>
