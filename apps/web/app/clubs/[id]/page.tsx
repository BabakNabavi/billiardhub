...
const tableTypes = [
  { key: 'snookerTables',    label: 'اسنوکر',     color: '#C7A66A', price: '۱۸۰,۰۰۰' },
...
export default function ClubProfilePage() {
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: '#F7F7F5', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ fontSize: 13, color: 'rgba(0,0,0,0.35)' }}>در حال بارگذاری...</div>
  ...
  return (
    <>
      <style>{`
        ...
        .tab-btn.active { background:rgba(199,166,106,0.12);border-color:rgba(199,166,106,0.40);color:#A07840 }
        .tab-btn:not(.active) { background:#FFFFFF;color:rgba(0,0,0,0.42) }
        .tab-btn:not(.active):hover { background:rgba(0,0,0,0.04);color:rgba(0,0,0,0.65) }
        ...
        .img-thumb.active { border-color:#C7A66A;box-shadow:0 0 12px rgba(199,166,106,0.35) }
        .img-thumb:hover  { border-color:rgba(199,166,106,0.4) }

        .coach-card { padding:16px;background:#FFFFFF;border:1px solid rgba(0,0,0,0.07);border-radius:16px;transition:all 0.3s }
        .coach-card:hover { background:rgba(0,0,0,0.04);border-color:rgba(199,166,106,0.25);transform:translateY(-3px) }
        ...
        .book-fixed {
          ...
          padding:12px 16px;background:rgba(247,247,245,0.97);
          border-top:1px solid rgba(199,166,106,0.20);
      ...
      <div style={{ minHeight: '100vh', background: '#F7F7F5', direction: 'rtl', fontFamily: 'Vazirmatn, sans-serif', paddingBottom: 90 }}>
        ...
        <div style={{ position: 'relative', height: 'clamp(280px,50vw,580px)', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 55% 65% at 20% 60%,rgba(199,166,106,0.08) 0%,transparent 100%)' }} />
          ...
          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 'clamp(14px,3vw,24px) clamp(16px,4vw,36px)' }}>
            <button onClick={() => router.push('/clubs')} style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'rgba(255,255,255,0.8)', fontSize: 13, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(16px)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 10, padding: '8px 14px', cursor: 'pointer', fontFamily: 'inherit' }}>
              <ChevronRight size={14} /> باشگاه‌ها
            </button>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, background: isOpen ? 'rgba(199,166,106,0.15)' : 'rgba(239,68,68,0.15)', backdropFilter: 'blur(16px)', border: `1px solid ${isOpen ? 'rgba(199,166,106,0.3)' : 'rgba(239,68,68,0.3)'}`, borderRadius: 10, padding: '7px 14px' }}>
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: isOpen ? '#C7A66A' : '#ef4444', animation: 'pulse 2s infinite', display: 'inline-block' }} />
          ...
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 10, padding: 'clamp(16px,3vw,36px) clamp(16px,4vw,40px)' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(0,0,0,0.04)', border: '1px solid rgba(199,166,106,0.25)', borderRadius: 100, padding: '4px 14px', marginBottom: 10, backdropFilter: 'blur(12px)' }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'rgba(0,0,0,0.06)', backdropFilter: 'blur(12px)', borderRadius: 20, padding: '5px 12px', fontSize: 12, color: 'rgba(255,255,255,0.8)' }}>
                <MapPin size={11} style={{ color: '#C7A66A' }} /> {club.city}
              </div>
              {distance && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'rgba(199,166,106,0.15)', borderRadius: 20, padding: '5px 12px', fontSize: 12, color: '#6ee7b7' }}>
                  <Navigation size={11} /> {distance}
                </div>
              )}
              <div style={{ display: 'flex', gap: 2, alignItems: 'center', background: 'rgba(0,0,0,0.06)', borderRadius: 20, padding: '5px 12px' }}>
          ...
          {images.length > 1 && (
            <>
              <button onClick={() => setActiveImg(p => (p - 1 + images.length) % images.length)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', zIndex: 10, width: 38, height: 38, borderRadius: '50%', background: 'rgba(0,0,0,0.06)', backdropFilter: 'blur(12px)', border: '1px solid rgba(0,0,0,0.09)', cursor: 'pointer', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        {images.length > 1 && (
          <div style={{ background: '#F7F7F5', borderBottom: '1px solid rgba(0,0,0,0.07)', padding: '10px 16px', display: 'flex', gap: 8, overflowX: 'auto' }}>
            {images.map((img, i) => (
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: 'clamp(16px,3vw,36px) clamp(12px,3vw,28px)' }}>
          <div className="club-layout">
                  ...
                  <div style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 18, padding: 'clamp(16px,3vw,24px)' }}>
                    ...
                    <div className="amenity-grid">
                      {[
                        ...
                        { cond: true,                      label: 'تهویه مطبوع',      color: '#C7A66A' },
              ...
              {tab === 'tables' && (
                <div style={{ animation: 'fadeUp 0.4s ease both', display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {activeTables.map((t, i) => (
                    <div key={i} style={{ background: '#FFFFFF', border: `1px solid ${t.color}18`, borderRadius: 16, padding: 16, display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 100 }}>
                        <div style={{ fontSize: 15, fontWeight: 800, color: '#111111', marginBottom: 5 }}>{t.label}</div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
                        <div style={{ textAlign: 'center' }}>
                          ...
                          <div style={{ fontSize: 10, color: 'rgba(0,0,0,0.35)' }}>تومان / ساعت</div>
                        </div>
                  ...
                  <div style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 18, padding: 'clamp(16px,3vw,24px)' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                      {Object.entries(club.workingHours ?? {}).map(([day, hours]: any) => {
                        ...
                        return (
                          <div key={day} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '11px 14px', borderRadius: 10, background: isToday ? 'rgba(199,166,106,0.06)' : 'transparent', border: `1px solid ${isToday ? 'rgba(199,166,106,0.15)' : 'transparent'}` }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                              ...
                              {isToday && <span style={{ fontSize: 9, background: 'rgba(199,166,106,0.15)', color: '#C7A66A', padding: '2px 8px', borderRadius: 20, fontWeight: 700 }}>امروز</span>}
                            </div>
                            {hours.isOpen ? (
                ...
                <div style={{ animation: 'fadeUp 0.4s ease both', display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 18, padding: 'clamp(16px,3vw,24px)' }}>
                    <div style={{ display: 'flex', gap: 24, alignItems: 'center', flexWrap: 'wrap' }}>
                      <div style={{ textAlign: 'center', flexShrink: 0 }}>
                        <div style={{ fontSize: 44, fontWeight: 900, color: '#111111', lineHeight: 1 }}>۴.۸</div>
                        ...
                        <div style={{ fontSize: 11, color: 'rgba(0,0,0,0.40)' }}>۱۲۴ نظر</div>
                      </div>
                      <div style={{ flex: 1, minWidth: 160, display: 'flex', flexDirection: 'column', gap: 5 }}>
                        {[{s:5,p:72},{s:4,p:18},{s:3,p:7},{s:2,p:2},{s:1,p:1}].map(r => (
                          <div key={r.s} style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                            <span style={{ fontSize: 11, color: 'rgba(0,0,0,0.42)', width: 10, flexShrink: 0 }}>{toFa(r.s)}</span>
                            <Star size={9} style={{ color: '#f59e0b', fill: '#f59e0b', flexShrink: 0 }} />
                            <div style={{ flex: 1, height: 5, background: 'rgba(0,0,0,0.05)', borderRadius: 3, overflow: 'hidden' }}>
                              <div style={{ height: '100%', width: `${r.p}%`, background: 'linear-gradient(90deg,#f59e0b,#f59e0b80)', borderRadius: 3 }} />
                            </div>
                            <span style={{ fontSize: 10, color: 'rgba(0,0,0,0.35)', width: 24, textAlign: 'left', flexShrink: 0 }}>{toFa(r.p)}٪</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {reviews.map((r, i) => (
                    <div key={i} style={{ padding: 18, background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.05)', borderRadius: 16 }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <div style={{ width: 36, height: 36, borderRadius: 10, background: 'linear-gradient(135deg,#C7A66A,#A07840)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 900, color: '#fff', flexShrink: 0 }}>
                            {r.name[0]}
                          </div>
                          <div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2, flexWrap: 'wrap' }}>
                              <span style={{ fontSize: 13, fontWeight: 700, color: '#111111' }}>{r.name}</span>
                              {r.verified && <span style={{ fontSize: 9, color: '#C7A66A', background: 'rgba(199,166,106,0.1)', border: '1px solid rgba(199,166,106,0.2)', borderRadius: 20, padding: '2px 7px', fontWeight: 700 }}>✓ تأیید شده</span>}
                            </div>
                            <div style={{ fontSize: 11, color: 'rgba(0,0,0,0.35)' }}>{r.date}</div>
                          </div>
                        </div>
                        <div style={{ display: 'flex', gap: 2, flexShrink: 0 }}>
                          {[1,2,3,4,5].map(s => <Star key={s} size={11} style={{ color: s <= r.rating ? '#f59e0b' : 'rgba(0,0,0,0.08)', fill: s <= r.rating ? '#f59e0b' : 'transparent' }} />)}
                        </div>
                      </div>
                      <p style={{ fontSize: 13, color: 'rgba(0,0,0,0.50)', margin: 0, lineHeight: 1.75 }}>{r.text}</p>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="club-sidebar">
              <div style={{ background: '#FFFFFF', border: '1px solid rgba(199,166,106,0.2)', borderRadius: 20, padding: 20, position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ fontSize: 10, color: 'rgba(199,166,106,0.6)', letterSpacing: '0.2em', fontWeight: 700, marginBottom: 12, textAlign: 'center' }}>رزرو آنلاین</div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 14 }}>
                  {[
                    { v: '۸', l: 'میز آزاد', c: '#C7A66A' },
                    { v: '۳', l: 'مشغول',    c: '#ef4444' },
                    { v: '۱۱', l: 'کل',      c: 'rgba(0,0,0,0.45)' },
                  ].map((x, i) => (
                    <div key={i} style={{ textAlign: 'center', padding: '10px 4px', background: 'rgba(0,0,0,0.03)', borderRadius: 12, border: '1px solid rgba(0,0,0,0.04)' }}>
                      <div style={{ fontSize: 18, fontWeight: 900, color: x.c }}>{x.v}</div>
                      <div style={{ fontSize: 10, color: 'rgba(0,0,0,0.35)', marginTop: 2 }}>{x.l}</div>
                    </div>
                  ))}
                </div>
                <button onClick={goBook} style={{ width: '100%', padding: 14, border: 'none', borderRadius: 14, background: 'linear-gradient(135deg,#C7A66A,#A07840)', color: '#fff', fontSize: 14, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 8px 28px rgba(199,166,106,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  {club.website && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 13 }}>
                      ...
                      <span style={{ color: '#C7A66A' }}>{club.website.replace('https://', '')}</span>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="book-fixed">
          <button onClick={goBook} style={{ flex: 1, padding: '14px', border: 'none', borderRadius: 14, background: 'linear-gradient(135deg,#C7A66A,#A07840)', color: '#fff', fontSize: 15, fontWeight: 800, cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 8px 28px rgba(199,166,106,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
