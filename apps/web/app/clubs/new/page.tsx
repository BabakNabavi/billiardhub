    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: '#F7F7F5', direction: 'rtl', paddingBottom: '60px' }}>
        <div style={{ maxWidth: '720px', margin: '0 auto', padding: '32px clamp(16px,4vw,32px) 0' }}>
          ...
          {error && (
            <div style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.25)', color: '#fca5a5', padding: '12px 16px', borderRadius: '12px', marginBottom: '20px', fontSize: '15px' }}>
              {error}
            </div>
          )}
          ...
          <div style={sectionStyle}>
            ...
            <div className="mb-4">
              ...
              <button type="button" onClick={getLocation} disabled={locationLoading}
                style={{ background: 'rgba(59,130,246,0.12)', border: '1px solid rgba(59,130,246,0.3)', color: '#93c5fd', padding: '8px 16px', borderRadius: '10px', fontSize: '15px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', opacity: locationLoading ? 0.5 : 1 }}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <label key={item.key} style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
                  ...
                  <span style={{ fontSize: '15px', color: 'rgba(0,0,0,0.48)' }}>{item.label}</span>
                </label>
              ))}
            </div>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {days.map((day) => (
                <div key={day.key} style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <div style={{ width: '64px', fontSize: '15px', color: 'rgba(0,0,0,0.45)', fontWeight: 600, flexShrink: 0 }}>{day.label}</div>
                  <input type="checkbox"
                  ...
                  {(form.workingHours as any)[day.key].isOpen ? (
                    <>
                      <input type="time"
                        ...
                        style={{ background: 'rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.08)', color: '#111111', borderRadius: '8px', padding: '4px 8px', fontSize: '15px' }} />
                      <span style={{ fontSize: '14px', color: 'rgba(0,0,0,0.40)' }}>تا</span>
                      <input type="time"
                    ...
                    <span style={{ fontSize: '14px', color: '#fca5a5' }}>تعطیل</span>
                  )}
                </div>
              ))}
            </div>
          ...
          <div style={sectionStyle}>
            ...
            <p style={{ fontSize: '14px', color: 'rgba(0,0,0,0.40)', marginBottom: '12px' }}>حداکثر ۱۰ عکس — فرمت JPG، PNG</p>
                ...
                {imagePreviews.map((src, i) => (
                  <div key={i} style={{ position: 'relative' }}>
                    ...
                    <button onClick={() => removeImage(i)}
                      style={{ position: 'absolute', top: '4px', left: '4px', background: 'rgba(239,68,68,0.8)', color: '#fff', borderRadius: '50%', width: '20px', height: '20px', fontSize: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', cursor: 'pointer' }}>
                      ×
                    </button>
                  </div>
                ))}
          ...
          <div style={sectionStyle}>
            ...
            <p style={{ fontSize: '14px', color: 'rgba(0,0,0,0.40)', marginBottom: '12px' }}>یک ویدیوی معرفی — حداکثر ۱۰۰ مگابایت — فرمت MP4</p>
            <input type="file" accept="video/mp4,video/*" onChange={handleVideoSelect}
              className="dark-file w-full text-sm" style={{ color: 'rgba(0,0,0,0.45)' }} />
            {videoFile && (
              <p style={{ fontSize: '14px', color: '#6ee7b7', marginTop: '8px' }}>✅ {videoFile.name} انتخاب شد</p>
            )}
          </div>

          {uploadProgress && (
            <div style={{ background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.2)', color: '#93c5fd', padding: '12px 16px', borderRadius: '12px', marginBottom: '20px', fontSize: '15px', textAlign: 'center' }}>
              {uploadProgress}
            </div>
          )}
