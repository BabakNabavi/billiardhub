    ...
    <div style={{ minHeight: '100vh', background: '#F7F7F5', color: '#111111', fontFamily: 'Estedad, Vazirmatn, sans-serif' }} dir="rtl">
      ...
      <div style={{ position: 'relative', overflow: 'hidden', background: 'linear-gradient(180deg,#111111 0%,#1C1C1A 100%)', borderBottom: '1px solid rgba(0,0,0,0.05)' }}>
        ...
        <div style={{ position: 'relative', maxWidth: '1200px', margin: '0 auto', padding: '64px 24px 40px' }}>
          ...
          <h1 style={{
            fontSize: 'clamp(40px,8vw,72px)', fontWeight: 900, textAlign: 'center',
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '40px 24px 80px' }}>
          ...
          <div style={{ marginBottom: 48 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 16 }}>
              {featured[0] && (
                ...
                >
                  <div style={{ position: 'absolute', top: 24, right: 24, fontSize: 56, opacity: 0.15 }}>{featured[0].image}</div>
                  ...
                  <div style={{ position: 'relative' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12 }}>
                      <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 99, background: 'rgba(199,166,106,0.18)', color: GOLD, fontWeight: 700 }}>{featured[0].category}</span>
                      {featured[0].tag && <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 99, background: `${featured[0].tagColor}22`, color: featured[0].tagColor, border: `1px solid ${featured[0].tagColor}44`, fontWeight: 700 }}>{featured[0].tag}</span>}
                    </div>
                    <h3 style={{ fontSize: 'clamp(18px,2.5vw,26px)', fontWeight: 900, color: '#FFFFFF', margin: '0 0 10px', lineHeight: 1.3, letterSpacing: '-0.02em' }}>{featured[0].title}</h3>
                    <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.50)', margin: '0 0 16px', lineHeight: 1.65 }}>{featured[0].excerpt}</p>
              ...
              {featured.slice(1).map(n => (
                ...
                >
                  <div style={{ position: 'absolute', top: 16, left: 16, fontSize: 36, opacity: 0.10 }}>{n.image}</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
                    <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 99, background: 'rgba(199,166,106,0.10)', color: GOLD_DARK, fontWeight: 700 }}>{n.category}</span>
                    {n.tag && <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 99, background: `${n.tagColor}18`, color: n.tagColor, fontWeight: 700 }}>{n.tag}</span>}
                  </div>
                  <h3 style={{ fontSize: 15, fontWeight: 800, color: '#111111', margin: '0 0 8px', lineHeight: 1.4, letterSpacing: '-0.01em' }}>{n.title}</h3>
                  <p style={{ fontSize: 12, color: 'rgba(0,0,0,0.48)', margin: '0 0 12px', lineHeight: 1.65 }}>{n.excerpt}</p>
                  <div style={{ fontSize: 11, color: 'rgba(0,0,0,0.35)', marginTop: 'auto' }}>{n.author} · {n.date}</div>
                </Link>
              ))}
            </div>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 20 }}>
          {regular.map(n => (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 110, fontSize: 48, background: 'linear-gradient(135deg,rgba(199,166,106,0.06),rgba(199,166,106,0.03))', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
                {n.image}
              </div>
              <div style={{ padding: 20, flex: 1, display: 'flex', flexDirection: 'column' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12 }}>
                  <span style={{ fontSize: 10, padding: '3px 9px', borderRadius: 99, background: 'rgba(199,166,106,0.10)', color: GOLD_DARK, fontWeight: 700 }}>{n.category}</span>
                  {n.tag && <span style={{ fontSize: 10, padding: '3px 9px', borderRadius: 99, background: `${n.tagColor}18`, color: n.tagColor, fontWeight: 700 }}>{n.tag}</span>}
                </div>
                <h3 style={{ fontSize: 15, fontWeight: 800, color: '#111111', margin: '0 0 8px', lineHeight: 1.4, flex: 1, letterSpacing: '-0.01em' }}>{n.title}</h3>
                <p style={{ fontSize: 12, color: 'rgba(0,0,0,0.48)', margin: '0 0 16px', lineHeight: 1.65 }}>{n.excerpt}</p>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 11, color: 'rgba(0,0,0,0.35)' }}>
                  <span>{n.author}</span>
        ...
        {filtered.length === 0 && (
          <div style={{ textAlign: 'center', padding: '80px 24px', color: 'rgba(0,0,0,0.40)' }}>
            ...
            <p style={{ fontSize: 16 }}>خبری یافت نشد</p>
          </div>
        )}
      </div>
    </div>
