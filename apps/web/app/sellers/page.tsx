  ...
  return (
    <div style={{ background: '#010604', minHeight: '100vh', color: '#f0faf5', fontFamily: 'Vazirmatn, system-ui', direction: 'rtl' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #051a10 0%, #030d08 100%)', padding: 'clamp(40px, 8vw, 80px) clamp(16px, 4vw, 32px) clamp(28px, 5vw, 48px)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, backgroundImage: 'radial-gradient(circle, rgba(16,185,129,0.06) 1px, transparent 1px)', backgroundSize: '28px 28px' }} />
        <div style={{ position: 'relative', maxWidth: 600, margin: '0 auto' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.25)', color: '#10b981', fontSize: 12, padding: '5px 16px', borderRadius: 20, marginBottom: 16, fontWeight: 600 }}>
            🏪 فروشندگان تجهیزات
          </div>
          <h1 style={{ fontSize: 'clamp(22px, 5vw, 36px)', fontWeight: 900, margin: '0 0 12px', background: 'linear-gradient(135deg, #f0faf5, #10b981)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: 'clamp(20px, 4vw, 32px) clamp(16px, 4vw, 32px)' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 28, alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 6, overflowX: 'auto', flexShrink: 0 }}>
            {CITIES.map(city => (
              <button key={city} onClick={() => setActiveCity(city)} style={{
                background: activeCity === city ? '#10b981' : 'rgba(255,255,255,0.04)',
                border: `1px solid ${activeCity === city ? '#10b981' : 'rgba(255,255,255,0.08)'}`,
          ...
          <button onClick={() => setOnlyVerified(!onlyVerified)} style={{
            background: onlyVerified ? 'rgba(16,185,129,0.15)' : 'rgba(255,255,255,0.04)',
            border: `1px solid ${onlyVerified ? 'rgba(16,185,129,0.4)' : 'rgba(255,255,255,0.08)'}`,
          ...
          <select value={sort} onChange={e => setSort(e.target.value as any)} style={{
            ...
            color: '#94a3b8', padding: '7px 14px', borderRadius: 20, fontSize: 12, cursor: 'pointer',
            marginRight: 'auto',
          }}>
          ...
          {filtered.map(seller => (
            <Link key={seller.id} href={`/sellers/${seller.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
              <div style={{
                ...
                border: '1px solid rgba(16,185,129,0.1)', borderRadius: 18,
                overflow: 'hidden', transition: 'all 0.3s', cursor: 'pointer', height: '100%',
              }}
              onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.border = '1px solid rgba(16,185,129,0.35)'; (e.currentTarget as HTMLDivElement).style.transform = 'translateY(-4px)'; (e.currentTarget as HTMLDivElement).style.boxShadow = '0 12px 40px rgba(16,185,129,0.1)'; }}
              onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.border = '1px solid rgba(16,185,129,0.1)'; (e.currentTarget as HTMLDivElement).style.transform = 'translateY(0)'; (e.currentTarget as HTMLDivElement).style.boxShadow = 'none'; }}
              >
                {/* Card Header */}
                <div style={{ height: 90, background: 'linear-gradient(135deg, rgba(16,185,129,0.06), rgba(6,182,212,0.04))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 48, position: 'relative', opacity: 0.7 }}>
                  {seller.emoji}
                  {seller.elite && (
                ...
                <div style={{ padding: '16px 18px 18px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
                    <h3 style={{ color: '#f0faf5', fontSize: 15, fontWeight: 700, margin: 0, lineHeight: 1.4, flex: 1 }}>{seller.name}</h3>
                    {seller.verified && (
                      <span style={{ background: 'rgba(16,185,129,0.12)', border: '1px solid rgba(16,185,129,0.25)', color: '#10b981', fontSize: 10, padding: '2px 7px', borderRadius: 10, fontWeight: 600, flexShrink: 0, marginRight: 8 }}>✓</span>
                    )}
                  </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                    ...
                    <span style={{ color: '#f0faf5', fontSize: 13, fontWeight: 700 }}>{seller.rating}</span>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
                    {seller.brands.slice(0, 3).map(b => (
                      <span key={b} style={{ background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.15)', color: '#10b981', fontSize: 10, padding: '2px 8px', borderRadius: 10 }}>{b}</span>
                    ))}
                  </div>
                  {/* Footer */}
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 12, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    <span style={{ color: '#10b981', fontSize: 12, fontWeight: 600 }}>مشاهده فروشگاه ←</span>
                  </div>
                </div>
              </div>
            </Link>
