...
export default function SellersPage() {
  ...
  useEffect(() => {
    api.get('/user/by-role/seller')
      .then(res => { setSellers(res.data); setLoading(false); })
      .catch(() => { setSellers(mockSellers); setLoading(false); });
  }, []);
    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg,#010604 0%,#050c08 100%)', padding: 'clamp(24px,4vw,48px) clamp(16px,3vw,32px)', direction: 'rtl' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          ...
          {/* Count */}
          <div style={{ marginBottom: '20px', color: 'rgba(240,250,245,0.3)', fontSize: '13px' }}>
            {filtered.length} فروشنده یافت شد
          </div>
          ...
          {loading ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: '16px' }}>
              {[1,2,3,4,5,6].map(i => (
                <div key={i} style={{ height: '180px', background: 'rgba(255,255,255,0.03)', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.05)' }} />
              ))}
            </div>
          ) : filtered.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '80px 20px', color: 'rgba(240,250,245,0.2)' }}>
              <ShoppingBag size={40} style={{ opacity: 0.2, marginBottom: '12px', display: 'block', margin: '0 auto 12px' }} />
              <p style={{ fontSize: '14px' }}>فروشنده‌ای پیدا نشد</p>
            </div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: '16px' }}>
              {filtered.map((seller, i) => (
                <Link key={seller.id} href={`/users/${seller.id}`} style={{ textDecoration: 'none' }}>
                  <div className="seller-card" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '20px', padding: '20px', cursor: 'pointer', animation: `fadeUp 0.5s ${i * 0.05}s ease both` }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        {seller.city && (
                          <div style={{ display: 'flex', alignItems: 'center', gap: '4px', color: 'rgba(240,250,245,0.35)', fontSize: '12px' }}>
                            <MapPin size={11} /><span>{seller.city}</span>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    {seller.bio && <p style={{ color: 'rgba(240,250,245,0.4)', fontSize: '12px', lineHeight: 1.6, margin: 0, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{seller.bio}</p>}
                  </div>
                </Link>
              ))}
            </div>
