      ...
      <div style={{ background: '#F7F7F5', minHeight: '100vh', direction: 'rtl', fontFamily: 'Vazirmatn,Tahoma,sans-serif', color: TEXT }}>
        ...
        <div style={{ position: 'relative', height: 'clamp(240px,32vw,380px)', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'absolute', top: 20, right: 20, zIndex: 10 }}>
            <Link href="/sellers" style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 13, color: 'rgba(255,255,255,0.88)', textDecoration: 'none', padding: '7px 14px', borderRadius: 24, background: 'rgba(0,0,0,0.32)', backdropFilter: 'blur(20px) saturate(180%)', WebkitBackdropFilter: 'blur(20px) saturate(180%)', border: '1px solid rgba(255,255,255,0.18)', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.16)', transition: 'background 0.2s' }}
          ...
          <div style={{ marginTop: -70, position: 'relative', zIndex: 20, background: LQ_BG, backdropFilter: 'blur(40px) saturate(220%)', WebkitBackdropFilter: 'blur(40px) saturate(220%)', border: LQ_BOR, borderRadius: 24, boxShadow: LQ_SHAD, padding: 'clamp(18px,3vw,28px)', marginBottom: 24, animation: 'fadeUp 0.4s ease both', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1, display: 'flex', gap: 'clamp(14px,2vw,24px)', flexWrap: 'wrap', alignItems: 'flex-start' }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignSelf: 'center' }}>
                <a href={`tel:${seller.phone}`} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '11px 20px', borderRadius: 12, background: `linear-gradient(135deg,${GOLD},#A07840)`, color: '#fff', fontSize: 14, fontWeight: 700, textDecoration: 'none', boxShadow: 'inset 0 1.5px 0 rgba(255,255,255,0.30), 0 4px 16px rgba(199,166,106,0.42)', whiteSpace: 'nowrap' }}>
                ...
                <a href={waLink} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '11px 18px', borderRadius: 12, background: 'linear-gradient(135deg,#25D366,#128C7E)', color: '#fff', fontSize: 14, fontWeight: 700, textDecoration: 'none', boxShadow: 'inset 0 1.5px 0 rgba(255,255,255,0.28), 0 4px 16px rgba(37,211,102,0.34)', whiteSpace: 'nowrap' }}>
            ...
            <div className="cat-scroll" style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4, marginBottom: 18 }}>
              {allCats.map(cat => (
                <button key={cat} className="chip-btn" onClick={() => setActiveCat(cat)} style={{
                  ...
                  background: activeCat === cat ? `linear-gradient(135deg,${GOLD},#A07840)` : 'rgba(255,255,255,0.80)',
                  backdropFilter: activeCat === cat ? 'none' : 'blur(14px) saturate(190%)',
                  WebkitBackdropFilter: activeCat === cat ? 'none' : 'blur(14px) saturate(190%)',
                  color: activeCat === cat ? '#fff' : TEXT_SEC,
                  boxShadow: activeCat === cat ? 'inset 0 1.5px 0 rgba(255,255,255,0.28), 0 3px 10px rgba(199,166,106,0.38)' : 'inset 0 1.5px 0 rgba(255,255,255,0.95), 0 2px 8px rgba(0,0,0,0.06)',
                  border: activeCat === cat ? 'none' : '1px solid rgba(255,255,255,0.88)',
                }}>
                  {cat}
                </button>
              ))}
