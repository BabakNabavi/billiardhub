...
export default function ContactPage() {
  ...
  const labelStyle: React.CSSProperties = { fontSize: 12, fontWeight: 600, color: 'rgba(0,0,0,0.50)', marginBottom: 6, display: 'block' }
        ...
        <div style={{ position: 'relative', height: 240, overflow: 'hidden', background: '#F7F7F5' }}>
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(199,166,106,0.08)', border: '1px solid rgba(199,166,106,0.20)', borderRadius: 100, padding: '6px 18px', animation: 'fadeUp 0.6s ease both' }}>
              <span style={{ fontSize: 10, color: '#C7A66A', fontWeight: 700, letterSpacing: '0.22em' }}>CONTACT US</span>
            </div>
            <h1 style={{ fontSize: 'clamp(28px,4vw,46px)', fontWeight: 900, color: '#fff', margin: 0, letterSpacing: '-0.035em', animation: 'fadeUp 0.6s ease 0.15s both' }}>تماس با ما</h1>
            <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.35)', margin: 0, animation: 'fadeUp 0.6s ease 0.3s both' }}>همیشه در کنار شما هستیم</p>
          </div>
        </div>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 1100, margin: '0 auto', padding: '60px 24px 80px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginBottom: 60 }}>
            {[
              ...
              <div key={i} className="info-card" style={{ padding: '24px 20px', background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 20, animation: `fadeUp 0.5s ease ${i * 0.1}s both` }}>
                ...
                <p style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)', margin: '0 0 6px', fontWeight: 700 }}>{c.title}</p>
                <p style={{ fontSize: 14, fontWeight: 700, color: '#111111', margin: '0 0 4px' }}>{c.value}</p>
                <p style={{ fontSize: 12, color: 'rgba(0,0,0,0.38)', margin: 0 }}>{c.sub}</p>
              </div>
            ))}
          </div>
              ...
              <div style={{ padding: '32px' }}>
                {sent ? (
                  <div style={{ textAlign: 'center', padding: '40px 20px', animation: 'scaleIn 0.5s ease both' }}>
                    ...
                    <h2 style={{ fontSize: 22, fontWeight: 900, color: '#111111', margin: '0 0 12px' }}>پیام ارسال شد!</h2>
                    <p style={{ fontSize: 14, color: 'rgba(0,0,0,0.45)', lineHeight: 1.7, margin: '0 0 28px' }}>
                      تیم پشتیبانی ما در اسرع وقت با شما تماس خواهد گرفت.
                    </p>
                    <button onClick={() => { setSent(false); setForm({ name: '', email: '', phone: '', subject: '', message: '' }) }}
                      style={{ padding: '12px 24px', borderRadius: 12, background: 'rgba(199,166,106,0.10)', border: '1px solid rgba(199,166,106,0.25)', color: '#C7A66A', fontSize: 14, fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer' }}>
                      ارسال پیام جدید
                    </button>
                  </div>
                ) : (
                  <>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 28 }}>
                      <div style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(199,166,106,0.10)', border: '1px solid rgba(199,166,106,0.20)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <i className="ti ti-send" style={{ fontSize: 17, color: '#C7A66A' }} />
                      </div>
                      <h2 style={{ fontSize: 17, fontWeight: 800, color: '#111111', margin: 0 }}>ارسال پیام</h2>
                    </div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                        ...
                        <div style={{ gridColumn: 'span 2' }}>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 }}>
                            {errors.message && <p style={{ fontSize: 11, color: '#ef4444', margin: 0 }}>{errors.message}</p>}
                            <span style={{ fontSize: 11, color: 'rgba(0,0,0,0.38)', marginRight: 'auto' }}>{toFa(form.message.length)} کاراکتر</span>
                          </div>
                        </div>
                      </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 20, overflow: 'hidden' }}>
                ...
                <div style={{ padding: '20px' }}>
                  <p style={{ fontSize: 13, fontWeight: 800, color: '#111111', margin: '0 0 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <i className="ti ti-clock" style={{ fontSize: 15, color: '#C7A66A' }} />
                    ساعت پشتیبانی
                  </p>
                  {[
                    ...
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, padding: '10px 0', borderBottom: i < 2 ? '1px solid rgba(0,0,0,0.06)' : 'none' }}>
                      <span style={{ color: 'rgba(0,0,0,0.45)' }}>{r.day}</span>
                      <span style={{ color: r.active ? '#C7A66A' : '#ef4444', fontWeight: 600 }}>{r.hours}</span>
                    </div>
                  ))}
                  <div style={{ marginTop: 14, padding: '10px 12px', background: 'rgba(199,166,106,0.06)', borderRadius: 10, border: '1px solid rgba(199,166,106,0.12)', fontSize: 12, color: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', gap: 6 }}>
                    <i className="ti ti-info-circle" style={{ fontSize: 13, color: '#C7A66A', flexShrink: 0 }} />
              ...
              <div style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 20, padding: '20px' }}>
                <p style={{ fontSize: 13, fontWeight: 800, color: '#111111', margin: '0 0 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
                  <i className="ti ti-share" style={{ fontSize: 15, color: '#C7A66A' }} />
                  شبکه‌های اجتماعی
                </p>
                <div className="social-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  {[
                    ...
                    <div key={s.label} style={{ padding: '12px', background: s.bg, border: `1px solid ${s.color}20`, borderRadius: 12, display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', transition: 'all 0.2s' }}
                      ...
                      <div>
                        <p style={{ fontSize: 11, fontWeight: 700, color: '#111111', margin: '0 0 1px' }}>{s.label}</p>
                        <p style={{ fontSize: 10, color: 'rgba(0,0,0,0.38)', margin: 0 }}>{s.handle}</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div style={{ background: 'rgba(199,166,106,0.04)', border: '1px solid rgba(199,166,106,0.12)', borderRadius: 20, padding: '20px' }}>
                <p style={{ fontSize: 13, fontWeight: 800, color: '#111111', margin: '0 0 14px', display: 'flex', alignItems: 'center', gap: 8 }}>
                  <i className="ti ti-link" style={{ fontSize: 15, color: '#C7A66A' }} />
                  لینک‌های مفید
                </p>
                {[
                  ...
                  <Link key={l.href} href={l.href} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 0', borderBottom: '1px solid rgba(0,0,0,0.06)', color: 'rgba(0,0,0,0.50)', textDecoration: 'none', fontSize: 13, transition: 'color 0.2s' }}
                    ...
                    <i className={`ti ${l.icon}`} style={{ fontSize: 14, color: '#C7A66A', opacity: 0.7 }} />
                    {l.label}
                    <i className="ti ti-arrow-left" style={{ fontSize: 12, marginRight: 'auto' }} />
                  </Link>
                ))}
              </div>
            </div>
          ...
          <section>
            <div style={{ textAlign: 'center', marginBottom: 36 }}>
              <p style={{ fontSize: 10, color: '#A07840', letterSpacing: '0.22em', fontWeight: 700, marginBottom: 10, textTransform: 'uppercase' }}>FAQ</p>
              <h2 style={{ fontSize: 'clamp(22px,2.5vw,30px)', fontWeight: 900, color: '#111111', margin: 0 }}>سوالات متداول</h2>
            </div>
              ...
              <div style={{ padding: '8px 24px' }}>
                {FAQ_ITEMS.map((item, i) => (
                  <div key={i} style={{ borderBottom: i < FAQ_ITEMS.length - 1 ? '1px solid rgba(0,0,0,0.06)' : 'none' }}>
                    ...
                    >
                      <span style={{ fontSize: 14, fontWeight: 700, color: faqOpen === i ? '#C7A66A' : '#111111', transition: 'color 0.2s' }}>{item.q}</span>
                      <span style={{ fontSize: 20, color: '#C7A66A', transition: 'transform 0.3s', transform: faqOpen === i ? 'rotate(45deg)' : 'rotate(0)', flexShrink: 0, lineHeight: 1 }}>+</span>
                    </button>
                    {faqOpen === i && (
                      <p style={{ fontSize: 13, color: 'rgba(0,0,0,0.50)', lineHeight: 1.8, margin: '0 0 16px', paddingRight: 4 }}>{item.a}</p>
                    )}
                  </div>
                ))}
              </div>
