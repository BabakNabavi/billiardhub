...
export default function HomePage() {
  ...
  const handleClubsScroll = useCallback(() => {
    ...
    cards.forEach((card, i) => {
      ...
      card.style.filter = t > 0.25
        ? `drop-shadow(0 ${(t * 10).toFixed(1)}px ${(t * 18).toFixed(1)}px rgba(48,197,90,${(t * 0.38).toFixed(2)}))`
        : 'none';
      if (dist < minDist) { minDist = dist; newActive = i; }
    });
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .club-mob-card { transform-origin:center; position:relative; border-radius:12px; overflow:hidden; }
        ...
        <div style={{ maxWidth: '1340px', margin: '0 auto' }}>
          <SR>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '48px', flexWrap: 'wrap', gap: '20px' }}>
              ...
              <Link href="/shop" style={{ display: 'flex', alignItems: 'center', gap: '6px', textDecoration: 'none', color: '#30C55A', fontSize: '15px', fontWeight: 700, transition: 'color 0.25s', textShadow: '0 0 12px rgba(48,197,90,0.35)' }}
