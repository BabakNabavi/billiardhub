...
export default function PlayersPage() {
  ...
  useEffect(() => {
    api.get('/user/by-role/player')
      .then(res => { setPlayers(res.data?.length ? res.data : mockPlayers); setLoading(false); })
        ...
        <div style={{maxWidth:'1100px',margin:'0 auto'}}>
          ...
          <div style={{marginBottom:'32px',animation:'fadeUp 0.6s ease both'}}>
            <div style={{display:'flex',alignItems:'center',gap:'12px',marginBottom:'8px'}}>
              ...
              <div>
                ...
                <p style={{color:'rgba(240,250,245,0.35)',fontSize:'13px',margin:'2px 0 0'}}>بازیکنان حرفه‌ای بیلیارد ایران</p>
              </div>
            </div>
          </div>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'12px',marginBottom:'28px',animation:'fadeUp 0.6s 0.1s ease both'}}>
            ...
            <div style={{display:'flex',gap:'8px',flexWrap:'wrap'}}>
              ...
              <span style={{marginRight:'auto',color:'rgba(240,250,245,0.25)',fontSize:'12px',alignSelf:'center'}}>{filtered.length} نفر</span>
            </div>
          </div>
          ...
          {loading?(
            ...
            <div style={{textAlign:'center',padding:'80px 20px',color:'rgba(240,250,245,0.2)'}}>
              ...
              <p style={{fontSize:'14px'}}>بازیکنی پیدا نشد</p>
            </div>
          ):(
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))',gap:'16px'}}>
              {filtered.map((player,i)=>{
                ...
                return(
                  <Link key={player.id} href={`/players/${player.id}`} style={{textDecoration:'none'}}>
                    <div className="player-card" style={{background:'rgba(255,255,255,0.025)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:'20px',padding:'20px',cursor:'pointer',animation:`fadeUp 0.5s ${i*0.04}s ease both`,position:'relative',overflow:'hidden'}}>
                      ...
                      <div style={{display:'flex',alignItems:'flex-start',gap:'14px',marginBottom:'14px'}}>
                        <div style={{width:'48px',height:'48px',borderRadius:'14px',background:`linear-gradient(135deg,${specColor}33,${specColor}11)`,border:`1px solid ${specColor}33`,display:'flex',alignItems:'center',justifyContent:'center',color:specColor,fontWeight:900,fontSize:'18px',flexShrink:0,overflow:'hidden'}}>
                          {player.avatar?<img src={player.avatar} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>:player.firstName?.[0]}
                        </div>
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{display:'flex',alignItems:'center',gap:'6px',marginBottom:'2px'}}>
                            <span style={{color:'#f0faf5',fontWeight:700,fontSize:'15px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{player.firstName} {player.lastName}</span>
                            {player.verificationStatus==='verified'&&<CheckCircle size={13} color={specColor} style={{flexShrink:0}}/>}
                          </div>
                          {player.city&&<div style={{display:'flex',alignItems:'center',gap:'4px',color:'rgba(240,250,245,0.35)',fontSize:'12px'}}><MapPin size={11}/><span>{player.city}</span></div>}
                        </div>
                        <ChevronLeft size={16} color="rgba(240,250,245,0.15)"/>
                      </div>
                      {player.playerProfile&&(
                        <div style={{display:'flex',flexWrap:'wrap',gap:'6px',marginBottom:'10px'}}>
                          {player.playerProfile.specialty&&<span style={{fontSize:'11px',fontWeight:600,color:specColor,background:`${specColor}14`,border:`1px solid ${specColor}22`,borderRadius:'20px',padding:'3px 10px'}}>{specialtyLabels[player.playerProfile.specialty]??player.playerProfile.specialty}</span>}
                          {player.playerProfile.level&&<span style={{fontSize:'11px',color:'rgba(240,250,245,0.4)',background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.07)',borderRadius:'20px',padding:'3px 10px'}}>{levelLabels[player.playerProfile.level]??player.playerProfile.level}</span>}
                          {player.playerProfile.experience&&<span style={{fontSize:'11px',color:'rgba(240,250,245,0.3)',background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.06)',borderRadius:'20px',padding:'3px 10px'}}>{player.playerProfile.experience} سال</span>}
                        </div>
                      )}
                      {player.bio&&<p style={{color:'rgba(240,250,245,0.4)',fontSize:'12px',lineHeight:1.6,margin:0,overflow:'hidden',display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical'}}>{player.bio}</p>}
                    </div>
                  </Link>
                );
              })}
