    ...
    <div style={{ direction: 'rtl', fontFamily: "'Vazirmatn',Tahoma,sans-serif", background: BG, minHeight: '100vh', color: TEXT }}>
      <div style={{ maxWidth: 940, margin: '0 auto', padding: '28px clamp(16px,4vw,32px) 80px' }}>
        ...
        <div style={{ marginBottom: 22 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: 'rgba(199,166,106,0.12)', border: '1px solid rgba(199,166,106,0.30)', color: GOLD_D, fontSize: 11, fontWeight: 800, borderRadius: 20, padding: '4px 12px', letterSpacing: '0.08em', marginBottom: 10 }}>ADMIN · COACHES</div>
          <h1 style={{ fontSize: 'clamp(21px,3vw,27px)', fontWeight: 900, letterSpacing: '-0.02em' }}>تایید مربیان</h1>
          <p style={{ fontSize: 13.5, color: TEXT_S, marginTop: 6 }}>
            درخواست‌های ثبت‌شده‌ی مربیان را بررسی کنید. تایید به معنی اجازه‌ی داشتن پروفایل در سایت است؛ در صورت آپلود مدرک می‌توانید تیک آبی بدهید، در غیر این صورت مربی «آزاد» معرفی می‌شود.
          </p>
                ...
                <div key={c.slug} style={card}>
                  ...
                  <div style={{ padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
                    <div style={{ width: 44, height: 44, borderRadius: '50%', overflow: 'hidden', background: 'rgba(17,17,16,0.06)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', border: CBOR }}>
                      {c.photo ? <img loading="lazy" decoding="async" src={c.photo} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontWeight: 800, color: TEXT_M }}>{c.firstNameFa[0] ?? '؟'}</span>}
                    </div>
                    <div style={{ flex: 1, minWidth: 160 }}>
                      ...
                      <div style={{ fontSize: 12, color: TEXT_S, marginTop: 3, direction: 'ltr', textAlign: 'right' }}>
                        {c.firstNameEn} {c.lastNameEn} · /coaches/{c.slug}
                      </div>
                    ...
                    <span style={{ ...btn(hasCert ? 'rgba(5,118,66,0.08)' : 'rgba(17,17,16,0.04)', hasCert ? '#057642' : TEXT_M, hasCert ? '1px solid rgba(5,118,66,0.20)' : CBOR), cursor: 'default' }}>
                  ...
                  {isOpen && (
                    <div style={{ borderTop: CBOR, padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                      ...
                      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                        ...
                        <Link href={`/coaches/${c.slug}`} style={btn('transparent', GOLD_D, '1px solid rgba(199,166,106,0.34)')}>مشاهده پروفایل</Link>
                      </div>
                    </div>
                  )}
