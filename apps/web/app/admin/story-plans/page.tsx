...
/* بسته‌های استوری — آینه‌ی دقیقِ بسته‌های استوری، روی جدول‌های story_*.
   ...
   کاربرِ عادی استوری می‌گذارد. کاربری که چند نقش دارد، سخاوتمندانه‌ترین
   سهمیه را می‌گیرد.

   نکته‌ی متفاوت با آگهی: سقفِ استوری از اول وجود داشته، پس کلیدِ زیر
   فقط تعیین می‌کند «خریدِ بسته معنا دارد یا نه»؛ سهمیه‌ی رایگانِ نقش
   همیشه اعمال می‌شود. */
  ...
  return (
    <div dir="rtl" style={{ fontFamily: 'var(--font-base)', padding: 'clamp(14px,2.5vw,28px)', maxWidth: 1120, margin: '0 auto' }}>
      ...
      <section style={CARD}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
          <div style={{ minWidth: 250, flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 900, color: INK, marginBottom: 5 }}>فروشِ بسته‌ی استوری</div>
            <p style={{ fontSize: 12.5, color: MUT, margin: 0, lineHeight: 1.95 }}>
              سهمیه‌ی رایگانِ هر نقش همیشه اعمال می‌شود. این کلید تعیین می‌کند که وقتی
              سهمیه تمام شد، به کاربر پیشنهادِ خریدِ بسته داده شود یا فقط پیامِ «سقف پر شد».
            </p>
          </div>
          <button
            onClick={() => saveSettings({ stories_quota_enabled: !quotaOn }, quotaOn ? 'فروشِ بسته خاموش شد' : 'فروشِ بسته روشن شد')}
            ...
            <Power size={15} /> {quotaOn ? 'روشن — بسته‌ها قابلِ فروش‌اند' : 'خاموش'}
          </button>
        </div>
        ...
        <div style={{ borderTop: `1px dashed ${LINE}`, marginTop: 20, paddingTop: 18 }}>
          ...
          <p style={{ fontSize: 12.5, color: MUT, margin: '0 0 16px', lineHeight: 1.95 }}>
            عددِ صفر یعنی آن نقش اصلاً حقِ استوری ندارد. کاربری که چند نقش دارد، سخاوتمندانه‌ترین سهمیه را می‌گیرد —
      ...
      <section style={CARD}>
        ...
        <button onClick={() => saveSettings({ story_platform_bank: bank }, 'حسابِ واریز ذخیره شد')}
      ...
      <section style={CARD}>
        ...
        <div style={{ display: 'grid', gap: 14, gridTemplateColumns: 'repeat(auto-fit,minmax(170px,1fr))' }}>
          ...
          <div style={{ gridColumn: '1 / -1' }}><label style={LABEL}>توضیح</label>
            <input style={INPUT} value={draft.description} onChange={e => setDraft(d => ({ ...d, description: e.target.value }))} placeholder="مناسبِ باشگاه‌های فعال" /></div>
        </div>
    ...
    <div style={{
      ...
      <div style={{ minWidth: 190, flex: 1 }}>
        ...
        <div style={{ fontSize: 12, color: MUT, marginTop: 6, lineHeight: 1.95 }}>
          {p.quota === 0 ? 'استوریِ نامحدود' : `${fa(p.quota)} استوری در هر ${PERIOD_FA[p.period]}`}
