...
const CONVS: Conversation[] = [
  ...
  },
  {
    ...
    avatar: 'س', avatarColor: '#C7A66A', lastMsg: 'رزرو شما تأیید شد', lastTime: '۱۱:۰۵',
...
const TYPE_CONFIG: Record<ConvType, { icon: React.ReactNode; color: string; label: string }> = {
  ...
  club:       { icon: <Shield size={11} />,        color: '#C7A66A', label: 'باشگاه' },
  seller:     { icon: <ShoppingBag size={11} />,   color: '#f59e0b', label: 'فروشنده' },
  booking:    { icon: <Calendar size={11} />,      color: '#C7A66A', label: 'رزرو' },
...
function MsgBubble({ msg, isMe }: { msg: Message; isMe: boolean }) {
  if (msg.type === 'system') return (
    <div style={{ display: 'flex', justifyContent: 'center', margin: '8px 0' }}>
      <div style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: '20px', padding: '6px 16px', fontSize: '14px', color: 'rgba(245,158,11,0.8)', fontWeight: 600, maxWidth: '80%', textAlign: 'center' }}>
        {msg.text}
      </div>
    </div>
  );
    ...
    <div style={{ display: 'flex', justifyContent: isMe ? 'flex-start' : 'flex-end', margin: '4px 0' }}>
      <div style={{ maxWidth: '280px', background: 'rgba(199,166,106,0.08)', border: '1px solid rgba(199,166,106,0.2)', borderRadius: '16px', overflow: 'hidden' }}>
        <div style={{ background: 'linear-gradient(135deg,#064e3b,#065f46)', padding: '12px 14px' }}>
          <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.5)', fontWeight: 700, marginBottom: '4px' }}>تأییدیه رزرو</div>
          <div style={{ fontSize: '16px', fontWeight: 800, color: '#fff' }}>{msg.bookingCard.club}</div>
        </div>
        <div style={{ padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
          {[
            ...
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px' }}>
              <span style={{ color: 'rgba(0,0,0,0.42)' }}>{r.l}</span>
              <span style={{ color: '#111111', fontWeight: 600 }}>{r.v}</span>
            </div>
          ))}
          <div style={{ marginTop: '4px', padding: '5px 10px', background: 'rgba(199,166,106,0.12)', border: '1px solid rgba(199,166,106,0.25)', borderRadius: '20px', fontSize: '12px', color: '#C7A66A', fontWeight: 700, textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
  ...
  return (
    <div style={{ display: 'flex', justifyContent: isMe ? 'flex-start' : 'flex-end', margin: '3px 0', animation: 'msgIn 0.25s cubic-bezier(0.22,1,0.36,1) both' }}>
      <div style={{
        ...
        border: isMe ? '1px solid rgba(0,0,0,0.06)' : 'none',
        boxShadow: isMe ? 'none' : '0 4px 16px rgba(199,166,106,0.25)',
        position: 'relative',
      }}>
        <p style={{ fontSize: '16px', color: isMe ? 'rgba(0,0,0,0.55)' : '#fff', margin: 0, lineHeight: 1.55, wordBreak: 'break-word' }}>
          {msg.text}
        </p>
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginTop: '4px', justifyContent: isMe ? 'flex-start' : 'flex-end' }}>
          <span style={{ fontSize: '12px', color: isMe ? 'rgba(0,0,0,0.30)' : 'rgba(255,255,255,0.5)' }}>{msg.time}</span>
          {!isMe && (
...
function ConvItem({ conv, active, onClick }: { conv: Conversation; active: boolean; onClick: () => void }) {
  ...
  return (
    <div onClick={onClick} style={{
      ...
      background: active ? 'rgba(199,166,106,0.08)' : 'transparent',
      border: `1px solid ${active ? 'rgba(199,166,106,0.2)' : 'transparent'}`,
      transition: 'all 0.2s',
      position: 'relative',
    }}
      ...
      <div style={{ position: 'relative', flexShrink: 0 }}>
        ...
        {/* Online dot */}
        {conv.online && (
          <div style={{ position: 'absolute', bottom: '1px', right: '1px', width: '11px', height: '11px', borderRadius: '50%', background: '#C7A66A', border: '2px solid rgba(6,13,10,0.98)', boxShadow: '0 0 6px #C7A66A' }} />
        )}
        {/* Type badge */}
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px', minWidth: 0 }}>
            <span style={{ fontSize: '16px', fontWeight: conv.unread > 0 ? 800 : 600, color: active ? '#C7A66A' : '#f0faf5', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {conv.name}
            </span>
            {conv.verified && <Check size={11} style={{ color: '#C7A66A', flexShrink: 0 }} />}
            {conv.pinned && <span style={{ fontSize: '10px' }}>📌</span>}
          </div>
          <span style={{ fontSize: '12px', color: 'rgba(0,0,0,0.30)', flexShrink: 0, marginRight: '4px' }}>{conv.lastTime}</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: '14px', color: conv.unread > 0 ? 'rgba(0,0,0,0.48)' : 'rgba(0,0,0,0.40)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: conv.unread > 0 ? 600 : 400 }}>
            {conv.lastMsg}
          </span>
          {conv.unread > 0 && (
            <div style={{ flexShrink: 0, minWidth: '20px', height: '20px', borderRadius: '10px', background: 'linear-gradient(135deg,#C7A66A,#A07840)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 900, color: '#fff', padding: '0 5px', boxShadow: '0 4px 10px rgba(199,166,106,0.4)' }}>
              {toFa(conv.unread)}
            </div>
          )}
        </div>
  ...
  return (
    <>
      <style>{`
        ...
        .msg-input::placeholder { color:rgba(0,0,0,0.32); }
        ...
        .icon-btn:hover { background:rgba(0,0,0,0.06); color:rgba(0,0,0,0.55); }
        ...
        .send-btn {
          ...
          box-shadow:0 6px 18px rgba(199,166,106,0.3);
        }
        .send-btn:hover { transform:scale(1.05); box-shadow:0 8px 24px rgba(199,166,106,0.45); }
        .send-btn:active { transform:scale(0.95); }
        .send-btn:disabled { opacity:0.4; cursor:not-allowed; transform:none; box-shadow:none; }

        .typing-dot { width:6px; height:6px; border-radius:50%; background:rgba(0,0,0,0.42); animation:typing 1.2s ease infinite; }
      ...
      <div style={{ minHeight: '100vh', background: '#F7F7F5', display: 'flex', flexDirection: 'column' }}>
        ...
        {/* ── Top bar ── */}
        <div style={{ background: 'rgba(2,8,6,0.98)', borderBottom: '1px solid rgba(0,0,0,0.04)', padding: '0 clamp(16px,3vw,32px)', position: 'sticky', top: '62px', zIndex: 90, backdropFilter: 'blur(24px)' }}>
          <div style={{ maxWidth: '1280px', margin: '0 auto', height: '52px', display: 'flex', alignItems: 'center', gap: '14px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              <div style={{ position: 'relative' }}>
                <MessageCircle size={20} style={{ color: '#C7A66A' }} />
                {totalUnread > 0 && (
                  <div style={{ position: 'absolute', top: '-6px', left: '-6px', width: '16px', height: '16px', borderRadius: '50%', background: '#ef4444', fontSize: '10px', fontWeight: 900, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 8px rgba(239,68,68,0.5)' }}>
                    {toFa(totalUnread)}
                  </div>
                )}
              </div>
              <div>
                <div style={{ fontSize: '10px', color: 'rgba(199,166,106,0.6)', letterSpacing: '0.2em', fontWeight: 700 }}>MESSAGES</div>
                <div style={{ fontSize: '17px', fontWeight: 800, color: '#111111', letterSpacing: '-0.01em' }}>پیام‌ها</div>
              </div>
            </div>
            <div style={{ marginRight: 'auto', fontSize: '14px', color: 'rgba(0,0,0,0.35)' }}>
        ...
        <div style={{ flex: 1, maxWidth: '1280px', width: '100%', margin: '0 auto', padding: 'clamp(16px,3vw,24px)', display: 'flex', gap: '0', minHeight: 0, height: 'calc(100vh - 114px)' }}>
          <div className="messages-layout" style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '16px', width: '100%', height: '100%' }}>
            ...
            <div className="conv-list-panel"
              style={{ display: (mobileList || window?.innerWidth > 768) ? 'flex' : 'none', flexDirection: 'column', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(0,0,0,0.07)', borderRadius: '22px', overflow: 'hidden' }}>

              {/* Search */}
              <div style={{ padding: '14px 14px 10px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'rgba(0,0,0,0.04)', border: `1px solid ${searchFocus ? 'rgba(199,166,106,0.3)' : 'rgba(0,0,0,0.06)'}`, borderRadius: '12px', padding: '10px 14px', transition: 'all 0.2s' }}>
                  <Search size={14} style={{ color: 'rgba(0,0,0,0.30)', flexShrink: 0 }} />
                  <input
                    value={search} onChange={e => setSearch(e.target.value)}
                    onFocus={() => setSearchFocus(true)} onBlur={() => setSearchFocus(false)}
                    placeholder="جستجو در مکالمات..."
                    style={{ flex: 1, background: 'transparent', border: 'none', outline: 'none', color: '#111111', fontSize: '15px', fontFamily: 'inherit' }} />
                  {search && <button onClick={() => setSearch('')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(0,0,0,0.35)', padding: 0, display: 'flex' }}><X size={12} /></button>}
                </div>
              </div>
              ...
              <div style={{ padding: '0 14px 10px', display: 'flex', gap: '6px', overflowX: 'auto' }}>
                {[{l:'همه',k:'all'},{l:'مربیان',k:'coach'},{l:'باشگاه‌ها',k:'club'},{l:'فروشندگان',k:'seller'}].map((f,i) => (
                  <button key={i} style={{ padding: '5px 13px', borderRadius: '20px', fontSize: '13px', fontWeight: 600, border: '1px solid', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap', transition: 'all 0.2s', background: i === 0 ? 'rgba(199,166,106,0.1)' : 'transparent', borderColor: i === 0 ? 'rgba(199,166,106,0.3)' : 'rgba(0,0,0,0.07)', color: i === 0 ? '#C7A66A' : 'rgba(0,0,0,0.42)' }}>
                    {f.l}
                  </button>
                ))}
              </div>
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: '0 10px 10px' }}>
                ...
                {filtered.filter(c => c.pinned).length > 0 && (
                  <div style={{ padding: '8px 6px 4px', fontSize: '12px', color: 'rgba(0,0,0,0.30)', fontWeight: 700 }}>📌 پین شده</div>
                )}
                {filtered.filter(c => c.pinned).map(c => <ConvItem key={c.id} conv={c} active={activeId === c.id} onClick={() => openConv(c.id)} />)}

                {/* All */}
                {filtered.filter(c => c.pinned).length > 0 && (
                  <div style={{ padding: '10px 6px 4px', fontSize: '12px', color: 'rgba(0,0,0,0.30)', fontWeight: 700 }}>همه مکالمات</div>
                )}
                {filtered.filter(c => !c.pinned).map(c => <ConvItem key={c.id} conv={c} active={activeId === c.id} onClick={() => openConv(c.id)} />)}

                {filtered.length === 0 && (
                  <div style={{ textAlign: 'center', padding: '40px 16px', color: 'rgba(0,0,0,0.30)', fontSize: '15px' }}>
            ...
            <div className="chat-panel"
              ...
              {!activeConv ? (
                /* Empty state */
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '16px', color: 'rgba(0,0,0,0.30)', animation: 'fadeUp 0.4s ease both' }}>
                  <div style={{ width: '80px', height: '80px', borderRadius: '24px', background: 'rgba(199,166,106,0.06)', border: '1px solid rgba(199,166,106,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <MessageCircle size={36} style={{ color: 'rgba(199,166,106,0.3)' }} />
                  </div>
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: '18px', fontWeight: 700, color: 'rgba(0,0,0,0.42)', marginBottom: '6px' }}>یک مکالمه انتخاب کنید</div>
                    <div style={{ fontSize: '15px' }}>یا گفتگوی جدیدی شروع کنید</div>
                  </div>
                  <button style={{ padding: '10px 24px', background: 'linear-gradient(135deg,#C7A66A,#A07840)', border: 'none', borderRadius: '12px', color: '#fff', fontSize: '15px', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 6px 18px rgba(199,166,106,0.3)', display: 'flex', alignItems: 'center', gap: '7px' }}>
                ...
                <>
                  {/* Chat header */}
                  <div style={{ padding: '14px 20px', borderBottom: '1px solid rgba(0,0,0,0.04)', display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0, background: 'rgba(255,255,255,0.01)' }}>

                    {/* Mobile back */}
                    <button className="mobile-back-btn" onClick={() => setMobileList(true)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(0,0,0,0.45)', padding: 0, display: 'none', alignItems: 'center' }}>
                    ...
                    <div style={{ position: 'relative', flexShrink: 0 }}>
                      ...
                      {activeConv.online && <div style={{ position: 'absolute', bottom: '1px', right: '1px', width: '10px', height: '10px', borderRadius: '50%', background: '#C7A66A', border: '2px solid rgba(6,13,10,0.98)', boxShadow: '0 0 6px #C7A66A' }} />}
                    </div>

                    {/* Name */}
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '2px' }}>
                        <span style={{ fontSize: '17px', fontWeight: 800, color: '#111111', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{activeConv.name}</span>
                        {activeConv.verified && <Check size={12} style={{ color: '#C7A66A', flexShrink: 0 }} />}
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px' }}>
                        ...
                        <span style={{ color: activeConv.online ? '#C7A66A' : 'rgba(0,0,0,0.35)' }}>
                  ...
                  <div style={{ flex: 1, overflowY: 'auto', padding: '20px 20px 8px', display: 'flex', flexDirection: 'column', gap: '2px' }}>
                        ...
                        <div key={msg.id}>
                          {showDate && i === 0 && (
                            <div style={{ display: 'flex', justifyContent: 'center', margin: '8px 0 14px' }}>
                              <span style={{ fontSize: '12px', color: 'rgba(0,0,0,0.30)', background: 'rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.05)', borderRadius: '20px', padding: '4px 12px' }}>
                                امروز
                              </span>
                            </div>
                          )}
                    ...
                    {typing && (
                      <div style={{ display: 'flex', justifyContent: 'flex-end', margin: '6px 0', animation: 'fadeUp 0.25s ease both' }}>
                        <div style={{ display: 'flex', gap: '5px', padding: '12px 16px', background: 'rgba(0,0,0,0.05)', border: '1px solid rgba(0,0,0,0.06)', borderRadius: '18px 18px 4px 18px', alignItems: 'center' }}>
                          {[0,1,2].map(i => (
                  ...
                  {/* Input area */}
                  <div style={{ padding: '12px 16px', borderTop: '1px solid rgba(0,0,0,0.04)', flexShrink: 0, background: 'rgba(255,255,255,0.01)' }}>
                    ...
                    <div style={{ display: 'flex', gap: '7px', overflowX: 'auto', marginBottom: '10px', paddingBottom: '4px' }}>
                      {['👍 باشه', '🕐 بعداً پیام میدم', '✅ تأیید میکنم', '❓ توضیح بیشتر'].map((q, i) => (
                        <button key={i} onClick={() => setInput(q.replace(/^[\u{1F300}-\u{1F9FF}]\s/u,''))}
                          style={{ padding: '5px 12px', borderRadius: '20px', background: 'rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.07)', color: 'rgba(0,0,0,0.45)', fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap', transition: 'all 0.2s', flexShrink: 0 }}
                          onMouseEnter={e => { (e.currentTarget).style.background = 'rgba(199,166,106,0.08)'; (e.currentTarget).style.borderColor = 'rgba(199,166,106,0.2)'; (e.currentTarget).style.color = '#C7A66A'; }}
                          onMouseLeave={e => { (e.currentTarget).style.background = 'rgba(0,0,0,0.04)'; (e.currentTarget).style.borderColor = 'rgba(0,0,0,0.07)'; (e.currentTarget).style.color = 'rgba(0,0,0,0.45)'; }}>
                          {q}
                        </button>
                      ))}
                    </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-end', gap: '10px' }}>
                      ...
                      <div style={{ flex: 1, background: 'rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.07)', borderRadius: '16px', padding: '12px 16px', transition: 'all 0.2s', boxShadow: input ? '0 0 0 2px rgba(199,166,106,0.15)' : 'none', borderColor: input ? 'rgba(199,166,106,0.3)' : 'rgba(0,0,0,0.07)' }}>
                        <input
                          ref={inputRef}
                          className="msg-input"
                          value={input}
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', marginTop: '8px' }}>
                      <button style={{ display: 'flex', alignItems: 'center', gap: '5px', background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(0,0,0,0.30)', fontSize: '13px', fontFamily: 'inherit', transition: 'color 0.2s' }}
