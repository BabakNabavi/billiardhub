      ...
      {steps.map((s, i) => (
        <div key={s.key} style={{ display: 'flex', alignItems: 'center' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ fontSize: 11, fontWeight: i === idx ? 700 : 400, color: i <= idx ? '#A07840' : 'rgba(0,0,0,0.45)' }}>{s.label}</span>
          </div>
          {i < steps.length - 1 && (
...
export default function CheckoutPage() {
  ...
  const labelStyle = { fontSize: 12, fontWeight: 600, color: 'rgba(0,0,0,0.50)', marginBottom: 6, display: 'block' }
  const inputStyle = (err?: string) => ({
    width: '100%', boxSizing: 'border-box' as const,
    background: '#F7F7F5', border: `1px solid ${err ? 'rgba(239,68,68,0.5)' : 'rgba(0,0,0,0.08)'}`,
    borderRadius: 12, padding: '12px 14px', color: '#111111', fontSize: 14, fontFamily: 'inherit',
    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: '#F7F7F5', fontFamily: 'Vazirmatn,Tahoma,sans-serif', direction: 'rtl', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
        <div style={{ textAlign: 'center', maxWidth: 440 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 900, color: '#111111', margin: '0 0 12px' }}>سفارش ثبت شد!</h1>
          <p style={{ fontSize: 14, color: 'rgba(0,0,0,0.45)', lineHeight: 1.7, margin: '0 0 24px' }}>
            سفارش شما با موفقیت ثبت شد.<br />
            کد پیگیری: <strong style={{ color: '#C7A66A', fontSize: 16 }}>{orderNum}</strong>
          </p>
          <p style={{ fontSize: 13, color: 'rgba(0,0,0,0.38)', margin: '0 0 32px', background: 'rgba(199,166,106,0.06)', border: '1px solid rgba(199,166,106,0.12)', borderRadius: 12, padding: '12px 16px' }}>
            پس از تأیید پرداخت، کالا در سریع‌ترین زمان ممکن ارسال خواهد شد.
          </p>
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link href="/shop" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'linear-gradient(135deg,#C7A66A,#A07840)', color: '#fff', padding: '13px 24px', borderRadius: 14, textDecoration: 'none', fontWeight: 800, fontSize: 14, boxShadow: '0 8px 24px rgba(199,166,106,0.35)' }}>
              <i className="ti ti-shopping-bag" style={{ fontSize: 16 }} /> ادامه خرید
            </Link>
            <Link href="/" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(0,0,0,0.04)', border: '1px solid rgba(0,0,0,0.06)', color: 'rgba(0,0,0,0.50)', padding: '13px 24px', borderRadius: 14, textDecoration: 'none', fontSize: 14 }}>
              بازگشت به خانه
            </Link>
          </div>
        </div>
    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: '#F7F7F5', fontFamily: 'Vazirmatn,Tahoma,sans-serif', direction: 'rtl', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center' }}>
          <i className="ti ti-shopping-cart-off" style={{ fontSize: 60, color: 'rgba(0,0,0,0.35)', display: 'block', marginBottom: 16 }} />
          <p style={{ fontSize: 16, color: 'rgba(0,0,0,0.45)', marginBottom: 20 }}>سبد خرید خالی است</p>
      ...
      <div style={{ minHeight: '100vh', background: '#F7F7F5', fontFamily: 'Vazirmatn,Tahoma,sans-serif', direction: 'rtl', color: '#111111' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 1100, margin: '0 auto', padding: '32px 20px 80px' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: 32 }}>
            <Link href="/cart" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'rgba(0,0,0,0.45)', textDecoration: 'none', marginBottom: 20, transition: 'color 0.2s' }}
            ...
            <h1 style={{ fontSize: 26, fontWeight: 900, color: '#111111', margin: '0 0 8px' }}>تکمیل خرید</h1>
            <p style={{ fontSize: 13, color: 'rgba(0,0,0,0.45)', margin: 0 }}>{toFa(count)} محصول — مجموع: {fmt(grand)} تومان</p>
          </div>
              ...
              <div style={{ padding: '28px' }}>
                  ...
                  <div style={{ animation: 'fadeUp 0.35s ease both' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
                      <div style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(199,166,106,0.10)', border: '1px solid rgba(199,166,106,0.20)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <i className="ti ti-map-pin" style={{ fontSize: 17, color: '#C7A66A' }} />
                      </div>
                      <h2 style={{ fontSize: 16, fontWeight: 800, color: '#111111', margin: 0 }}>اطلاعات تحویل</h2>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                      ...
                      <div>
                        ...
                        {errors.province && <p style={{ fontSize: 11, color: '#ef4444', marginTop: 4 }}>{errors.province}</p>}
                      </div>
                      ...
                      <div style={{ gridColumn: 'span 2' }}>
                        ...
                        {errors.postalCode && <p style={{ fontSize: 11, color: '#ef4444', marginTop: 4 }}>{errors.postalCode}</p>}
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      style={{ width: '100%', marginTop: 24, padding: '15px', borderRadius: 14, border: 'none', background: 'linear-gradient(135deg,#C7A66A,#A07840)', color: '#fff', fontSize: 15, fontWeight: 800, fontFamily: 'inherit', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: '0 8px 28px rgba(199,166,106,0.35)' }}
                    >
                      ادامه — انتخاب روش پرداخت <i className="ti ti-arrow-left" style={{ fontSize: 16 }} />
                    </button>
                  </div>
                )}
                ...
                {step === 'payment' && (
                  <div style={{ animation: 'fadeUp 0.35s ease both' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
                      <div style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(199,166,106,0.10)', border: '1px solid rgba(199,166,106,0.20)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <i className="ti ti-credit-card" style={{ fontSize: 17, color: '#C7A66A' }} />
                      </div>
                      <h2 style={{ fontSize: 16, fontWeight: 800, color: '#111111', margin: 0 }}>روش پرداخت</h2>
                    </div>
                    ...
                    {[
                      ...
                      >
                        <div style={{ width: 40, height: 40, borderRadius: 10, background: form.payMethod === pm.key ? 'rgba(199,166,106,0.12)' : 'rgba(0,0,0,0.04)', border: `1px solid ${form.payMethod === pm.key ? 'rgba(199,166,106,0.30)' : 'rgba(0,0,0,0.07)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <i className={`ti ${pm.icon}`} style={{ fontSize: 18, color: form.payMethod === pm.key ? '#C7A66A' : 'rgba(0,0,0,0.45)' }} />
                        </div>
                        <div style={{ flex: 1 }}>
                          <p style={{ fontSize: 14, fontWeight: 700, color: form.payMethod === pm.key ? '#111111' : 'rgba(0,0,0,0.50)', margin: '0 0 3px' }}>{pm.label}</p>
                          <p style={{ fontSize: 12, color: 'rgba(0,0,0,0.38)', margin: 0 }}>{pm.desc}</p>
                        </div>
                        <div style={{ width: 18, height: 18, borderRadius: '50%', border: `2px solid ${form.payMethod === pm.key ? '#C7A66A' : 'rgba(0,0,0,0.12)'}`, background: form.payMethod === pm.key ? '#C7A66A' : 'none', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          {form.payMethod === pm.key && <i className="ti ti-check" style={{ fontSize: 10, color: '#fff' }} />}
                        </div>
                      </div>
                    ))}
                    ...
                    {form.payMethod === 'card' && (
                      <div style={{ marginTop: 16, padding: '16px', background: 'rgba(0,0,0,0.02)', border: '1px solid rgba(0,0,0,0.05)', borderRadius: 14 }}>
                        <p style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)', margin: '0 0 14px', display: 'flex', alignItems: 'center', gap: 6 }}>
                          <i className="ti ti-info-circle" style={{ fontSize: 14, color: '#C7A66A' }} />
                    ...
                    <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
                      <button onClick={() => setStep('address')} style={{ padding: '14px 20px', borderRadius: 14, border: '1px solid rgba(0,0,0,0.06)', background: '#FFFFFF', color: 'rgba(0,0,0,0.50)', fontSize: 14, fontFamily: 'inherit', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                        <i className="ti ti-arrow-right" style={{ fontSize: 15 }} /> برگشت
                      </button>
                      <button
                        className="next-btn"
                        onClick={() => setStep('confirm')}
                        style={{ flex: 1, padding: '14px', borderRadius: 14, border: 'none', background: 'linear-gradient(135deg,#C7A66A,#A07840)', color: '#fff', fontSize: 15, fontWeight: 800, fontFamily: 'inherit', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: '0 8px 28px rgba(199,166,106,0.35)' }}
                      >
                        بررسی نهایی سفارش <i className="ti ti-arrow-left" style={{ fontSize: 16 }} />
                      </button>
                    </div>
                  </div>
                )}
                ...
                {step === 'confirm' && (
                  <div style={{ animation: 'fadeUp 0.35s ease both' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24 }}>
                      <div style={{ width: 36, height: 36, borderRadius: 10, background: 'rgba(199,166,106,0.10)', border: '1px solid rgba(199,166,106,0.20)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <i className="ti ti-clipboard-check" style={{ fontSize: 17, color: '#C7A66A' }} />
                      </div>
                      <h2 style={{ fontSize: 16, fontWeight: 800, color: '#111111', margin: 0 }}>بررسی نهایی</h2>
                    </div>
                    ...
                    <div style={{ background: 'rgba(0,0,0,0.02)', border: '1px solid rgba(0,0,0,0.05)', borderRadius: 14, padding: '16px 20px', marginBottom: 14 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                        <p style={{ fontSize: 12, fontWeight: 700, color: 'rgba(0,0,0,0.45)', margin: 0, display: 'flex', alignItems: 'center', gap: 5 }}>
                          <i className="ti ti-map-pin" style={{ fontSize: 13, color: '#C7A66A' }} /> آدرس تحویل
                        </p>
                        <button onClick={() => setStep('address')} style={{ fontSize: 11, color: '#C7A66A', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>ویرایش</button>
                      </div>
                      <p style={{ fontSize: 14, fontWeight: 700, color: '#111111', margin: '0 0 4px' }}>{form.firstName} {form.lastName}</p>
                      <p style={{ fontSize: 13, color: 'rgba(0,0,0,0.50)', margin: '0 0 2px' }}>{form.province} — {form.city}</p>
                      <p style={{ fontSize: 13, color: 'rgba(0,0,0,0.50)', margin: '0 0 2px' }}>{form.address}</p>
                      <p style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)', margin: 0 }}>کد پستی: {toFa(form.postalCode)}</p>
                    </div>
                    ...
                    <div style={{ background: 'rgba(0,0,0,0.02)', border: '1px solid rgba(0,0,0,0.05)', borderRadius: 14, padding: '16px 20px', marginBottom: 24 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                        <p style={{ fontSize: 12, fontWeight: 700, color: 'rgba(0,0,0,0.45)', margin: 0, display: 'flex', alignItems: 'center', gap: 5 }}>
                          <i className="ti ti-credit-card" style={{ fontSize: 13, color: '#C7A66A' }} /> روش پرداخت
                        </p>
                        <button onClick={() => setStep('payment')} style={{ fontSize: 11, color: '#C7A66A', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>ویرایش</button>
                      </div>
                      <p style={{ fontSize: 14, color: '#111111', margin: 0 }}>
                        ...
                        return (
                          <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: 'rgba(0,0,0,0.02)', border: '1px solid rgba(0,0,0,0.04)', borderRadius: 12 }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <p style={{ fontSize: 13, fontWeight: 600, color: '#111111', margin: '0 0 2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.title}</p>
                              <p style={{ fontSize: 11, color: 'rgba(0,0,0,0.45)', margin: 0 }}>{toFa(item.quantity)} عدد</p>
                            </div>
                            <p style={{ fontSize: 14, fontWeight: 700, color: '#C7A66A', margin: 0, flexShrink: 0 }}>{fmt(fp * item.quantity)} ت</p>
                          </div>
                        )
                      })}
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                      <button onClick={() => setStep('payment')} style={{ padding: '14px 20px', borderRadius: 14, border: '1px solid rgba(0,0,0,0.06)', background: '#FFFFFF', color: 'rgba(0,0,0,0.50)', fontSize: 14, fontFamily: 'inherit', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
                        <i className="ti ti-arrow-right" style={{ fontSize: 15 }} /> برگشت
                      </button>
                      <button
                        onClick={placeOrder}
                        style={{ flex: 1, padding: '15px', borderRadius: 14, border: 'none', background: 'linear-gradient(135deg,#C7A66A,#A07840)', color: '#fff', fontSize: 15, fontWeight: 900, fontFamily: 'inherit', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: '0 8px 28px rgba(199,166,106,0.40)' }}
                      >
            ...
            <div style={{ height: 'fit-content', position: 'sticky', top: 80 }}>
              <div style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.07)', borderRadius: 24, overflow: 'hidden' }}>
                ...
                <div style={{ padding: '20px' }}>
                  <h3 style={{ fontSize: 14, fontWeight: 800, color: '#111111', margin: '0 0 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <i className="ti ti-receipt" style={{ fontSize: 16, color: '#C7A66A' }} />
                    خلاصه سفارش
                  </h3>
                    ...
                    {items.slice(0, 4).map(item => (
                      <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <img src={item.image || '/images/billiadr-club-3.jpg'} alt="" style={{ width: 36, height: 36, borderRadius: 8, objectFit: 'cover', flexShrink: 0 }}
                          onError={e => { (e.target as HTMLImageElement).src = '/images/billiadr-club-3.jpg' }} />
                        <p style={{ fontSize: 12, color: 'rgba(0,0,0,0.50)', margin: 0, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.title}</p>
                        <span style={{ fontSize: 12, color: 'rgba(0,0,0,0.45)', flexShrink: 0 }}>×{toFa(item.quantity)}</span>
                      </div>
                    ))}
                    {items.length > 4 && <p style={{ fontSize: 11, color: 'rgba(0,0,0,0.38)', margin: 0 }}>و {toFa(items.length - 4)} محصول دیگر...</p>}
                  </div>
                  ...
                  {[
                    ...
                    <div key={r.label} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginBottom: 10 }}>
                      <span style={{ color: 'rgba(0,0,0,0.45)' }}>{r.label}</span>
                      <span style={{ color: r.green ? '#16a34a' : '#111111', fontWeight: 600 }}>{r.value}</span>
                    </div>
                  ))}

                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 15, fontWeight: 700, paddingTop: 12, borderTop: '1px solid rgba(0,0,0,0.04)' }}>
              ...
              <div style={{ marginTop: 16, padding: '16px', background: 'rgba(199,166,106,0.04)', border: '1px solid rgba(199,166,106,0.10)', borderRadius: 14, textAlign: 'center' }}>
                <i className="ti ti-shield-check" style={{ fontSize: 24, color: '#C7A66A', display: 'block', marginBottom: 8 }} />
                <p style={{ fontSize: 12, fontWeight: 700, color: '#111111', margin: '0 0 4px' }}>خرید ۱۰۰٪ امن</p>
                <p style={{ fontSize: 11, color: 'rgba(0,0,0,0.38)', margin: 0 }}>اطلاعات شما با رمزنگاری SSL محافظت می‌شود</p>
              </div>
            </div>
          </div>
        </div>
