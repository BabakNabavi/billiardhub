    ...
    <div style={{ background: isLive ? 'rgba(239,68,68,0.05)' : '#FFFFFF', border: `1px solid ${isLive ? 'rgba(239,68,68,0.2)' : 'rgba(0,0,0,0.07)'}`, borderRadius: '16px', padding: '16px 18px', transition: 'all 0.3s', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          {isCompleted && <span style={{ fontSize: '11px', color: '#C7A66A', fontWeight: 700, letterSpacing: '0.1em' }}>FINAL</span>}
          {match.status === 'upcoming' && <span style={{ fontSize: '11px', color: 'rgba(0,0,0,0.40)', letterSpacing: '0.08em' }}>UPCOMING</span>}
        </div>
        <div style={{ display: 'flex', gap: '8px', fontSize: '12px', color: 'rgba(0,0,0,0.35)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '10px', opacity: isCompleted && winner === 2 ? 0.45 : 1 }}>
          <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: 'linear-gradient(135deg,#C7A66A,#A07840)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '15px', fontWeight: 900, color: '#fff', flexShrink: 0, boxShadow: isCompleted && winner === 1 ? '0 0 14px rgba(199,166,106,0.4)' : 'none' }}>
            {match.player1.avatar}
          </div>
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: '14px', fontWeight: isCompleted && winner === 1 ? 800 : 600, color: isCompleted && winner === 1 ? '#f0faf5' : 'rgba(0,0,0,0.48)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {match.player1.name.split(' ').slice(-1)[0]}
            </div>
            <div style={{ fontSize: '11px', color: 'rgba(0,0,0,0.35)' }}>#{toFa(match.player1.rank)}</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
          ...
          <div style={{ fontSize: '14px', color: 'rgba(0,0,0,0.30)', fontWeight: 700 }}>:</div>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '10px', justifyContent: 'flex-end', opacity: isCompleted && winner === 1 ? 0.45 : 1 }}>
          <div style={{ minWidth: 0, textAlign: 'right' }}>
            <div style={{ fontSize: '14px', fontWeight: isCompleted && winner === 2 ? 800 : 600, color: isCompleted && winner === 2 ? '#f0faf5' : 'rgba(0,0,0,0.48)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {match.player2.name.split(' ').slice(-1)[0]}
            </div>
            <div style={{ fontSize: '11px', color: 'rgba(0,0,0,0.35)' }}>#{toFa(match.player2.rank)}</div>
          </div>
          <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: 'linear-gradient(135deg,#06b6d4,#0891b2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '15px', fontWeight: 900, color: '#fff', flexShrink: 0, boxShadow: isCompleted && winner === 2 ? '0 0 14px rgba(6,182,212,0.4)' : 'none' }}>
    ...
    <div style={{ background: live ? 'rgba(239,68,68,0.06)' : '#FFFFFF', border: `1px solid ${live ? 'rgba(239,68,68,0.2)' : 'rgba(0,0,0,0.07)'}`, borderRadius: '12px', overflow: 'hidden', width: '200px', position: 'relative' }}>
      ...
      {[{ name: p1, score: s1, w: winner === 1 }, { name: p2, score: s2, w: winner === 2 }].map((p, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 12px', background: i === 0 ? 'transparent' : 'rgba(255,255,255,0.02)', borderTop: i === 1 ? '1px solid rgba(0,0,0,0.04)' : 'none' }}>
          <span style={{ fontSize: '13px', fontWeight: p.w ? 800 : 500, color: p.w ? '#f0faf5' : 'rgba(0,0,0,0.45)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '130px' }}>
            {p.name.split(' ').slice(-1)[0]}
          </span>
          <span style={{ fontSize: '15px', fontWeight: 900, color: p.w ? '#C7A66A' : done ? 'rgba(0,0,0,0.30)' : 'rgba(0,0,0,0.42)', flexShrink: 0, marginRight: '8px', textShadow: p.w ? '0 0 12px rgba(199,166,106,0.5)' : 'none' }}>
      ...
      <div style={{ minHeight: '100vh', background: '#F7F7F5' }}>
        ...
        <div style={{ position: 'relative', height: 'clamp(500px,68vh,740px)', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'absolute', top: '24px', left: 0, right: 0, zIndex: 10, padding: '0 clamp(16px,4vw,48px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <Link href="/events" style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'rgba(255,255,255,0.5)', fontSize: '13px', textDecoration: 'none', background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(16px)', border: '1px solid rgba(0,0,0,0.06)', borderRadius: '10px', padding: '7px 14px' }}>
          ...
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 10, padding: 'clamp(24px,4vw,52px)', opacity: heroOpacity }}>
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: t.status === 'live' ? 'rgba(239,68,68,0.12)' : t.status === 'completed' ? 'rgba(0,0,0,0.05)' : 'rgba(199,166,106,0.1)', border: `1px solid ${t.status === 'live' ? 'rgba(239,68,68,0.3)' : t.status === 'completed' ? 'rgba(0,0,0,0.08)' : 'rgba(199,166,106,0.25)'}`, borderRadius: '100px', padding: '6px 18px', marginBottom: '18px', backdropFilter: 'blur(16px)', animation: t.status === 'live' ? 'liveGlow 3s infinite' : 'none' }}>
              ...
              <span style={{ fontSize: '11px', color: t.status === 'live' ? '#fca5a5' : t.status === 'completed' ? '#94a3b8' : '#6ee7b7', fontWeight: 700, letterSpacing: '0.15em' }}>
                {t.status === 'live' ? '● LIVE NOW' : t.status === 'completed' ? 'COMPLETED' : 'UPCOMING'}
              </span>
            </div>

            <div style={{ fontSize: '11px', color: 'rgba(245,158,11,0.7)', letterSpacing: '0.25em', fontWeight: 700, marginBottom: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '5px', background: 'rgba(0,0,0,0.06)', backdropFilter: 'blur(12px)', borderRadius: '20px', padding: '6px 14px', fontSize: '13px', color: 'rgba(255,255,255,0.8)' }}>
                <MapPin size={11} style={{ color: '#C7A66A' }} /> {t.venue}، {t.city}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '5px', background: 'rgba(0,0,0,0.06)', backdropFilter: 'blur(12px)', borderRadius: '20px', padding: '6px 14px', fontSize: '13px', color: 'rgba(255,255,255,0.8)' }}>
                <Calendar size={11} style={{ color: '#C7A66A' }} /> {t.startDate} — {t.endDate}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '5px', background: 'rgba(245,158,11,0.15)', backdropFilter: 'blur(12px)', borderRadius: '20px', padding: '6px 14px', fontSize: '13px', color: '#fcd34d' }}>
        ...
        {t.status === 'live' && liveMatches.length > 0 && (
          <div style={{ background: 'rgba(239,68,68,0.06)', borderBottom: '1px solid rgba(239,68,68,0.15)', padding: '12px clamp(16px,4vw,40px)', display: 'flex', alignItems: 'center', gap: '20px', overflowX: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
              ...
              <span style={{ fontSize: '12px', color: '#ef4444', fontWeight: 700, letterSpacing: '0.12em' }}>LIVE</span>
            </div>
            {liveMatches.map((m, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '6px 14px', background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.15)', borderRadius: '100px', flexShrink: 0 }}>
                <span style={{ fontSize: '13px', color: 'rgba(0,0,0,0.48)', fontWeight: 600 }}>{m.player1.name.split(' ').slice(-1)[0]}</span>
                <span style={{ fontSize: '15px', fontWeight: 900, color: '#ef4444' }}>{toFa(m.score1)}:{toFa(m.score2)}</span>
                <span style={{ fontSize: '13px', color: 'rgba(0,0,0,0.48)', fontWeight: 600 }}>{m.player2.name.split(' ').slice(-1)[0]}</span>
                <span style={{ fontSize: '11px', color: 'rgba(0,0,0,0.35)' }}>{m.round}</span>
              </div>
            ))}
          </div>
        )}
          ...
          <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', borderBottom: 'none' }}>
              {[
                ...
                <div key={i} style={{ padding: '18px 16px', textAlign: 'center', borderLeft: i > 0 ? '1px solid rgba(0,0,0,0.04)' : 'none' }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'rgba(0,0,0,0.35)', marginTop: '4px' }}>{s.l}</div>
                </div>
              ))}
            </div>
            {/* Progress */}
            <div style={{ padding: '12px 0 14px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '7px' }}>
                <span style={{ fontSize: '12px', color: 'rgba(0,0,0,0.40)' }}>پیشرفت مسابقات</span>
                <span style={{ fontSize: '12px', color: '#C7A66A', fontWeight: 700 }}>{toFa(progress)}٪</span>
              </div>
            ...
            <div>
                ...
                <div style={{ animation: 'fadeUp 0.4s ease both' }}>
                      ...
                      <div style={{ background: 'rgba(239,68,68,0.04)', border: '1px solid rgba(239,68,68,0.15)', borderRadius: '20px', padding: '24px', marginBottom: '20px', position: 'relative', overflow: 'hidden' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '18px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                            <span style={{ width: '7px', height: '7px', borderRadius: '50%', background: '#ef4444', boxShadow: '0 0 10px #ef4444', display: 'inline-block', animation: 'pulse 1.5s infinite' }} />
                            <span style={{ fontSize: '13px', color: '#ef4444', fontWeight: 700, letterSpacing: '0.1em' }}>در حال بازی</span>
                          </div>
                        </div>
                  ...
                  <ScrollReveal>
                    <div style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.07)', borderRadius: '20px', padding: '26px', marginBottom: '20px' }}>
                      ...
                      <p style={{ fontSize: '15px', color: 'rgba(0,0,0,0.45)', lineHeight: 1.9, margin: '0 0 20px' }}>{t.description}</p>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: '10px' }}>
                        {[
                          ...
                          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 14px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(0,0,0,0.04)', borderRadius: '12px' }}>
                            ...
                            <div style={{ minWidth: 0 }}>
                              <div style={{ fontSize: '11px', color: 'rgba(0,0,0,0.35)', marginBottom: '2px' }}>{r.label}</div>
                              <div style={{ fontSize: '14px', fontWeight: 600, color: '#111111', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.value}</div>
                            </div>
                          </div>
                        ))}
                      </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                        {players.slice(0, 5).map((p, i) => (
                          <div key={i} className="player-row">
                            <div style={{ width: '28px', textAlign: 'center', fontSize: '15px', fontWeight: 800, color: i === 0 ? '#f59e0b' : i === 1 ? '#94a3b8' : i === 2 ? '#cd7c4a' : 'rgba(0,0,0,0.30)', flexShrink: 0 }}>
                              {i < 3 ? ['🥇','🥈','🥉'][i] : toFa(i + 1)}
                            </div>
                            <div style={{ width: '38px', height: '38px', borderRadius: '11px', background: `linear-gradient(135deg, ${['#C7A66A','#06b6d4','#a78bfa','#f59e0b','#ef4444'][i % 5]}, ${['#059669','#0891b2','#7c3aed','#d97706','#dc2626'][i % 5]})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '15px', fontWeight: 900, color: '#fff', flexShrink: 0 }}>
                              {p.avatar}
                            </div>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ fontSize: '14px', fontWeight: 700, color: '#111111', marginBottom: '3px' }}>{p.name}</div>
                              <div style={{ fontSize: '12px', color: 'rgba(0,0,0,0.40)' }}>{p.city} · رنک #{toFa(p.rank)}</div>
                            </div>
                            <div style={{ textAlign: 'left', flexShrink: 0 }}>
                              <div style={{ fontSize: '14px', fontWeight: 700, color: '#f59e0b' }}>{toFa(p.points.toLocaleString())}</div>
                              <div style={{ fontSize: '11px', color: 'rgba(0,0,0,0.35)' }}>امتیاز</div>
                            </div>
                          </div>
                        ))}
                      </div>
              ...
              {tab === 'matches' && (
                <div style={{ animation: 'fadeUp 0.4s ease both', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {['live','upcoming','completed'].map(status => {
                    ...
                    return (
                      <div key={status}>
                        <div style={{ fontSize: '12px', color: status === 'live' ? '#ef4444' : status === 'upcoming' ? '#06b6d4' : '#C7A66A', fontWeight: 700, marginBottom: '10px', paddingRight: '4px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '32px', alignItems: 'center', minWidth: 'max-content', paddingBottom: '8px' }}>
                      {bracket.rounds.map((round, ri) => (
                        <div key={ri} style={{ display: 'flex', flexDirection: 'column', gap: `${Math.pow(2, ri + 1) * 8}px`, justifyContent: 'center' }}>
                          <div style={{ fontSize: '11px', color: 'rgba(0,0,0,0.40)', fontWeight: 700, letterSpacing: '0.15em', textAlign: 'center', marginBottom: '12px', padding: '5px 12px', background: 'rgba(0,0,0,0.03)', borderRadius: '20px', border: '1px solid rgba(0,0,0,0.05)' }}>
                            {round.name}
                          </div>
                    ...
                    <ScrollReveal key={gi} delay={gi * 0.1}>
                      <div style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.07)', borderRadius: '20px', overflow: 'hidden' }}>
                        ...
                        <div style={{ padding: '16px 20px', background: 'rgba(255,255,255,0.02)', borderBottom: '1px solid rgba(0,0,0,0.04)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                          <span style={{ fontSize: '15px', fontWeight: 800, color: '#111111', letterSpacing: '-0.01em' }}>{g.name}</span>
                          <div style={{ display: 'flex', gap: '16px', fontSize: '11px', color: 'rgba(0,0,0,0.35)', fontWeight: 700, letterSpacing: '0.08em' }}>
                        ...
                        <div>
                          {g.players.map((r, i) => (
                            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '13px 20px', borderBottom: i < g.players.length - 1 ? '1px solid rgba(0,0,0,0.04)' : 'none', background: i < 2 ? 'rgba(199,166,106,0.02)' : 'transparent' }}>
                              <span style={{ fontSize: '13px', fontWeight: 700, color: i < 2 ? '#C7A66A' : 'rgba(0,0,0,0.35)', width: '16px', flexShrink: 0 }}>{toFa(i + 1)}</span>
                              <div style={{ width: '30px', height: '30px', borderRadius: '8px', background: `linear-gradient(135deg, ${['#C7A66A','#06b6d4','#a78bfa','#f59e0b'][i]},${['#059669','#0891b2','#7c3aed','#d97706'][i]})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: 900, color: '#fff', flexShrink: 0 }}>
                                {r.player.avatar}
                              </div>
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ fontSize: '14px', fontWeight: 600, color: '#111111', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.player.name.split(' ').slice(-1)[0]}</div>
                                {i < 2 && <div style={{ fontSize: '10px', color: '#C7A66A', fontWeight: 700 }}>صعود کرد</div>}
                              </div>
                              <div style={{ display: 'flex', gap: '16px', fontSize: '14px', flexShrink: 0 }}>
              ...
              {tab === 'players' && (
                <div style={{ animation: 'fadeUp 0.4s ease both' }}>
                  <div style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.07)', borderRadius: '20px', overflow: 'hidden' }}>
                    {/* Table header */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 20px', background: 'rgba(255,255,255,0.02)', borderBottom: '1px solid rgba(0,0,0,0.04)', fontSize: '11px', color: 'rgba(0,0,0,0.40)', fontWeight: 700 }}>
                    ...
                    {players.map((p, i) => (
                      <div key={i} className="player-row" style={{ borderBottom: i < players.length - 1 ? '1px solid rgba(0,0,0,0.04)' : 'none', borderRadius: 0 }}>
                        <span style={{ width: '28px', fontSize: '14px', fontWeight: 800, color: i === 0 ? '#f59e0b' : i === 1 ? '#94a3b8' : i === 2 ? '#cd7c4a' : 'rgba(0,0,0,0.30)', flexShrink: 0 }}>
                          {i < 3 ? ['🥇','🥈','🥉'][i] : toFa(i + 1)}
                        </span>
                        <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: `linear-gradient(135deg, ${['#C7A66A','#06b6d4','#a78bfa','#f59e0b','#ef4444','#8b5cf6','#ec4899','#14b8a6'][i % 8]}, ${['#059669','#0891b2','#7c3aed','#d97706','#dc2626','#7c3aed','#db2777','#0d9488'][i % 8]})`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '15px', fontWeight: 900, color: '#fff', flexShrink: 0 }}>
                          {p.avatar}
                        </div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: '14px', fontWeight: 700, color: '#111111', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.name}</div>
                        </div>
                        <span style={{ width: '60px', fontSize: '13px', color: 'rgba(0,0,0,0.42)', flexShrink: 0 }}>{p.city}</span>
                        <span style={{ width: '50px', fontSize: '13px', color: 'rgba(0,0,0,0.42)', flexShrink: 0 }}>#{toFa(p.rank)}</span>
                        <span style={{ width: '70px', fontSize: '14px', fontWeight: 700, color: '#f59e0b', flexShrink: 0 }}>{toFa(p.points.toLocaleString())}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div style={{ position: 'sticky', top: '80px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ background: '#FFFFFF', border: '1px solid rgba(245,158,11,0.2)', borderRadius: '22px', overflow: 'hidden', position: 'relative' }}>
                ...
                <div style={{ padding: '22px 22px 14px' }}>
                  <div style={{ fontSize: '11px', color: 'rgba(245,158,11,0.6)', letterSpacing: '0.2em', fontWeight: 700, marginBottom: '8px', textAlign: 'center' }}>PRIZE POOL</div>
                  ...
                  <div style={{ textAlign: 'center', fontSize: '13px', color: 'rgba(0,0,0,0.35)', marginBottom: '18px' }}>تومان</div>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1px' }}>
                  {t.prizes.map((p, i) => (
                    <div key={i} className="prize-card" style={{ borderRadius: 0, border: 'none', borderTop: '1px solid rgba(0,0,0,0.04)' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: '13px', color: p.color, fontWeight: 700 }}>{p.place}</div>
                      </div>
                      <div style={{ fontSize: '15px', fontWeight: 900, color: p.color, letterSpacing: '-0.01em' }}>{p.amount}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.07)', borderRadius: '20px', padding: '20px' }}>
                <div style={{ fontSize: '14px', fontWeight: 700, color: '#111111', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {[
                    ...
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 12px', borderRadius: '10px', transition: 'background 0.2s' }}>
                      <span style={{ flexShrink: 0 }}>{r.icon}</span>
                      <span style={{ fontSize: '13px', color: 'rgba(0,0,0,0.42)', flex: 1 }}>{r.label}</span>
              ...
              <div style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.07)', borderRadius: '20px', padding: '20px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                  <span style={{ fontSize: '13px', color: 'rgba(0,0,0,0.42)' }}>ثبت‌نام‌شدگان</span>
                  <span style={{ fontSize: '14px', fontWeight: 700, color: '#111111' }}>{toFa(t.participants)}/{toFa(t.maxParticipants)}</span>
                </div>
                ...
                <div style={{ textAlign: 'center', padding: '12px', background: t.registrationOpen ? 'rgba(199,166,106,0.06)' : 'rgba(0,0,0,0.03)', border: `1px solid ${t.registrationOpen ? 'rgba(199,166,106,0.2)' : 'rgba(0,0,0,0.05)'}`, borderRadius: '12px', fontSize: '13px', color: t.registrationOpen ? '#C7A66A' : 'rgba(0,0,0,0.35)', fontWeight: 600 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {[
                    ...
                    <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 12px', borderRadius: '10px' }}>
                      <span style={{ fontSize: '13px', color: 'rgba(0,0,0,0.42)' }}>{s.label}</span>
                      <span style={{ fontSize: '15px', fontWeight: 800, color: s.c, letterSpacing: '-0.01em' }}>{s.v}</span>
                    </div>
                  ))}
                </div>
              </div>
