...
const EVENTS = [
  {
    ...
    statusColor: GOLD, startDate: '۱ فروردین ۱۴۰۴', endDate: '۳۰ خرداد ۱۴۰۴',
...
const statusStyle: Record<string, { bg: string; text: string }> = {
  'در جریان': { bg: `rgba(199,166,106,0.15)`, text: GOLD },
...
export default function EventsPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background: '#F7F7F5', color: '#1C1C1A', fontFamily: 'Vazirmatn, sans-serif' }} dir="rtl">
      {/* Dark hero — intentionally kept dark */}
      <div className="relative overflow-hidden" style={{ background: '#F7F7F5' }}>
        <div className="absolute inset-0" style={{
          backgroundImage: `radial-gradient(ellipse at 20% 0%, rgba(199,166,106,0.12) 0%, transparent 50%), radial-gradient(ellipse at 80% 50%, rgba(199,166,106,0.08) 0%, transparent 50%)`,
        }} />
        <div className="absolute inset-0 opacity-5" style={{
          backgroundImage: `repeating-linear-gradient(0deg, transparent, transparent 80px, rgba(199,166,106,0.4) 80px, rgba(199,166,106,0.4) 81px)`,
        }} />
        <div className="relative max-w-6xl mx-auto px-4 pt-14 pb-10">
          ...
          <h1 className="text-5xl sm:text-6xl font-black text-center mb-2 leading-none" style={{
            background: `linear-gradient(135deg,#f0faf5 30%,${GOLD})`,
            WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent',
          }}>
            مسابقات بیلیارد
          </h1>
          <p className="text-center mb-10" style={{ color: 'rgba(0,0,0,0.45)' }}>تورنمنت‌ها، لیگ‌ها و رویدادهای رسمی کشور</p>
          <div className="grid grid-cols-3 gap-4 max-w-md mx-auto">
            {[
              { label: 'در جریان', value: live.length, color: GOLD },
              ...
              <div key={s.label} className="text-center rounded-2xl p-4" style={{ background: 'rgba(0,0,0,0.05)', border: `1px solid ${s.color}22` }}>
      ...
      <div className="sticky top-0 z-40" style={{ background: 'rgba(247,247,245,0.92)', backdropFilter: 'blur(20px)', borderBottom: `1px solid rgba(28,28,26,0.08)` }}>
        <div className="max-w-6xl mx-auto px-4 py-3 flex flex-col sm:flex-row gap-3">
          <div className="flex items-center gap-2 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
            <span className="text-xs flex-shrink-0" style={{ color: 'rgba(28,28,26,0.48)' }}>رشته:</span>
            {DISCIPLINES.map(d => (
              <button key={d} onClick={() => setDiscipline(d)}
                className="flex-shrink-0 px-3 py-1 rounded-full text-xs font-medium transition-all"
                style={discipline === d ? { background: GOLD, color: '#F7F7F5' } : { background: `rgba(199,166,106,0.08)`, color: 'rgba(28,28,26,0.48)', border: `1px solid rgba(199,166,106,0.2)` }}>
                {d}
              </button>
            ))}
          </div>
          <div className="flex items-center gap-2 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
            <span className="text-xs flex-shrink-0" style={{ color: 'rgba(28,28,26,0.48)' }}>وضعیت:</span>
            {STATUSES.map(s => (
              <button key={s} onClick={() => setStatusFilter(s)}
                className="flex-shrink-0 px-3 py-1 rounded-full text-xs font-medium transition-all"
                style={statusFilter === s ? { background: GOLD, color: '#F7F7F5' } : { background: `rgba(199,166,106,0.08)`, color: 'rgba(28,28,26,0.48)', border: `1px solid rgba(199,166,106,0.15)` }}>
                {s}
              </button>
            ))}
          </div>
      ...
      {live.length > 0 && statusFilter === 'همه' && discipline === 'همه' && (
        <div className="max-w-6xl mx-auto px-4 mt-8">
          <div className="rounded-2xl p-1 mb-2" style={{ background: `linear-gradient(135deg,${GOLD},#A07840)` }}>
            <div className="rounded-xl px-5 py-4 flex items-center gap-4" style={{ background: 'rgba(255,255,255,0.78)', backdropFilter: 'blur(20px) saturate(1.5)' }}>
              <div className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full animate-pulse" style={{ background: GOLD }} />
                <span className="text-sm font-bold" style={{ color: GOLD }}>LIVE</span>
              </div>
              <span className="text-sm" style={{ color: 'rgba(28,28,26,0.6)' }}>{live[0]?.title} — هم‌اکنون در حال برگزاری</span>
              <Link href={`/events/${live[0]?.id}`} className="mr-auto text-xs px-3 py-1 rounded-full font-bold"
                style={{ background: `rgba(199,166,106,0.15)`, color: GOLD }}>
                مشاهده زنده ←
              </Link>
            </div>
          </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        {filtered.length === 0 ? (
          <div className="text-center py-20" style={{ color: 'rgba(28,28,26,0.48)' }}>
            ...
            {filtered.map(event => {
              ...
              return (
                <Link href={`/events/${event.id}`} key={event.id}
                  ...
                  style={{ background: 'rgba(255,255,255,0.78)', backdropFilter: 'blur(20px) saturate(1.5)', border: '1px solid rgba(28,28,26,0.08)' }}>
                  <div className="h-1 w-full" style={{ background: event.status === 'در جریان' ? `linear-gradient(90deg,${GOLD},#A07840)` : event.status === 'ثبت‌نام باز' ? 'linear-gradient(90deg,#f59e0b,#ef4444)' : 'rgba(107,114,128,0.3)' }} />
                  <div className="p-6 flex-1 flex flex-col">
                    ...
                    <h3 className="font-black text-lg mb-1 leading-snug transition-colors" style={{ color: '#1C1C1A' }}>{event.title}</h3>
                    <p className="text-xs mb-4 line-clamp-2" style={{ color: 'rgba(28,28,26,0.48)' }}>{event.description}</p>
                    <div className="space-y-1.5 text-xs mb-4" style={{ color: 'rgba(28,28,26,0.6)' }}>
                      ...
                      <div className="flex items-center gap-2"><span>💰</span><span className="font-bold" style={{ color: GOLD }}>{event.prize}</span></div>
                    </div>
                    {event.status !== 'پایان یافته' && (
                      <div className="mb-4">
                        <div className="flex items-center justify-between text-xs mb-1" style={{ color: 'rgba(28,28,26,0.48)' }}>
                          <span>شرکت‌کنندگان</span><span>{event.participants} / {event.maxParticipants}</span>
                        </div>
                        <div className="h-1.5 rounded-full overflow-hidden" style={{ background: `rgba(199,166,106,0.12)` }}>
                          <div className="h-full rounded-full" style={{ width: `${fillPct}%`, background: fillPct >= 90 ? '#ef4444' : fillPct >= 60 ? '#f59e0b' : GOLD }} />
                        </div>
                      </div>
                    )}
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {event.highlights.map((h, i) => (
                        <span key={i} className="text-xs px-2 py-0.5 rounded-full" style={{ background: `rgba(199,166,106,0.08)`, color: GOLD, border: `1px solid rgba(199,166,106,0.2)` }}>{h}</span>
                      ))}
                    </div>
                    <div className="mt-auto">
                      {event.status === 'ثبت‌نام باز' && <button className="w-full py-2.5 rounded-xl font-bold text-sm" style={{ background: `linear-gradient(135deg,${GOLD},#A07840)`, color: '#F7F7F5' }}>ثبت‌نام در مسابقه</button>}
                      {event.status === 'در جریان' && <button className="w-full py-2.5 rounded-xl font-bold text-sm" style={{ background: `linear-gradient(135deg,${GOLD},#A07840)`, color: '#F7F7F5' }}>مشاهده نتایج زنده</button>}
                      {event.status === 'پایان یافته' && <button className="w-full py-2.5 rounded-xl font-bold text-sm" style={{ background: 'rgba(107,114,128,0.12)', color: '#6b7280', border: '1px solid rgba(107,114,128,0.2)' }}>مشاهده نتایج نهایی</button>}
                      {event.status === 'زود هنگام' && <button className="w-full py-2.5 rounded-xl font-bold text-sm" style={{ background: 'rgba(167,139,250,0.12)', color: '#a78bfa', border: '1px solid rgba(167,139,250,0.3)' }}>یادآوری برایم بگذار</button>}
                    </div>
                  </div>
                </Link>
              );
