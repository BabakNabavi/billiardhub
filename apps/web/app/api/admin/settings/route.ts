...
const WRITABLE: Record<string, 'bool' | 'json'> = {
  ...
  /* کدِ متنِ هر الگوی پیامک، همان‌طور که پنل ملی‌پیامک می‌دهد:
       { "booking_confirmed": 524, "role_approved": 531, … }
     در کد نیست چون کد را پنل بعد از تأییدِ هر متن می‌دهد و با هر
     اصلاحِ متن عوض می‌شود؛ گذاشتنش در کد یعنی هر بار یک دیپلوی. */
  sms_body_ids: 'json',
  ...
  /* ── `platform_bank` و `story_platform_bank` حذف شدند ──
     هر دو فقط نوشته می‌شدند و خوانده نمی‌شدند: فرمِ پنل ذخیره‌شان
     می‌کرد و همان فرم دوباره می‌خواندشان، و هیچ صفحه‌ی کاربری یا
     مسیرِ پرداختی سراغشان نمی‌رفت.

     خریدِ بسته‌ها از اول از درگاه می‌رود، پس حسابِ دومی معنا ندارد.
     نگه‌داشتنِ فرمِ فعال هم این توقع را می‌ساخت که جایی مؤثر است —
     و اگر روزی کسی آن شماره را واقعاً به خریدار نشان می‌داد، پولی
     بیرونِ سایت جابه‌جا می‌شد که نه در دفترِ مالی می‌نشیند نه
     قابلِ بازپرداخت است. */
  ...
  /* تعرفه‌ی ارتقای آگهی (مهاجرت ۰۷۹) — تازه‌سازی و فوری. قیمت عوض
     می‌شود و نباید دیپلوی بخواهد، پس این‌جاست نه در کد. */
  ad_boost_pricing: 'json',
};
