...
/* وضعیت بسته و سهمیه‌ی استوری خود کاربر.

   کلید شمارش استوری‌ها ownerKey است (همان چیزی که موقع انتشار ثبت
...
export async function GET(req: NextRequest) {
  ...
  /* پارامتر `ownerKey` از کوئری حذف شد.

     هیچ فراخوانی‌ای در پروژه آن را نمی‌فرستاد (هر سه مصرف‌کننده بدون
     پارامتر صدا می‌زنند)، ولی چون در شمارش شرکت می‌کرد، هر کسی
     می‌توانست کلید شخص دیگری را بفرستد و از عدد برگشتی تعداد
     استوری او را استنتاج کند. مجوز هیچ‌وقت به آن وابسته نبود، پس
     حذفش هیچ رفتاری را عوض نمی‌کند و یک ورودی کلاینت‌محور کم می‌شود. */
