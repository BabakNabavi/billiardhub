...
export async function POST(req: NextRequest) {
  ...
  const callbackUrl = `${callbackOrigin()}/api/stories/plans/callback/${provider.name}?order=${order.id}`;

  const res = await provider.createPayment({
    paymentId: order.id,
    amount: plan.price,
    description: `بسته‌ی استوری ${plan.name}`,
    callbackUrl,
  });

  if (!res.ok || !res.redirectUrl) {
    await sb().from('story_plan_orders').update({ status: 'FAILED' }).eq('id', order.id);
    return NextResponse.json({ message: res.message || 'اتصال به درگاه انجام نشد' }, { status: 502 });
  }

  await sb().from('story_plan_orders').update({
    provider: provider.name, provider_authority: res.authority ?? null,
  }).eq('id', order.id);

  void audit({
    actorId: actor.id, actorRole: actor.role, action: 'STORY_PLAN_ORDER_CREATED',
    entityType: 'story_plan_order', entityId: order.id,
