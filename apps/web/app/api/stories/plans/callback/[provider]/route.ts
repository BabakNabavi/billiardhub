...
async function handle(req: NextRequest, providerName: string) {
  ...
  const done = (ok: boolean, extra = '') =>
    NextResponse.redirect(new URL(`/story-plans/result?ok=${ok ? 1 : 0}${extra}`, callbackOrigin()), { status: 303 });
  const fail = (msg: string) => done(false, `&reason=${encodeURIComponent(msg)}`);

  if (!orderId) return fail('سفارش نامعتبر');

  const { data: oRow } = await sb().from('story_plan_orders')
  ...
  if (ret.canceled) {
    await sb().from('story_plan_orders').update({ status: 'CANCELED' }).eq('id', o.id);
  ...
  if (!v.ok || !v.paid) {
    await sb().from('story_plan_orders').update({ status: 'FAILED' }).eq('id', o.id);
  ...
  void audit({
    actorId: o.user_id, action: 'STORY_PLAN_ACTIVATED', entityType: 'story_plan_order', entityId: o.id,
