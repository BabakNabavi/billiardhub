...
/* حسابِ بانکیِ باشگاه — ثبت/تغییر توسطِ مالک، تأیید توسطِ ادمین.
   هر تغییر، حساب را به وضعیتِ «در انتظارِ تأیید» می‌برد و تا تأیید، تسویه ممکن نیست. */

const normIban = (v: string) => String(v || '').toUpperCase().replace(/[^A-Z0-9]/g, '')
const validIban = (v: string) => /^IR\d{24}$/.test(v)

/* POST { accountHolderName, bankName, iban, cardNumber? } — درخواستِ ثبت/تغییر */
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!holder) return NextResponse.json({ message: 'نامِ صاحبِ حساب الزامی است' }, { status: 400 });
  ...
  /* حسابِ قبلی غیرفعال و حسابِ جدید در انتظارِ تأیید ثبت می‌شود (تاریخچه حفظ می‌شود) */
  ...
  if (error) {
    if (/does not exist|schema cache/i.test(error.message)) return NextResponse.json({ message: 'سیستمِ مالی هنوز راه‌اندازی نشده است' }, { status: 503 });
    return NextResponse.json({ message: 'خطا در ثبتِ حسابِ بانکی' }, { status: 500 });
  }
  ...
  return NextResponse.json({ ok: true, status: 'PENDING', message: 'حساب ثبت شد و پس از تأییدِ ادمین قابلِ تسویه است' }, { status: 201 });
}
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (error || !data) return NextResponse.json({ message: 'حسابِ بانکیِ فعالی یافت نشد' }, { status: 404 });
