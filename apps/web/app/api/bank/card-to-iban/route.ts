...
export async function POST(req: NextRequest) {
  ...
  if (!cm.match) {
    return NextResponse.json({
      ok: true, match: false,
      message: 'این کارت بانکی به نام شما نیست. حساب باید متعلق به خود صاحب باشگاه باشد.',
    }, { status: 422 });
  }
  ...
  /* ── ۴ مقصد واقعی پول هم به نام خودش است؟ ── */
  ...
  if (clubId) {
    ...
    /* جدول تسویه هم همین‌جا به‌روز می‌شود، وگرنه تب مالی حساب را
       ثبت‌نشده می‌بیند و فرم دومی برای همان شبا نشان می‌دهد. */
