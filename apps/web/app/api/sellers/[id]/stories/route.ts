...
const indexPath = (id: string) => `sellers/${id}/stories/index.json`;
...
async function writeIndex(id: string, stories: any[]): Promise<void> {
  const buf = Buffer.from(JSON.stringify(stories), 'utf8');
  await getSupabaseServer()
    .storage.from(BUCKET)
    .upload(indexPath(id), buf, { upsert: true, contentType: 'application/json' });
}
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (active.length !== all.length) writeIndex(id, active).catch(() => {});
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await writeIndex(id, [...active, story]);
