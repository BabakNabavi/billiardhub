...
/* مهاجرتِ بی‌صدای کاربرانی که توکنشان در localStorage است.
   کلاینت یک‌بار توکنِ قدیمی را می‌فرستد، سرور آن را تأیید می‌کند و
   نشستِ کوکی‌محور می‌سازد؛ کاربر چیزی نمی‌بیند و لاگین می‌ماند.

   این مسیر موقتی است و پس از منقضی‌شدنِ آخرین توکنِ قدیمی (۷ روز)
   حذف می‌شود. چیزی به کسی نمی‌دهد که از قبل نداشته باشد: ورودی باید
   یک توکنِ معتبرِ امضاشده با همان JWT_SECRET باشد. */
