...
/* ثبت و تأیید ایمیل کاربر — با استعلام واقعی CheckEmail.
   ...
   محدودیتی که باید بدانیم: این سرویس «وجود و فعال بودن نشانی» را
   می‌سنجد، نه اینکه نشانی مال همین کاربر باشد. اثبات مالکیت به
   فرستادن کد نیاز دارد که سرویس جداگانه‌ای می‌خواهد. پس این تیک
...
/* حذف ایمیل — کاربر باید بتواند برش دارد */
export async function DELETE(req: NextRequest) {
  ...
  const { error } = await sb().from('users')
    ...
    .eq('id', actor.id);
  if (error) return NextResponse.json({ message: 'حذف ایمیل انجام نشد' }, { status: 500 });
