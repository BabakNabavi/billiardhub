...
/* یک مسابقه — خواندن، ویرایش، لغو.
   ...
   راهی برای اصلاحش نبود. غلطِ تایپی در عنوان، تاریخِ اشتباه یا مبلغِ
   نادرست ⇒ فقط لغو و ساختِ دوباره.

   حذفِ فیزیکی عمداً نیست: مسابقه‌ای که ثبت‌نام یا پرداخت داشته سابقه‌ی
   مالی دارد و پاک‌کردنش یعنی گم‌شدنِ ردِ پول. «لغو» جای حذف را می‌گیرد
   و فقط مسابقه‌ی پیش‌نویسِ بدونِ ثبت‌نام واقعاً حذف می‌شود. */

const UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const FORMATS = new Set(['bo3', 'bo5', 'bo7', 'bo9', 'bo11']);

/* وضعیت‌هایی که بعدشان دیگر ویرایشِ محتوایی مجاز نیست — مسابقه‌ی در
   حالِ اجرا یا تمام‌شده نباید تاریخ و مبلغش عوض شود. */
...
/* PATCH — ویرایشِ مسابقه توسطِ مالکِ باشگاه */
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (FROZEN.has(t.status)) {
    return NextResponse.json(
      { message: 'مسابقه‌ی در حالِ اجرا، تمام‌شده یا لغوشده ویرایش نمی‌شود' }, { status: 409 });
  }
  ...
  /* ظرفیت را نمی‌شود زیرِ تعدادِ ثبت‌نامِ فعلی آورد — وگرنه مسابقه‌ای
  ...
  if (b.maxPlayers !== undefined) {
    ...
    if ((count ?? 0) > n) {
      return NextResponse.json(
        { message: `ظرفیت نمی‌تواند کمتر از ${count} ثبت‌نامِ فعلی باشد` }, { status: 409 });
    }
    patch.max_players = n;
  }

  /* مبلغ فقط وقتی می‌تواند عوض شود که هنوز کسی پرداخت نکرده باشد.
     Snapshotِ مبلغ در هر ثبت‌نام ذخیره شده، ولی تغییرِ مبلغ پس از
     پرداخت یعنی دو نفر با دو قیمتِ متفاوت در یک مسابقه — و بازپرداخت
     هم مبهم می‌شود. */
  if (b.entryFee !== undefined) {
    ...
    if ((count ?? 0) > 0) {
      return NextResponse.json(
        { message: 'پس از اولین پرداخت، مبلغِ ورودی تغییر نمی‌کند' }, { status: 409 });
    }
  ...
  if (startsAt && regEnds && regEnds > startsAt) {
    return NextResponse.json(
      { message: 'مهلتِ ثبت‌نام نمی‌تواند بعد از تاریخِ برگزاری باشد' }, { status: 400 });
  }
...
/* DELETE — لغو (یا حذفِ واقعیِ پیش‌نویسِ خالی) */
export async function DELETE(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  /* پیش‌نویسِ بدونِ ثبت‌نام: چیزی برای نگه‌داشتن نیست ⇒ واقعاً حذف */
  ...
  /* ثبت‌نام دارد ⇒ فقط لغو، تا ردِ مالی و سابقه بماند.
     اعلانِ لغو پیش از هر چیز به ثبت‌نام‌کننده‌ها می‌رود. */
