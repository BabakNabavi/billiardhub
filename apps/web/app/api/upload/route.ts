...
/* ─────────────────────────────────────────────────────────────
   آپلودِ فایل — سمتِ سرور.

   تا امروز مرورگر مستقیم با کلیدِ anon در Storage می‌نوشت. یعنی
   اعتبارسنجیِ نوع و حجم فقط در جاوااسکریپت بود و هر کسی — حتی
   بدونِ حساب — می‌توانست هر فایلی با هر نامی بالا بدهد.

   حالا هر آپلود از این‌جا می‌گذرد و چهار چیز سرورساید بررسی می‌شود:
     ۱) نشستِ معتبر
     ۲) نوعِ واقعیِ فایل از روی بایت‌ها (نه از روی برچسبِ مرورگر)
     ۳) سقفِ حجم
     ۴) مسیرِ ایمن — بدونِ ../ و فقط زیرِ پیشوندهای مجاز
   ───────────────────────────────────────────────────────────── */
...
export async function POST(req: NextRequest) {
  ...
  if (!kind) {
    /* SVG و هر چیزِ ناشناخته‌ی دیگر عمداً رد می‌شود: SVG می‌تواند
  ...
  /* پسوند از نوعِ واقعی ساخته می‌شود، نه از نامِ ورودی */
