...
/* ─────────────────────────────────────────────────────────────
   Feature Flagها در سمت کلاینت.

   چرا Context و نه خواندن در هر کامپوننت: مقدار یک‌بار در کل اپ
   گرفته می‌شود و در ناوبری سمت کلاینت دوباره درخواست نمی‌رود
   (کش سطح ماژول). یعنی حداکثر یک درخواست کوچک برای هر بارگذاری
   کامل، آن هم با کش لبه.

   چرا پیش‌فرض «خاموش»: تا وقتی پاسخ نرسیده، رابط چیزی از قابلیت
   خاموش نشان نمی‌دهد. اگر پیش‌فرض «روشن» بود، در هر بارگذاری یک
   لحظه دکمه‌های لایک/پاسخ ظاهر و بعد ناپدید می‌شدند.

   هیچ دسترسی‌ای به window/localStorage در رندر اول نیست، پس SSR و
   hydration دست‌نخورده می‌مانند: سرور و کلاینت هر دو با همان مقدار
...
/* کش سطح ماژول: بین ناوبری‌های کلاینتی زنده می‌ماند. */
...
async function loadOnce(): Promise<Features> {
  ...
  inflight = fetch('/api/features')
    ...
    .catch(() => OFF)          // شکست شبکه ⇒ خاموش بماند (fail-closed)
 ...
 *  توجه: دیدن استوری زیر این پرچم نیست و همیشه در دسترس است. */
