...
import { Clapperboard, Loader2, ArrowLeft, Check } from 'lucide-react'
...
export default function MyStoryPlan({ compact = false }: { compact?: boolean }) {
      ...
      {!unlimited && (
        <>
          ...
          <div style={{ fontSize: 12.5, fontWeight: 800, color: left === 0 ? '#B23B2E' : FELT, marginBottom: 12 }}>
            {left === 0 ? 'سهمیه‌ی این بازه تمام شده' : `${fa(left)} استوری مانده`}
          </div>
        </>
      )}
          ...
          <div style={{ display: 'grid', gap: 7 }}>
            {paid.slice(0, 5).map(o => (
              <div key={o.id} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12.5, color: SEC }}>
                ...
                <span style={{ flex: 1 }}>{o.snapshot?.name ?? 'بسته‌ی استوری'}</span>
