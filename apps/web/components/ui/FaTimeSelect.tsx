...
/* ─────────────────────────────────────────────────────────────
   انتخابِ ساعت با ارقامِ فارسی.

   `<input type="time">`ِ بومی همیشه ارقامِ لاتین و قالبِ AM/PM را
   نشان می‌دهد و هیچ راهی برای فارسی‌کردنش نیست — ظاهرش را مرورگر
   می‌سازد، نه ما. برای همین با دو `<select>` جایگزین می‌شود.

   مقدار همان `HH:MM`ِ ۲۴ساعتیِ لاتین می‌ماند، پس جای هر
   `<input type="time">` می‌نشیند بدونِ تغییرِ منطقِ اطراف.
   ───────────────────────────────────────────────────────────── */
...
export default function FaTimeSelect({
  ...
  /** `HH:MM` ۲۴ساعتی — همان قالبِ input type="time" */
  value: string
  onChange: (next: string) => void
  /** ۱۵ برای ساعتِ کاری، ۵ یا ۱ اگر دقتِ بیشتری لازم شد */
  ...
  /* اگر دقیقه‌ی فعلی روی پله نیست، خودش هم در فهرست بیاید تا مقدارِ
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }}>
      <select
        aria-label={ariaLabel ? `ساعتِ ${ariaLabel}` : 'ساعت'}
