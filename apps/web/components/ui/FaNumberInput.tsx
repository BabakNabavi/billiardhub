...
/* ─────────────────────────────────────────────────────────────
   ...
   مقدارِ داخلش همان چیزی است که به سرور می‌رود و فارسی‌کردنش
   اعتبارسنجی و ذخیره را می‌شکند. نتیجه این بود که کاربر همه‌جا فارسی
   می‌دید جز داخلِ فیلدها — تعدادِ میز، قیمت، شماره، مبلغ.
   ...
   `<input>`ِ عددیِ موجود می‌نشیند بدونِ تغییرِ منطقِ اطراف.
   ───────────────────────────────────────────────────────────── */
...
export const toLatin = (v: string) =>
  String(v ?? '')
    ...
    .replace(/[٠-٩]/g, ch => String('٠١٢٣٤٥٦٧٨٩'.indexOf(ch)))   // ارقامِ عربی هم
...
export default function FaNumberInput({
  ...
  return (
    <input
      ...
      onChange={e => {
        /* هرچه تایپ شد — فارسی، عربی، لاتین یا با جداکننده — به لاتینِ
