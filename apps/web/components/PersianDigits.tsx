...
/* ─────────────────────────────────────────────────────────────
   نمایشِ فارسیِ همه‌ی ارقام — یک‌جا برای کلِ سایت.

   چرا این‌طوری و نه فایل‌به‌فایل: در پروژه بیش از ۸۵ نسخه‌ی محلیِ
   `toFa` وجود دارد و هر عددی که از دیتابیس، `toLocaleString`، یا
   کتابخانه‌ی بیرونی می‌آید از دستشان در می‌رود. این کامپوننت به‌جای
   دنبال‌کردنِ منبعِ اعداد، خروجیِ نهایی را اصلاح می‌کند: هر متنِ
   رندرشده در صفحه.
   ...
   چیزهایی که عمداً دست نمی‌خورند:
     ...
     • ویژگی‌ها (attribute) — فقط متنِ دیده‌شده عوض می‌شود

   نکته‌ی مهمِ همزیستی با React: وقتی React یک متن را دوباره می‌نویسد،
   نسخه‌ی لاتین برمی‌گردد؛ MutationObserver همان لحظه دوباره فارسی‌اش
   می‌کند. برای اینکه این رفت‌وبرگشت حلقه نشود، تغییرِ خودمان با یک
...
/* `OPTION` و `SELECT` عمداً **حذف شده‌اند** از این فهرست: متنِ گزینه
   چیزی است که کاربر می‌بیند و باید فارسی باشد، و `value` که منطق با آن
   کار می‌کند یک ویژگیِ جداست و دست نمی‌خورد. بودنشان این‌جا باعث شده
   بود همه‌ی عددهای داخلِ دراپ‌داون‌ها انگلیسی بمانند.
...
export default function PersianDigits() {
  useEffect(() => {
    ...
    /* در حالِ نوشتنِ خودمان ⇒ رویدادهای ناشی از آن نادیده گرفته شوند */
    let writing = false
