...
/* ─────────────────────────────────────────────────────────────
   بسته‌های آگهی — انتخاب و خرید.

   این بدنه پیش‌تر داخلِ `app/plans/page.tsx` بود و فقط از آن‌جا دیده
   می‌شد. حالا «مرکز تبلیغات» (/advertise) هم همین را نشان می‌دهد، پس
   به کامپوننتِ مشترک منتقل شد.

   عمداً کپی نشد: دو نسخه‌ی موازیِ همین کارت‌ها یعنی هر تغییرِ قیمت یا
   متن باید در دو جا انجام شود و روزی یکی‌شان جا می‌ماند. `/plans`
   همچنان کار می‌کند و همین کامپوننت را رندر می‌کند.

   ── تفاوتِ بسته و جایگاه ──
   بسته سهمیه‌ی *ثبت آگهی* می‌دهد؛ جایگاه فضای *تبلیغ* می‌فروشد. این
   دو در یک صفحه‌اند ولی یکی نیستند، و متنِ بالای هر بخش همین را
   می‌گوید تا کاربر بسته نخرد به امیدِ بنر.
   ───────────────────────────────────────────────────────────── */

import { useEffect, useState } from 'react'
import Link from 'next/link'
import { useRouter } from 'next/navigation'
import { Package, Check, Loader2, Sparkles, ArrowLeft } from 'lucide-react'
...
export default function AdPackages({ backTo = '/plans' }: { backTo?: string }) {
  ...
  const buy = async (planId: string) => {
    if (!user) { router.push(`/login?next=${encodeURIComponent(backTo)}`); return }
    ...
    <>
      ...
      {!quotaEnabled && (
        <div style={{
          ...
          خرید بسته فعلاً لازم نیست — این بخش برای وقتی است که محدودیت فعال شود.
        </div>
      )}
        ...
        <div style={{ display: 'grid', gap: 16, gridTemplateColumns: 'repeat(auto-fit,minmax(258px,1fr))' }}>
          {plans.map(p => (
            <article key={p.id} style={{
              ...
              <h3 style={{ fontSize: 17, fontWeight: 900, margin: '0 0 6px' }}>{p.name}</h3>
              {p.description && (
