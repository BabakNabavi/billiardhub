...
interface StoryGroup {
  ...
  roleLabel: string;
  allSeen: boolean;
  stories: { id: string; caption?: string; createdAt: string; }[];
}
    ...
    <>
      <style>{`
        ...
        .story-reaction-pop {
          ...
          font-size: 48px; pointer-events: none; z-index: 60;
          animation: reactionFloat 2.2s ease forwards;
        }
        .story-emoji-btn {
...
export default function Stories() {
  ...
  const [groups] = useState<StoryGroup[]>(sampleGroups);
        ...
        {groups.map((g, i) => {
          ...
          return (
            <div key={g.userId} className="story-thumb" onClick={() => openStory(i)}>
              ...
              <span style={{ fontSize: '11px', color: isSeen ? 'rgba(255,255,255,0.3)' : 'rgba(255,255,255,0.6)', maxWidth: '68px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {g.userName}
              </span>
              <span style={{ fontSize: '9px', color: isSeen ? 'rgba(255,255,255,0.2)' : g.roleColor, fontWeight: 700 }}>{g.roleLabel}</span>
            </div>
          );
        })}
      </div>
