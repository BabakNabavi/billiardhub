...
/* ─────────────────────────────────────────────────────────────
   جنسیت + رشته‌ها + دسته/رده‌ی سنی بازیکن.

   هرجا نقش بازیکن گرفته می‌شود (انتخاب نقش و پنل بازیکن) همین
   کامپوننت استفاده می‌شود تا دو جای مختلف دو لیست متفاوت نسازند —
...
export default function PlayerDisciplines({
  ...
  const setGender = (g: Gender) => {
    /* بانوان «دسته یک» ندارند — انتخاب نامعتبر را به دسته برتر برگردان */
    const fixed = g === 'f'
            ...
            <div key={e.discipline} className="rounded-xl border border-[#E7E2D6] bg-[#FAFAF7] p-3.5">
              ...
              {!isRanked(e.discipline) && (
                <p className="mt-2.5 text-[11.5px] text-[#A69F8E]">
                  های‌بال هنوز جدول رنکینگ ملی ندارد؛ این دسته‌بندی فقط روی پروفایل شما نمایش داده می‌شود.
                </p>
              )}
            </div>
          ))}
