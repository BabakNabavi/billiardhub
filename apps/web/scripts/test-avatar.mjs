/* تستِ ماندگاریِ عکسِ پروفایل.
       node scripts/test-avatar.mjs [BASE]

   باگی که این تست نگهبانش است: عکس فقط در localStorage می‌نشست و با
   خروج/ورودِ دوباره ناپدید می‌شد. این‌جا اثبات می‌شود که به دیتابیس
   می‌رسد و از آن‌جا هم خوانده می‌شود.

   مقدارِ اولیه‌ی `avatar` در پایان دقیقاً برگردانده می‌شود. */
...
const U = users?.[0]
if (!U) { console.error('کاربرِ آزمایشی پیدا نشد'); process.exit(1) }

const ORIGINAL = U.avatar ?? null
console.log(`  کاربر: ${U.id.slice(0, 8)}… · avatarِ اولیه: ${ORIGINAL ? 'دارد' : 'ندارد'}`)
...
/* یک PNG یک‌درپیکِ معتبر — کوچک‌ترین فایلِ ممکن برای تستِ آپلود */
...
try {
  head('۱) آپلودِ فایل روی استوریج')
  ...
  t('ستونِ users.avatar به‌روز شد', after?.avatar === url, `db=${String(after?.avatar).slice(0, 60)}`)

  head('۴) نشستِ تازه همان عکس را می‌بیند (سناریوی «رفتم و برگشتم»)')
  ...
  /* فیلدی که در فهرستِ سفید نیست نباید نوشته شود */
  ...
  t('نقش از راهِ PATCH بالا نمی‌رود', chk?.primaryRole === U.primaryRole,
    `role=${chk?.primaryRole} (بود ${U.primaryRole}) status=${escalate.status}`)

} finally {
  head('بازگردانیِ وضعیتِ اولیه')
  ...
  t('avatar به مقدارِ اولیه برگشت', (back?.avatar ?? null) === ORIGINAL, `now=${String(back?.avatar).slice(0, 40)}`)
}
