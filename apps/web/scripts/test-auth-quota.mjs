...
/* قفلِ IP (آستانه ۳۰) با اجراهای پیاپیِ همین تست پر می‌شود و آن‌وقت
...
if (ipSaturated) {
  console.log('\n⚠ قفلِ IP فعال است (اجراهای پیاپیِ تست). بخشِ ورود این بار اجرا نمی‌شود.')
  console.log('  خودِ این یعنی محافظتِ IP کار می‌کند؛ برای تستِ کاملِ ورود کمی صبر کنید.')
}
...
}  /* پایانِ بخشِ ورود (رد می‌شود اگر قفلِ IP فعال باشد) */

/* این یکی همیشه اجرا می‌شود: پیکربندی را از خودِ فایل می‌خواند، پس به
   وضعیتِ سرور وابسته نیست. */
