/* تست پرچم «تعاملات اجتماعی» + مشاهده‌ی عمومی استوری.
       node scripts/test-social-flag.mjs [BASE]

   نشست‌ها این‌جا با همان JWT_SECRET محلی امضا می‌شوند (هیچ مقداری چاپ
   نمی‌شود) تا مسیرهای احرازشده هم واقعاً تست شوند، نه فقط مسیر مهمان.
...
/* یک ادمین واقعی و یک کاربر عادی واقعی — فقط خوانده می‌شوند */
...
if (!ADMIN || !USER) { console.error('کاربر آزمایشی پیدا نشد'); process.exit(1) }
console.log(`  ادمین: ${ADMIN.id.slice(0, 8)}…  ·  کاربر عادی: ${USER.id.slice(0, 8)}… (نقش: ${USER.primaryRole})`)

const AT_ADMIN = sign(ADMIN)
const AT_USER = sign(USER)

/* وضعیت اولیه را نگه می‌داریم تا در پایان دقیقاً برگردانده شود */
const { data: before } = await sb.from('app_settings').select('value').eq('key', FLAG).maybeSingle()
const ORIGINAL = before ? before.value : null
console.log(`  وضعیت اولیه‌ی پرچم در دیتابیس: ${ORIGINAL === null ? '«ردیفی وجود ندارد»' : JSON.stringify(ORIGINAL)}`)
...
const setFlag = async on => {
  ...
  /* کش ۳۰ ثانیه‌ای در همان پروسه است و مسیر نوشتن پاکش می‌کند */
  return r
}
const flagState = async () => (await req('/api/features')).body?.socialInteractions

/* ════════════════════════════════════════════════════════════════
   بخش ۸ — امنیت تغییر پرچم
   ════════════════════════════════════════════════════════════════ */
head('بخش ۸ — چه کسی می‌تواند پرچم را عوض کند')
{
  ...
  t(`کاربر عادی (${USER.primaryRole}) ⇒ ۴۰۳`, normal.status === 403, `status=${normal.status}`)

  const guestRead = await req('/api/admin/settings')
  t('خواندن تنظیمات توسط مهمان ⇒ ۴۰۱', guestRead.status === 401, `status=${guestRead.status}`)

  const userRead = await req('/api/admin/settings', { headers: authHeaders(AT_USER) })
  t('خواندن تنظیمات توسط کاربر عادی ⇒ ۴۰۳', userRead.status === 403, `status=${userRead.status}`)

  /* نقش از توکن می‌آید ولی مرجع نهایی دیتابیس است — نقش جعلی در JWT
     نباید کافی باشد (isAdmin مستقیم از جدول users می‌خواند) */
  ...
  t('نقش admin جعل‌شده در توکن ⇒ باز هم ۴۰۳', spoofed.status === 403, `status=${spoofed.status}`)

  const ok = await setFlag(false)
  t('ادمین واقعی ⇒ موفق', ok.status === 200, `status=${ok.status}`)
}

/* ════════════════════════════════════════════════════════════════
   وضعیت خاموش
   ════════════════════════════════════════════════════════════════ */
head('پرچم = خاموش — وضعیت عمومی')
await setFlag(false)
...
head('بخش ۲ و ۱۳ — مهمان باید استوری ببیند')
{
  ...
  /* هیچ کلید خام مالک (شماره‌ی موبایل) نباید به مهمان برسد */
  const raw = JSON.stringify(s.body ?? [])
  const phoneLeak = /"ownerKey"\s*:\s*"09\d{9}"/.test(raw)
  t('کلید خام مالک (شماره‌ی موبایل) لو نمی‌رود', !phoneLeak)
  const privateLeak = /"(password|passwordHash|nationalId|email|refreshToken|token)"/i.test(raw)
  t('هیچ فیلد خصوصی دیگری در پاسخ نیست', !privateLeak)
  console.log(`      (${Array.isArray(s.body) ? s.body.length : 0} استوری زنده)`)
}
...
head('بخش ۲.F — مهمان نباید بتواند چیزی بسازد')
{
  ...
  t('ساخت استوری توسط مهمان ⇒ ۴۰۱', create.status === 401, `status=${create.status}`)
...
head('بخش ۱۴ — کاربر لاگین با پرچم خاموش')
{
  const view = await req('/api/social/stories')
  t('دیدن استوری همچنان کار می‌کند', view.status === 200, `status=${view.status}`)
  ...
  t('خواندن لیست گفتگوها ⇒ ۴۰۳', list.status === 403, `status=${list.status}`)

  const del = await req('/api/social/dm?conv=a__b', { method: 'DELETE', headers: authHeaders(AT_USER) })
  t('حذف گفتگو ⇒ ۴۰۳', del.status === 403, `status=${del.status}`)
  ...
  t('خوانده‌شده‌کردن اعلان ⇒ ۴۰۳', nr.status === 403, `status=${nr.status}`)

  /* پاسخ نباید جزئیات داخلی لو بدهد */
  const leak = JSON.stringify((await req('/api/social/dm', { headers: authHeaders(AT_USER) })).body)
  t('پیام خطا نام پرچم/جدول را لو نمی‌دهد',
    !/app_settings|social_interactions_enabled|supabase/i.test(leak), leak)
}

head('بخش ۱۸ — بدون درخواست اضافه وقتی خاموش است')
{
  /* پرچم عمداً کش نمی‌شود: هر درجه‌ای از کهنگی یعنی کاربر قابلیتی را
     که ادمین تازه روشن کرده نمی‌بیند. (نسخه‌ی اول این هدر
...
/* ════════════════════════════════════════════════════════════════
   بخش ۱۵ — روشن‌کردن دوباره: معماری باید سالم باشد
   ════════════════════════════════════════════════════════════════ */
...
{
  const list = await req('/api/social/dm', { headers: authHeaders(AT_USER) })
  t('لیست گفتگوها دوباره باز شد (۲۰۰)', list.status === 200, `status=${list.status}`)
  t('پاسخ همان شکل قبلی است (آرایه)', Array.isArray(list.body), typeof list.body)
...
head('بخش ۱۲ — هیچ داده‌ای حذف نشد')
{
  ...
  t('جدول app_settings سر جایش است', !error && typeof count === 'number', String(error?.message))
  console.log(`      ${count} کلید تنظیمات در دیتابیس`)
}

console.log(`\n${'─'.repeat(50)}\n  نتیجه: ${pass} موفق، ${fail} ناموفق`)
console.log(`  وضعیت نهایی پرچم: خاموش (کلید اولیه: ${ORIGINAL === null ? 'نبود' : JSON.stringify(ORIGINAL)})\n`)
