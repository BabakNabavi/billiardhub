...
while ((m = re.exec(doc))) {
  ...
  /* ── قاعده‌ی سوم: هر متغیر باید در متن معرفی شده باشد ──
     پاسخِ پشتیبانی روی چهار متنِ باشگاه: «متغیر ۲ و ۳ در خود متن مشخص
     شود». یعنی خواننده‌ی پیامک باید از روی متن بفهمد هر جای‌گذار چیست.

     تشخیصِ کاملش ممکن نیست (معنا را باید آدم بخواند)، ولی پرنشانه‌ترین
     حالتش قابلِ گرفتن است: دو جای‌گذارِ چسبیده به هم که هیچ کلمه‌ای
     بینشان نیست. متنِ ردشده دقیقاً همین بود: «{1} در {2} {3} است». */
  const adjacent = body.match(/\{\d+\}[\s:،.\-—]*\{\d+\}/)
  if (adjacent) {
    problems.push(`دو متغیرِ لخت پشتِ سرِ هم — بینشان برچسب لازم است: «${adjacent[0]}»`)
  }
...
/* ── قاعده‌ی چهارم: واژه‌های فیلترشده ──
   پنل بعضی واژه‌ها را — حتی داخلِ *مقدارِ* یک متغیر — رد می‌کند. این
   با ساختار کاری ندارد و فقط با رد شدن معلوم می‌شود، پس هرکدام که
   کشف شد این‌جا ثبت می‌شود تا دوباره تکرار نشود.

   کشف‌شده در تیکتِ ۱۴۰۵/۰۵/۱۴ روی فهرستِ دلیلِ گزارش تخلف. */
const FILTERED_WORDS = ['کلاهبرداری', 'خلاف']

/* فقط بخشِ «مقدارهای متغیرها» و خودِ متن‌ها — نه توضیحاتِ سند، وگرنه
   همین یادداشت خودش را قرمز می‌کند. */
...
for (const b of blocks) {
  for (const w of FILTERED_WORDS) {
    ...
    const line = b.split('\n').find(l => l.includes(w))
    console.log('⛔ واژه‌ی فیلترشده «' + w + '» در: ' + (line ?? '').trim())
    filtered++
  }
}

console.log(`\n${total} الگو بررسی شد — ${bad} مشکل‌دار`)
if (filtered) console.log(`${filtered} مورد واژه‌ی فیلترشده`)
process.exit(bad || filtered ? 1 : 0)
